...
\begin{multicols}{2}
  ...
  \cref{1997-1:eq:2}を代入すると
  \begin{align}
     & (a-1)x^2-2apx+(a+1)p^2 = 0                               \\
     & (a-1)\left(x-p\right)\left(x-\dfrac{a+1}{a-1}p\right) = 0
  \end{align}
  ...
  \begin{align}
     & C_1: 2\dfrac{a+1}{a-1}p                     \\
     & C_{p,q}: 2a\left(\dfrac{a+1}{a-1}p-p\right)
  \end{align}
  だから，これらが直交する条件より，
  \begin{align}
     & 2a\left(\dfrac{a+1}{a-1}p-p\right) \cdot 2\dfrac{a+1}{a-1}p = -1 \\
     & 8a \dfrac{a+1}{(a-1)^2} p^2 = -1
  \end{align}
  ...
  \begin{align}
    p^2 = -\dfrac{(a-1)^2}{8a(a+1)}
  \end{align}
  このような実数$p\neq 0$が存在するには，右辺が$0$より大きければよく，
  \begin{align}
    -\dfrac{(a-1)^2}{8a(a+1)} > 0
  \end{align}
  が求める$a$の条件である．
  \cref{1997-1:eq:3}より$a\neq 1$だから分子は負であり，よってこの条件を満たすには
  分母が負であれば良い．
  \begin{align}
     & a(a+1) < 0 \\
    \therefore
     & -1 < a < 0
  \end{align}
  これは\cref{1997-1:eq:3}の条件を満たしているから，求める条件は
  \begin{align}
    -1 < a < 0
  \end{align}
  である． $\cdots$(答)
