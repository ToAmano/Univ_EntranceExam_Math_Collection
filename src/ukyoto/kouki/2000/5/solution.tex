...
\begin{multicols}{2}
  ...
  \paragraph{(1)}
  与えられた$a_n$の表現に\cref{2000-5:eq:1}を代入して\cref{2000-5:eq:2}を用いると
  \begin{align}
    a_n
     & = r^n e^{n}(\theta) + \dfrac{1}{r^n} e^{-n}(\theta)                                                              \\
     & = r^n e(n\theta) + \dfrac{1}{r^n} e(-n\theta)                                                                    \\
     & = \left(r^n+\dfrac{1}{r^n}\right) \cos n\theta + i\left(r^n-\dfrac{1}{r^n}\right)\sin n\theta \label{2000-5:eq:3}
  \end{align}
  を得る．従って，
  \begin{align}
    |a_n|^2
     & = \left(r^n+\dfrac{1}{r^n}\right)^2 \cos^2 n\theta + \left(r^n-\dfrac{1}{r^n}\right)^2 \sin^2 n\theta           \\
     & = \left(r^{2n}+\dfrac{1}{r^{2n}}+2\right)\cos^2 n\theta + \left(r^{2n}+\dfrac{1}{r^{2n}}-2\right)\sin^2 n\theta \\
     & = r^{2n}+\dfrac{1}{r^{2n}}+2(\cos^2 n\theta - \sin^2 n\theta)                                                  \\
     & = r^{2n}+\dfrac{1}{r^{2n}}+2\cos 2n\theta
  \end{align}
  である．
  ...
  従って題意を示すには
  \begin{align}
    |\cos m\theta|> \dfrac{1}{2} \\
  \end{align}
  となる$m \in \mathbb{N}$があることを示せば良い．
  $\theta$の値で場合分けすると，以下のようになる．
  \begin{itemize}
    \item $|\cos\theta| > \dfrac{1}{2}$の時，$|a_1|=2|\cos\theta|>1$より$m=1$とすれば良い．
    \item $|\cos\theta| < \dfrac{1}{2}$の時，$|a_2| = 2|\cos 2\theta| = 2|2\cos^2\theta - 1| = 2(1-2\cos^2\theta)> 1$ より$m=2$とすれば良い．
    \item $|\cos\theta| = \dfrac{1}{2}$の時，$|a_3| = 2|\cos 3\theta| = 2|4\cos^3\theta - 3\cos\theta| = 2 > 1 $より$m=3$とすれば良い．
  \end{itemize}
  ...
  であり，
  \begin{align}
     & \lim_{n\to\infty} \left|\alpha^n\right| = \infty \\
     & \lim_{n\to\infty} \left|\alpha^{-n}\right| = 0
  \end{align}
  より追い出しの原理より
  \begin{align}
