...
\begin{multicols}{2}
  ...
  だから，$f'(x)=0$の解は
  \begin{align}
    a^{\alpha-1} = \dfrac{1}{\log a}
  \end{align}
  なる$\alpha$である．$a>1$よりこの両辺は正なので自然対数をとって，
  \begin{align}
    (\alpha-1) \log a & = - \log (\log a)                                     \\
    \alpha            & = 1 - \dfrac{\log(\log a)}{\log a} \label{1993-4:eq:1}
  \end{align}
  $f(x)$の増減表は\cref{1993-4:table:1}となる．
  ...
  \begin{align}
    f(\alpha)
     & = a\left[a^{\alpha-1}-\alpha\right] \\
     & = \dfrac{1}{\log a} -\alpha
  \end{align}
  だから，
  \begin{align}
        & f(\alpha) \ge 0              \\
    iff & \alpha \le \dfrac{1}{\log a }
  \end{align}
  \cref{1993-4:eq:1}を代入して
  \begin{align}
     & 1 - \dfrac{\log(\log a)}{\log a} \le \dfrac{1}{\log a } \\
     & \log\dfrac{a}{\log a } \le 1
  \end{align}
  なる条件を得る．
  ...
  \begin{align}
    g'(a) = \dfrac{e}{a} -1
  \end{align}
  だから，$g(x)$の増減表は\cref{1993-4:table:2}となる．
  ...
  \begin{table}[H]
    ...
    \end{tabular}
  \end{table}
  従って，$g(a) \ge 0$を満たす $a$ は $a=e$のみであり，これは\cref{1993-4:eq:3}を満たすから，
  求める$a$は
  \begin{align}
    a = e
  \end{align}
