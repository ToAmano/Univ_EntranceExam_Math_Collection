...
\[
\vec{b} = b \begin{pmatrix} 1 \\ 0 \end{pmatrix}, \quad \vec{a} = \dfrac{a}{2} \begin{pmatrix} 1 \\ \sqrt{3} \end{pmatrix}
\]
とおける。$AB$ の中点を $M$ とする。
\[
\vec{AB} = \begin{pmatrix} b - \dfrac{1}{2}a \\ -\dfrac{\sqrt{3}}{2}a \end{pmatrix}
\]
だから、$|MC| = \dfrac{\sqrt{3}}{2}|AB|$, $MC \perp AB$, $C$ が $O$ と反対側であることをあわせて、
\[
\vec{MC} = \dfrac{\sqrt{3}}{2} \begin{pmatrix} \dfrac{\sqrt{3}}{2}a \\ b - \dfrac{1}{2}a \end{pmatrix}
\]
となり、
\begin{align}
\vec{c} &= \vec{OM} + \vec{MC} \\
&= a \begin{pmatrix} 1 \\ 0 \end{pmatrix} + \dfrac{1}{2}b \begin{pmatrix} 1 \\ \sqrt{3} \end{pmatrix} \quad \dots \text{①}
\end{align}
一方、
\[
\dfrac{b}{a}\vec{a} + \dfrac{a}{b}\vec{b} = b \cdot \dfrac{1}{2} \begin{pmatrix} 1 \\ \sqrt{3} \end{pmatrix} + a \begin{pmatrix} 1 \\ 0 \end{pmatrix} \quad \dots \text{②}
\]
①, ②から
\[
