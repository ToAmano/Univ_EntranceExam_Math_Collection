...
\bigskip
\noindent{\bf [解2]}
対称性から，$y=x$の方を$\ell$とする．$A\left(p,\dfrac{1}{p}\right)$とする（$0<p<1$）．$A$での接線の傾きは$-\dfrac{1}{p^2}$であり，$\ell$と$x$軸の交点を$C$として，$|OA|=|AC|$となる．（$\because$ $OA$の傾き符号）$\angle AOC>\pi/4$から題意の条件は$\angle OAC=\pi/6$である．この時$\angle AOC=\dfrac{5}{12}\pi$となる．
\begin{align}
