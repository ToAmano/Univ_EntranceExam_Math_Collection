...
$S$ のベクトル $\vec{a}, \vec{b}$ のなす角を $\theta$ とすると ($0 \le \theta \le \pi$), 条件から
\[
2 \dfrac{|\vec{a}|}{|\vec{b}|} \cos\theta \in \mathbb{Z}
\]
又、$\vec{a}, \vec{b}$ を逆にした
\[
2 \dfrac{|\vec{b}|}{|\vec{a}|} \cos\theta \in \mathbb{Z}
\]
...
\]
\[
\iff \cos^2\theta = 0, \dfrac{1}{4}, \dfrac{1}{2}, \dfrac{3}{4}, 1
\]
\[
\iff \cos\theta = 0, \pm \dfrac{1}{2}, \pm \dfrac{1}{\sqrt{2}}, \pm \dfrac{\sqrt{3}}{2}, \pm 1
\]
\[
\iff \theta = 0, \dfrac{\pi}{6}, \dfrac{\pi}{4}, \dfrac{\pi}{3}, \dfrac{\pi}{2}, \dfrac{2}{3}\pi, \dfrac{3}{4}\pi, \dfrac{5}{6}\pi, \pi
\]
より題意は示された.
...
角が $0$ の時. $2 \dfrac{|\vec{a}|}{|\vec{b}|}, 2 \dfrac{|\vec{b}|}{|\vec{a}|} \in \mathbb{Z}$ から,
\[
2 \dfrac{|\vec{a}|}{|\vec{b}|} = k, \quad 2 \dfrac{|\vec{b}|}{|\vec{a}|} = m \quad (k, m \in \mathbb{N})
\]
...
\[
\sqrt{3} \dfrac{|\vec{a}|}{|\vec{b}|} = k, \quad \sqrt{3} \dfrac{|\vec{b}|}{|\vec{a}|} = m
\]
辺々かけて $3 = km$ より $(k, m) = (1, 3), (3, 1)$. これらは $|\vec{a}| : |\vec{b}| = k : \sqrt{3} = \sqrt{3} : m$ をみたすから、2ベクトルの比は $1 : \sqrt{3}$.

・角が $\pi/3$ の時, $\dfrac{|\vec{a}|}{|\vec{b}|}, \dfrac{|\vec{b}|}{|\vec{a}|} \in \mathbb{Z}$ から ベクトルの比は $1 : 1$.

\medskip
\paragraph{(3)}

まず, $\vec{a} = \begin{pmatrix} 1 \\ 0 \end{pmatrix}, \vec{b} = |\vec{b}| \begin{pmatrix} \cos \pi/6 \\ \sin \pi/6 \end{pmatrix}$ とおける. なす角が $120^\circ, 150^\circ$ の時の2ベクトルの比は (2)の $60^\circ, 30^\circ$ の場合に等しく, $90^\circ$ の時は鏡であることから、下図のようにとればこれらは集合 $S$ になる. (ただし片方のベクトルのなす角 $30^\circ$)
