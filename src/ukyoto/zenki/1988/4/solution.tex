...
$\alpha$とz軸の交点$H$は$OM=\sqrt{2}$から$H(0, 0, t\sqrt{2})$である。\\
...
(2)と同じくして$z=k\sqrt{2}$における切り口の面積$S(k)$は（$0 \le k \le t$）
\[ S(k) = 2(1-k) \cdot 2k = 4k(1-k) \]
だから、求める体積$V$は
\begin{align}
V &= \int_0^{t\sqrt{2}} S(k) dz \\
&= \int_0^t S(k) \dfrac{dz}{dk} dk \\
&= \int_0^t 4k(1-k) \sqrt{2} dk \\
&= 4\sqrt{2} \left[ \dfrac{1}{2}k^2 - \dfrac{1}{3}k^3 \right]_0^t \\
&= 4\sqrt{2} \left( \dfrac{1}{2}t^2 - \dfrac{1}{3}t^3 \right) \quad //
\end{align}
