...
\noindent
[\textbf{解}] $OX$ を $x$ 軸、$O$ を原点とする右のような座標を考え、$OY$ の方向ベクトル $
\begin{pmatrix} \cos\theta \\ \sin\theta
\end{pmatrix}$ とおく ($0 < \theta < \pi$)。又点 $A(\alpha, \beta)$ とおく。\\
...
\[
  \tan \theta = \dfrac{\sin\theta}{\cos\theta} = \dfrac{|PQ| \cdot |PR| \sin\theta}{|PQ| \cdot |PR| \cos\theta} = \dfrac{2 (\triangle PQR \text{の面積})}{\vec{PQ} \cdot \vec{PR}}
\]
...
だから $t_1, t_2$ は方程式
\begin{equation}
  \dfrac{|\beta(t-\alpha) + \alpha\beta|}{-\alpha(t-\alpha) + \beta^2} = \dfrac{|\beta(t\cos\theta-\alpha) - \alpha(t\sin\theta-\beta)|}{-\alpha(t\cos\theta-\alpha) - \beta(t\sin\theta-\beta)} \dots \text{①}
\end{equation}
の2異実数解（ただし $C = \cos\theta, S = \sin\theta$ ときて、$
\begin{pmatrix} \alpha \\ \beta
\end{pmatrix} = r
\begin{pmatrix} \cos\varphi \\ \sin\varphi
\end{pmatrix}$ ($0 < r, 0 < \varphi < \theta$) とおいて、①に代入すると
\[
  \dfrac{|r\sin\varphi \cdot t|}{-r\cos\varphi \cdot t + r^2} = \dfrac{|r(\sin\varphi \cdot C - \cos\varphi \cdot S) t|}{-r(\cos\varphi \cdot C + \sin\varphi \cdot S) t + r^2}
\]
$t > 0, \sin\varphi > 0, t > 0, \sin(\theta-\varphi) > 0$ から
\[
  \dfrac{\sin\varphi}{-\cos\varphi \cdot t + r} = \dfrac{\sin(\theta-\varphi)}{-\cos(\theta-\varphi) \cdot t + r}
\]
\[
...
$\varphi = \dfrac{\theta}{2}$、つまり $A$ が $\angle XOY$ の2等分線上にある時。
...
正弦定理から
\[
  OP = \dfrac{a \sin\alpha}{\sin(\pi - \theta - \alpha)}, \quad OQ = \dfrac{a \sin\alpha}{\sin(\pi - \psi - \alpha)}
\]
...
\]
\[
  -2 \sin\left(\dfrac{\theta - \psi}{2}\right) \cos\left(\dfrac{\theta + \psi}{2} - \alpha\right) = 0
\]
$0 < \dfrac{\theta + \psi}{2} < \dfrac{\pi}{2}, 0 < \alpha < \pi$ から
\[
  \theta = \psi \quad \text{or} \quad \dfrac{\theta + \psi}{2} = \alpha
\]
$\theta = \psi$ の時 OK、$\dfrac{\theta + \psi}{2} = \alpha$ の時これを満たす $\alpha$ は2つはないからアウト。
