...
$b, c \in \mathbb{R}$ であるから、この時、$\beta = \overline{\alpha}$ となるので、
\[
\begin{cases}
\alpha + \beta = -2b \quad (\neq 0) \\
...
$\gamma = \dfrac{v}{2b}(\alpha + \beta) + \dfrac{w}{2d}(\alpha - \beta) = \left( -\dfrac{v}{2b} + \dfrac{w}{2d} \right) \alpha + \left( -\dfrac{v}{2b} - \dfrac{w}{2d} \right) \beta$\\
と表せる。$\blacksquare$

\paragraph{(2)}

(1)から、$b^2 - c \ge 0$ 又は $b = 0$ が必要である。\\
ア) $b^2 - c \ge 0$ の時\\
