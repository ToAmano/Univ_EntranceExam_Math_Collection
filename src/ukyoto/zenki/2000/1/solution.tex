...
$|\vec{p}|=1$ をみたすので、$|\vec{a}|=|\vec{b}|=|\vec{c}|=1$, $\vec{a}\cdot\vec{b} = \vec{b}\cdot\vec{c} = \vec{c}\cdot\vec{a} = -\dfrac{1}{2}$ より
\begin{align}
...
\end{align}
\[ \therefore k=0, \ \dfrac{1}{p^2-p+1} \]
$k=0$ は $A$ に対応するから $P$ に対応するのは $k = \dfrac{1}{p^2-p+1}$ で、①から
\begin{align}
\overrightarrow{AP} &= -\dfrac{1}{p^2-p+1}\vec{a} + \dfrac{1-p}{p^2-p+1}\vec{b} + \dfrac{p}{p^2-p+1}\vec{c} \\
&= \dfrac{1-p}{p^2-p+1}\overrightarrow{AB} + \dfrac{p}{p^2-p+1}\overrightarrow{AC}
\end{align}
