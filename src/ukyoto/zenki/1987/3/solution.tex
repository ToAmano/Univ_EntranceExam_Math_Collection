...
$2^\circ \ a>0$の時\\
$f(c)$の軸 $c = -\dfrac{b}{4a}$ によらず、$f(c)$は $c = \pm 1$で最大で
\begin{align}
&f(1) < 0 \land f(-1) < 0 \\
...
$3^\circ \ a<0$の時
\begin{align}
&\text{\textcircled{ア}} \ -1 \le -\dfrac{b}{4a} \le 1 \text{の時} \quad f(-\dfrac{b}{4a}) < 0 \iff \dfrac{(a+\dfrac{1}{2})^2}{1/4} + \dfrac{b^2}{1} < 1 \quad \cdots \text{A} \\
&\text{\textcircled{イ}} \ -\dfrac{b}{4a} \le -1 \text{の時} \quad f(-1) < 0 \\
&\text{\textcircled{ウ}} \ -\dfrac{b}{4a} \ge 1 \text{の時} \quad f(1) < 0
\end{align}

以上まとめて
\begin{itemize}
\item $a \ge 0 \land a-1 < b < a+1$
\item $4a \le b \le -4a \land a < 0 \land \dfrac{(a+\dfrac{1}{2})^2}{1/4} + \dfrac{b^2}{2} < 1$
...
\node at (-0.33, 0) [below] {$-\dfrac{1}{3}$};
\node[blue] at (1.5, 1) {[本時のミス]};
\node[blue] at (1.5, 0.5) {$\cdot$ 計算ミス};
\node[blue] at (1.5, 0) {$\cdot$ 軸についての勘違い};
\node at (-1, 1.5) {$\dfrac{(a+1/2)^2}{1/4} + \dfrac{b^2}{2} = 1$};
\node at (2.5, 2) {$4(a+1/2)^2 + \dfrac{b^2}{2} = 1, \ b = \pm (a-1) \text{ を連立すると}$};
\node at (2.5, 1.5) {$4(a+1/2)^2 + \dfrac{1}{2}(a^2-2a+1) = 1$};
\node at (2.5, 1) {$9a^2+6a+1 = 0 \qquad \therefore a=-\dfrac{1}{3}$};
