...
また $p = s+c$ とすると 
\begin{align}
 sc = \frac{p^2-1}{2}, -\sqrt{2} \leqq p \leqq \sqrt{2} \label{eq:1}
\end{align}
...
\begin{tikzpicture}[scale=1.1, >=stealth]
  ...
  % 基準線 y = 1/2
  \draw[dashed] (0, 0.5) node[left, font=\small, black] {$\frac{1}{2}$} -- (6.0, 0.5);
  ...
  % 主な x 軸ラベル
  \node[below, font=\scriptsize] at (0.785, 0) {$\frac{\pi}{4}$};
  \node[below, font=\scriptsize] at (3.927, 0) {$\frac{5\pi}{4}$};
...
\end{cases}
\end{align*}
となる．端点での値$-4$と比べると $-4 > -\frac{7}{4}\sqrt{7}$ だから求める最大最小値は
\begin{align*}
