\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

{\bf [解]}
$A: x^2+y^2=4$，$B_0: (x-3)^2+y^2=1$とおく．$B_0$の中心$C_0$，$B$の中心$C$，$A$と$B$の接点を$D$とおく．$\angle DOC_0=\theta$（$0\le\theta\le2\pi$）の時，弧長$=2\theta$（$P_0\to P$まで，$A$と接していた方の弧）だから，$\angle PCD$（弧度を含む方）$=2\theta$であり，$P$の座標$P(X,Y)$は
\begin{align}
\begin{pmatrix}X\\Y\end{pmatrix}=\overrightarrow{OC}+\overrightarrow{CP}=3\begin{pmatrix}\cos\theta\\\sin\theta\end{pmatrix}+\begin{pmatrix}\cos(3\theta-\pi)\\\sin(3\theta-\pi)\end{pmatrix}=3\begin{pmatrix}C\\S\end{pmatrix}-\begin{pmatrix}\cos3\theta\\\sin3\theta\end{pmatrix}
\end{align}
とかける（$C=\cos\theta,\ S=\sin\theta$）．
\begin{align}
\dfrac{dX}{d\theta}=-3S+3(3S-4S^3)=3S(2-4S^2)
\end{align}
\begin{align}
\dfrac{dY}{d\theta}=3C-3(4C^3-3C)=3C(4-4C^2)
\end{align}
から下表をうる．ただし，$\left.\begin{pmatrix}X\\Y\end{pmatrix}\right|_{\theta=t}$と$\left.\begin{pmatrix}X\\Y\end{pmatrix}\right|_{\theta=2\pi-t}$（$0\le t\le\pi$）が$x$軸に関して対称なので，$0\le\theta\le\pi$とした．
\begin{center}
\begin{tabular}{c|ccccccccc}
$\theta$ & $0$ & & $\dfrac{\pi}{4}$ & & $\dfrac{\pi}{2}$ & & $\dfrac{3}{4}\pi$ & & $\pi$ \\
\hline
$X'$ & $+$ & $+$ & $0$ & $-$ & $-$ & $-$ & $0$ & $+$ & \\
\hline
$Y'$ & $+$ & $+$ & $+$ & $0$ & $-$ & $-$ & $-$ & $-$ & \\
\hline
$(X,Y)$ & $(2,0)$ & $\nearrow$ & $(2\sqrt{2},\sqrt{2})$ & $\nwarrow$ & $(0,4)$ & $\swarrow$ & $(-2\sqrt{2},\sqrt{2})$ & $\searrow$ & $(-2,0)$
\end{tabular}
\end{center}
したがって，$C$（曲線）で$x$座標が最大なのは，$(2\sqrt{2},\pm\sqrt{2})$である．

又，$C$の長さを$L$として，$(X,Y)$が$x$軸についても対称なことから，
\begin{align}
\dfrac{L}{4}=\int_0^{\dfrac{\pi}{2}}\sqrt{\left[3S(2-4S^2)\right]^2+\left[3C(4-4C^2)\right]^2}\,d\theta
\end{align}
\begin{align}
=\int_0^{\dfrac{\pi}{2}}\sqrt{36S^2(1-2S^2)^2+144C^2S^4}\,d\theta
\end{align}
\begin{align}
=\int_0^{\dfrac{\pi}{2}}\sqrt{36\sin^2\theta}\,d\theta
\end{align}
\begin{align}
=\int_0^{\dfrac{\pi}{2}}6\sin\theta\,d\theta \quad(\because\sin\theta\ge0)
\end{align}
\begin{align}
=6\left[-\cos\theta\right]_0^{\dfrac{\pi}{2}}=6
\end{align}
\begin{align}
L=24
\end{align}


\newpage

\end{document}
