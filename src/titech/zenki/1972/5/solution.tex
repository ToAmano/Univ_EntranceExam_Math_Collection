...
\begin{align}
V &= 
  \left( \text{\begin{tikzpicture}[baseline={(0,-0.2)}, scale=0.8, >=stealth]
    % 直角三角形
 ...
 &= \frac{1}{3}\pi e - \pi\int_1^e (\log x)^2 dx \\
 &= \frac{1}{3}\pi e - \pi\left[x(\log x)^2-2x\log x+2x\right]_1^e \\
 &= \frac{1}{3}\pi e - \pi\left(e-2\right) \\
 &= \pi\left(-\frac{2e}{3}+2\right)
\end{align}
である．
...
\begin{align}
 \int (\log x)^n \, dx 
    ...
    &= x \sum_{k=0}^{n} (-1)^k \frac{n!}{(n-k)!} (\log x)^{n-k} + C
\end{align}
