\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

{\bf [解]}
$q=p(p^2-3)$とかける．又，
\begin{align}
f(x)=-(x-p)^2+p(p^2-3), \qquad g(x)=x(x^2-3)
\end{align}
とおくと$f(x)=g(x)$の異なる実解が2つである．
\begin{align}
\begin{split}
g(x)-f(x)&=x^3-3x+x^2-2px+p^2-p^3+3p \\
&=(x-p)\left(x^2+(p+1)x+(p^2-p-3)\right)
\end{split}
\end{align}
$\sim$部を$h(x)$とかくと，したがって，$h(x)=0$が$x\ne p$にただ1つ実解を持つ

\subparagraph{⑦ $h(x)=0$が$x\ne p$に重解を持つ時}

判別式$D$として，
\begin{align}
D=-3p^2+6p+13
\end{align}
だから，$D=0\iff p=\dfrac{3\pm4\sqrt{3}}{3}$の時，だがこれは$0<p<2$に反し不適

\subparagraph{⑦ $h(x)=0$が$x=p$を解に持つ時}

\begin{align}
h(p)=3(p^2-1)=0\iff p=\pm1
\end{align}
であり，$0<p<2$から$p=1$である．この時，$h(x)=0$の解は$x=1,-3$である

以上から，$(p,q)=(1,-2)$であり，グラフの概形は下図．

\begin{figure}[htb]
\begin{center}
\begin{tikzpicture}[xscale=0.9, yscale=0.14, >=stealth]
  % 座標軸
  \draw[->] (-4,0) -- (3,0) node[right] {$x$};
  \draw[->] (0,-25) -- (0,10) node[above] {$y$};
  % y=g(x)=x^3-3x, y=f(x)=-(x-1)^2-2 (p=1,q=-2)
  \draw[domain=-3.2:2.1, smooth] plot (\x,{\x*\x*\x-3*\x}) node[right] {$y=g(x)$};
  \draw[domain=-3.2:4.3, smooth] plot (\x,{-(\x-1)*(\x-1)-2}) node[below right] {$y=f(x)$};
  % 面積S: 2曲線で囲まれた部分 (x=-3から1)
  \fill[gray!30, domain=-3:1, variable=\x] plot ({\x},{\x*\x*\x-3*\x}) -- plot[domain=1:-3] ({\x},{-(\x-1)*(\x-1)-2}) -- cycle;
  \fill (-3,-18) circle (2pt) node[left] {$-3$};
  \fill (1,-2) circle (2pt) node[below] {$1$};
\end{tikzpicture}
\end{center}
\caption{$y=f(x)$，$y=g(x)$のグラフの概形}
\end{figure}

もとめる面積$S$として
\begin{align}
S=\int_{-3}^1(g(x)-f(x))dx=\int_{-3}^1(x-1)^2(x+3)dx=\dfrac{1}{12}\cdot4^4=\dfrac{64}{3}
\end{align}


\newpage

\end{document}
