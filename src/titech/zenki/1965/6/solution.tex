...
\end{align}
だから，この交点は $\left( \dfrac{\alpha+\beta}{2}, \alpha\beta \right)$ である．
題意よりこれが $(t, -1)$ に等しいので
\begin{align}
\begin{cases}
\alpha + \beta = 2t \\
...
S(t) 
  ...
  &= \dfrac{1}{12} (\beta - \alpha)^3 \label{eq:2}
\end{align}
である．以下これを$t$で書き直そう．
...
\begin{align}
\dfrac{S(t)}{\sqrt{t}} 
 &= \dfrac{1}{12} \dfrac{8(t^2+1)\sqrt{t^2+1}}{\sqrt{t}} \\
 &= \dfrac{2}{3} \sqrt{ \dfrac{(t^2+1)^3}{t} } \label{eq:3}
\end{align}
となる．以下この最小値を求める．
簡単のため
\begin{align}
 f(t) = \dfrac{(t^2+1)^3}{t}
\end{align}
とおく．$p = t^{\dfrac{1}{3}}$ とおいて$f(t)$を書き直して評価すると $p > 0$ で
\begin{align}
f(t)
  &= \left( \dfrac{p^6+1}{p} \right)^3 \\
  &= \left( p^5 + \dfrac{1}{p} \right)^3 \\
  &= \left( p^5 + 5 \cdot \dfrac{1}{5p} \right)^3 \\
  &\ge \left( 6 \sqrt[6]{ p^5 \cdot \left(\dfrac{1}{5p}\right)^5 } \right)^3 \\
  &= 6^3 \cdot \left(\dfrac{1}{5}\right)^{\dfrac{5}{2}} \quad (\because \text{AM-GM})
\end{align}
なる評価を得る．等号成立は $p = \left(\dfrac{1}{5}\right)^{\dfrac{1}{6}}$ つまり $t = \sqrt{1/5}$ の時である．

従って\cref{eq:3}に代入して求める最小値は
\begin{align}
\min \dfrac{S(t)}{\sqrt{t}} 
 &= \dfrac{2}{3} \sqrt{ 6^3 \left(\dfrac{1}{5}\right)^{\dfrac{5}{2}} } \\
 &= \dfrac{4}{5} \sqrt{\dfrac{6}{\sqrt{5}}}
\end{align}
である．
