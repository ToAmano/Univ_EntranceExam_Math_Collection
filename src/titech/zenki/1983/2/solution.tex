...
\begin{tikzpicture}[scale=1.0, >=stealth]
  ...
  \node[below] at (0.134,-1.6) {$1-\frac{\sqrt{3}}{2}$};
  \node[below] at (1.866,-1.6) {$1+\frac{\sqrt{3}}{2}$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=3, >=stealth, every node/.style={font=\small}]
  ...
  \draw[domain=-0.5:2.5, smooth, thick, red!80!black, samples=60] 
    plot (\x, { -(\x-1)*(\x-1) + 1.25 }) node[red!80!black, font=\small] {$y=-(x-1)^2+\frac{5}{4}$};

  % --- 5. 軸上の目盛りテキスト ---
  \node[below] at (0.134, 0) {\scriptsize $1-\frac{\sqrt{3}}{2}$};
  \node[below] at (1.866, 0) {\scriptsize $1+\frac{\sqrt{3}}{2}$};
  \node[below] at (1, 0) {$1$};
