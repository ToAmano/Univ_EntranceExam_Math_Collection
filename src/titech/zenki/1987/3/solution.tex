\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
  \footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

{\bf [解]}
\paragraph{(1)}

$\triangle ABC$の外接円の半径1として良い．$\triangle ABC$が鈍角三角形でないとき，
\begin{align}
  A(1,0), \quad B(\cos\alpha,\sin\alpha), \quad C(\cos\beta,\sin\beta) \quad\left(0<\alpha<\dfrac{\pi}{2},\ \pi\le\beta\le\alpha+\pi\right)
\end{align}
とおいて良い．$D$の中心を$O'$とし，$O'\ne O$と仮定する．$D$の条件から
\begin{align}
  |AD|\le1,\ |BD|\le1,\ |CD|\le1 \iff |AD|^2\le1,\ |BD|^2\le1,\ |CD|^2\le1 \quad(\because\text{各辺}\ge0)
\end{align}
$D(X,Y)$とおくと，
\begin{align}
  (X-1)^2+Y^2\le1 \wedge (X-\cos\alpha)^2+(Y-\sin\alpha)^2\le1 \wedge (X-\cos\beta)^2+(Y-\sin\beta)^2\le1 \quad\cdots\text{①}
\end{align}
である．3円$P,Q,R$を
\begin{align}
  P: (X-1)^2+Y^2=1, \qquad Q: (X-\cos\alpha)^2+(Y-\sin\alpha)^2=1, \qquad R: (X-\cos\beta)^2+(Y-\sin\beta)^2=1
\end{align}
で定める．$P,Q,R$の$(0,0)$での接線の方向ベクトルは各々$
\begin{pmatrix}0\\1
\end{pmatrix}$，$
\begin{pmatrix}-\sin\alpha\\\cos\alpha
\end{pmatrix}$，$
\begin{pmatrix}\sin\beta\\-\cos\beta
\end{pmatrix}$だから，これを図示すると下図（$\because *$）．

$P$は$Q$の左側，$Q$は$R$の上側，$R$は$P$の左側にあるから，$P,Q,R$の共有点は$O(0,0)$のみ，つまり
\begin{align}
  ①\iff(X,Y)=(0,0)
\end{align}
であるが，これは$O'\ne O$に反し矛盾．以上から示された．

\textbf{(2)} (1)のベクトル図から，$\triangle ABC$が鈍角三角形になれば①の領域式が$(0,0)$以外に広がり，$D\ne O$となる．したがって，
\begin{align}
  \triangle ABC\text{が非鈍角三角形}\iff D=O \quad\cdots\text{②}
\end{align}
となる．したがって，$\triangle ABC$が鈍角三角形になるような$C$の軌跡を求めれば良い．対称性から，$0\le x,\ 0<y$で考える．又，$C(x,y)$とする．

\subparagraph{$1^\circ$ $1<x$の時}

$\angle CAB>\pi/2$となり，$\triangle ABC$は鈍角三角形．

\subparagraph{$2^\circ$ $0\le x\le1$の時}

$\angle CAB\le\pi/2,\ \angle CBA\le\pi/2$だから，$\angle ACB>\pi/2$となる条件をしらべれば良い．このような条件は
\begin{align}
  x^2+y^2<1
\end{align}
である．

以上$1^\circ,2^\circ$から，$0\le x,\ 0<y$では，題意の条件は $1<x$ or $x^2+y^2<1$だから，対称性から求める領域式は
\begin{align}
  |x|>1 \ \text{または}\ x^2+y^2<1 \quad(y\ne0)
\end{align}
であり，制約$x^2+y^2\le4$とあわせて図示して下図斜線部（境界は$x^2+y^2=4$のみ含み，$x$軸は含まず）．

\begin{figure}[htb]
  \begin{center}
    \begin{tikzpicture}[scale=1.1, >=stealth]
      % 座標軸
      \draw[->] (-2.6,0) -- (2.6,0) node[right] {$x$};
      \draw[->] (0,-2.6) -- (0,2.6) node[above] {$y$};
      % 制約 x^2+y^2<=4 (境界含む) と 補助円 x^2+y^2=1 (境界含まず)
      \draw (0,0) circle (2);
      \draw[dashed] (0,0) circle (1);
      % 領域: |x|>1 または x^2+y^2<1 (かつ x^2+y^2<=4)
      \begin{scope}
        \clip (0,0) circle (2);
        \fill[gray!30] (-2.6,-2.6) rectangle (-1,2.6);
        \fill[gray!30] (1,-2.6) rectangle (2.6,2.6);
        \fill[gray!30] (0,0) circle (1);
      \end{scope}
      \draw[dashed] (-1,-2.6) -- (-1,2.6);
      \draw[dashed] (1,-2.6) -- (1,2.6);
      \draw[thick] (-2,0) -- (2,0);
      \node[below] at (-2,0) {$-2$};
      \node[below] at (-1,0) {$-1$};
      \node[below] at (1,0) {$1$};
      \node[below] at (2,0) {$2$};
      \node[left] at (0,2) {$2$};
      \node[left] at (0,1) {$1$};
      \node[left] at (0,-2) {$-2$};
    \end{tikzpicture}
  \end{center}
  \caption{条件を満たす領域の図示（斜線部）}
\end{figure}

\newpage

\end{document}
