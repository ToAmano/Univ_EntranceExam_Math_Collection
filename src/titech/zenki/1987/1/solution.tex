...
\subparagraph{⑦の時（$\alpha<0<\beta<\gamma$）は以下のように不適であることを示す}
...
\begin{align}
-1-\alpha<p<-\alpha, \qquad q\ge-\dfrac{\alpha}{1+\alpha}p
\end{align}
以上を図示すると，$-(1+\alpha)<0<-\alpha<1$と$p^2-4q>0$（すなわち$q<\dfrac{1}{4}p^2$）をあわせても，$-\dfrac{\alpha}{1+\alpha}p$と$\dfrac{1}{4}p^2$の大小関係および$p=-\alpha$での値を調べることで，条件をみたす$p,q$の存在範囲が空集合になる．すなわちこれをみたす$(\alpha,\beta,\gamma)$は存在しない．
