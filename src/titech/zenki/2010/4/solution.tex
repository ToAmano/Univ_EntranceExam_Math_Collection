...
&\Longleftrightarrow\ t\bigl[(x^2-2ax+y^2)t+2ax\bigr]\ge0.
\end{split}\tag{①}
\end{align}
...
\begin{tikzpicture}[scale=1.3]
  % a=2として図示。x>=0 から円の内部(開円板)を除いた領域
  \def\a{2}
  \begin{scope}
  ...
  % O, A は除外点なので白抜きの丸で示す
  \filldraw[white,draw=black,thick] (0,0) circle (0.9pt);
  \filldraw[white,draw=black,thick] (\a,0) circle (0.9pt);
