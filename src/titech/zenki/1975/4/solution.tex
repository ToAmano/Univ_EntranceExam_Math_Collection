...
\begin{align}
\ell_P: y=-\dfrac{1}{t^2}x+\dfrac{2}{t}, \qquad \ell_Q: y=\dfrac{1}{s^2}x+\dfrac{2}{s}
\end{align}
となる．
よって2接線の交点を$R$，$\ell_P$ の $x$ 切片 $T$，$\ell_Q$ の $x$ 切片 $U$ とするとこれらの座標は
\begin{align}
R\left(\dfrac{2ts(s-t)}{t^2+s^2},\dfrac{2(t+s)}{t^2+s^2}\right), \qquad T(2t,0), \qquad U(-2s,0) \label{eq:1}
\end{align}
である．
よってグラフは以下のようになる．
...
\begin{align}
f &=\dfrac{1}{2}|TU||RH| \\
  &=\dfrac{1}{2}(2t+2s)\cdot \dfrac{2(t+s)}{t^2+s^2} \\
  &=\dfrac{2(t+s)^2}{t^2+s^2} \\
  &=2\left(1+\dfrac{2ts}{t^2+s^2}\right) \label{eq:2}
\end{align}
である．以下この最大値を求める．
AM-GM不等式 から，
\begin{align}
