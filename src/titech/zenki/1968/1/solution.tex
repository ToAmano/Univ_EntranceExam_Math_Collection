...
\begin{align}
\frac{1}{c} + \frac{1}{abc} \le 1 \le \frac{1}{a} + \frac{1}{b} + \frac{1}{c} + \frac{1}{abc} \quad \cdots \text{③} \label{eq:2}
\end{align}
である．
簡単のため$T = \frac{1}{abc}$ とおく．
$c=1$ は\cref{eq:2}の左側の不等号が成立せず不適だから $c \ge 2$ が必要である．
\cref{eq:2}の右側の不等号に\cref{eq:1}の第二式を用いると
\begin{align}
1 \le \frac{1}{a} + \frac{1}{b} + \frac{1}{c} + \frac{1}{abc} < \frac{3}{c} + \frac{1}{c^3} \\
 \therefore 
...
\paragraph{$1^\circ \ c=2$の時}
\cref{eq:1}に$c=2$を代入すると
\begin{align}
\begin{cases}
\frac{1}{2ab} \le \frac{1}{2} \le \frac{1}{a} + \frac{1}{b} + \frac{1}{2ab} \\
3 \le b < a
...
\begin{axis}[
    ...
    % y=0 (x軸) との交点を追加
    extra x ticks={3.158}, 
    extra x tick labels={$\frac{3+\sqrt{11}}{2}$},
    extra x tick style={tick label style={below right, font=\footnotesize}},
    grid = major,
    grid style = {dashed, gray!30},
    domain = -0.7:4.3,
...
\end{tikzpicture}
\end{center}
\caption{$y=2x^2-6x-1$ のグラフ。$x > \frac{3+\sqrt{11}}{2} (\approx 3.16)$ では常に $y>0$ であり、$x \ge 4$ に解はない。}
\end{figure}
