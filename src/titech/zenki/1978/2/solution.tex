\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

{\bf [解]}
$A=(bc-1)(ca-1)(ab-1)$ とおく．以下 $a,b,c\in\mathbb{N}$ に注意する．
\begin{align*}
A=(abc)^2-(a^2bc+ab^2c+abc^2)+(bc+ca+ab)-1 = (abc)^2-(a+b+c)(abc)+(bc+ca+ab-1)
\end{align*}
だから，$A/abc\in\mathbb{Z}$ となる時（$bc+ca+ab-1$）$/abc\in\mathbb{Z}$ つまり $bc+ca+ab-1$ が $abc$ で割り切れる．（以上(1)）

したがって，$k\in\mathbb{N}$ に対し（$\because bc+ca+ab-1>0$）
\begin{align*}
\dfrac{bc+ca+ab-1}{abc}=k \iff \dfrac{1}{a}+\dfrac{1}{b}+\dfrac{1}{c}-\dfrac{1}{abc}=k \quad \cdots \text{①}
\end{align*}
とおける．又，$a\in\mathbb{N}$ 及び題意から
\begin{align*}
2\le a<b<c, \quad a,b,c\in\mathbb{N} \quad \cdots \text{②}
\end{align*}
である．②から
\begin{align*}
\dfrac{1}{a}+\dfrac{1}{b}+\dfrac{1}{c}-\dfrac{1}{abc}<\dfrac{1}{a}+\dfrac{1}{a}+\dfrac{1}{a}-\dfrac{1}{abc}=\dfrac{1}{a}\left(3-\dfrac{1}{bc}\right)
\end{align*}
となり，①及び $k\in\mathbb{N}$ から
\begin{align*}
1<\dfrac{1}{a}\left(3-\dfrac{1}{bc}\right) \quad\therefore\ a<3-\dfrac{1}{bc}<3 \quad (\because a,b,c>0)
\end{align*}
②とあわせて $a=2$ が必要．①に代入して
\begin{align*}
\dfrac{1}{b}+\dfrac{1}{c}-\dfrac{1}{2}\cdot\dfrac{1}{bc}=k-\dfrac{1}{2} \quad \cdots \text{②}'
\end{align*}
同様にして，
\begin{align*}
\dfrac{1}{2}\le k-\dfrac{1}{2}<\dfrac{1}{b}\left(2-\dfrac{1}{2c}\right) \quad\therefore\ b<2\left(2-\dfrac{1}{2c}\right)<4 \quad (\because b,c>0)
\end{align*}
だから②より，$b=3$ が必要．②'に代入して
\begin{align*}
\dfrac{1}{c}-\dfrac{1}{3}\cdot\dfrac{1}{c}=k-\dfrac{1}{2}-\dfrac{1}{3} \quad\therefore\ c=\dfrac{1}{\dfrac{6}{5}k-1} \quad \left(\because k\in\mathbb{N}\text{から}\dfrac{6}{5}k-1>0\right)
\end{align*}
$c\in\mathbb{N}$ から $c>b=3$ より
\begin{align*}
\dfrac{1}{\dfrac{6}{5}k-1}>3 \iff k<\dfrac{25}{18} \quad \left(\because\dfrac{6}{5}k-1>0\right)
\end{align*}
から，$k=1$ が必要で，この時 $c=5$ で条件をみたし十分．

題意をみたす $a,b,c$ の組は，①，②をみたす $k$ が存在する $(a,b,c)$ の組と同じだから以上より，
\begin{align*}
(a,b,c)=(2,3,5)
\end{align*}


\newpage

\end{document}
