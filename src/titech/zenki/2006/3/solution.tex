...
\begin{tikzpicture}[scale=2.6, >=stealth]
  ...
  \draw[thick, dashed, purple!80!black] (\sinHalf, -\yIntersect - 0.2) -- (\sinHalf, \yIntersect + 0.3)
    node[above, font=\small] {$x = \sin\frac{\xi}{2}$};

  % --- 6. 目盛りとラベル ---
  % 交点の x 座標
  \fill[purple!80!black] (\sinHalf, 0) circle (1.0pt) 
    node[below left, font=\small] {$\sin\frac{\xi}{2}$};
  ...
  \fill[red!80!black] (O2) circle (1.2pt) 
    node[below right, font=\small] {$2\sin\frac{\xi}{2}$};
...
\begin{tikzpicture}[scale=1.5, >=stealth]
  ...
  \draw[->, thick] (0, -0.3) -- (0, {1.5*\piVal + 0.8}) node[above] {$\alpha - \frac{\theta}{2}$};
  \node[below left, font=\small] at (0,0) {$O$};

  % --- 4. 領域の境界線の描画 ---
  % 上辺: Y = pi + theta/2
  \draw[thick, blue!80!black] (P_left) -- (P_top) 
    node[midway, above left, font=\scriptsize, rotate=17] {$\alpha - \frac{\theta}{2} = \pi + \frac{\theta}{2}$};

  % 下辺: Y = pi - theta/2
  \draw[thick, blue!80!black] (P_left) -- (P_bot) 
    node[midway, below left, font=\scriptsize, rotate=-17] {$\alpha - \frac{\theta}{2} = \pi - \frac{\theta}{2}$};
  ...
  \draw[ultra thick, red!80!black, dashed] (0, \piVal) -- ({\piVal + 0.3}, \piVal)
    node[right, font=\small] {$\alpha - \frac{\theta}{2} = \pi$（$S$ が最大）};
  ...
  % --- 6. 目盛りとラベル ---
  % 縦軸の目盛り
  \draw[dashed, gray] (P_top) -- (0, {1.5*\piVal}) node[left, font=\small] {$\frac{3\pi}{2}$};
  \draw[dashed, gray] (P_bot) -- (0, {0.5*\piVal}) node[left, font=\small] {$\frac{\pi}{2}$};
