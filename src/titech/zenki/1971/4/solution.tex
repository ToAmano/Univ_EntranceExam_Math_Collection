...
\subparagraph{1. $0\le t\le s$に2実解を持つ時}
...
\subparagraph{2. $0\le t\le s,\ s\le t\le c$ に1つずつ解を持つ時}
...
\begin{tikzpicture}[scale=3, >=stealth]
  ...
  \draw[dashed, gray] plot[domain=0:-0.8, samples=50] (\x, {0.25*\x*\x}) node[above, color=black, font=\small] {$v = \dfrac{1}{4}u^2$};
...
\begin{tikzpicture}[scale=3, >=stealth]
  ...
  \draw[dashed, gray] plot[domain={2*\s}:2.2, samples=50] (\x, {0.25*\x*\x}) node[above right, color=black, font=\small] {$v = \dfrac{1}{4}u^2$};
  ...
  \draw[dashed, gray] ({2*\s}, {0.5}) -- (2, {2*\s - 0.5}) node[right, color=black, font=\small] {$v = \dfrac{\sqrt{2}}{2}u - \dfrac{1}{2}$};
  ...
  % ラベル表記
  \node[below, font=\small] at ({\s}, 0) {$\dfrac{\sqrt{2}}{2}$};
  \node[below, font=\small] at ({2*\s}, 0) {$\sqrt{2}$};
 
  \node[left, font=\small] at (0, 0.5) {$\dfrac{1}{2}$};
...
\begin{align}
S(\theta) &= 
  \left( \text{\begin{tikzpicture}[baseline={(0,-0.1)}, scale=0.8]
    % 放物線領域
  ...
  &= \int_0^{2s} \dfrac{1}{4}u^2 \, du + \dfrac{1}{2}(c-s)(s^2+cs) - \dfrac{1}{2}c \cdot cs \\[2mm]
  &= \dfrac{2}{3}s^3 + \dfrac{1}{2}s(c^2-s^2) - \dfrac{1}{2}cs^2 \\[2mm]
  &= \dfrac{1}{6}s^3 + \dfrac{1}{2}sc^2 - \dfrac{1}{2}cs^2 \label{eq:5}
\end{align}
と計算できる．
...
S(\theta) &= 
  ...
  \end{tikzpicture}} \right) \\[2mm]
 &=\int_0^{\frac{\sqrt{2}}{2}}\dfrac{1}{4}u^2du - \dfrac{1}{2}\cdot\dfrac{\sqrt{2}}{2}\cdot\dfrac{1}{2} \\
 &=\dfrac{\sqrt{2}}{24} \label{eq:6}
\end{align}
...
\begin{align}
  S\left(\dfrac{\pi}{2}-\theta\right) = \dfrac{1}{6}s^3+\dfrac{1}{2}cs^2-\dfrac{1}{2}cs^2 = S(\theta)
\end{align}
であるから示される．
