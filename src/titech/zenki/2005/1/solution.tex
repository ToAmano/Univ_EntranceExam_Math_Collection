...
$n\ge3$の時，部分積分を用いる．まず
\begin{align}
a_{n-1}=\int_1^e(\log x)^{n-1}dx=\bigl[x(\log x)^{n-1}\bigr]_1^e-\int_1^ex\cdot(n-1)(\log x)^{n-2}\cdot\dfrac{1}{x}\,dx=e-(n-1)a_{n-2}
\end{align}
（$n-1\ge2$の場合の同じ部分積分公式）．同様に
\begin{align}
a_n=\bigl[x(\log x)^n\bigr]_1^e-\int_1^ex\cdot n(\log x)^{n-1}\cdot\dfrac{1}{x}\,dx=e-n\,a_{n-1}
\end{align}
...
$[1,e]$では$0\le\log x\le1$（②）．したがって$0\le(\log x)^{n+1}\le(\log x)^n$（$0\le t\le1$なら$t^{n+1}\le t^n$）．同区間で積分して
\begin{align}
0\le a_{n+1}\le a_n
\end{align}
...
\begin{align}
2n\,a_{2n}<(2n-1)a_{2n-2} \quad\therefore\ a_{2n}<\dfrac{2n-1}{2n}a_{2n-2}
\end{align}
これをくり返し用いて（$n,n-1,\dots,2$について）
\begin{align}
a_{2n}<\dfrac{2n-1}{2n}\cdot\dfrac{2n-3}{2(n-1)}\cdots\dfrac{3}{4}\,a_2
\end{align}
