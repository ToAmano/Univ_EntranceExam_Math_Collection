...
\begin{multicols}{2}
  ...
  \begin{align}
    \quad a_{m+1,n}
     ...
     & = \dfrac{1}{n} \left[ \theta^{m+1} \sin n\theta \right]_0^\pi - \dfrac{m+1}{n} \int_0^\pi \theta^m \sin n\theta \, d\theta \\
     & = -\dfrac{m+1}{n} b_{m,n}
  \end{align}                                                                                                                                                \\
  および
  \begin{align}
    \quad b_{m+1,n}
     ...
     & = -\dfrac{1}{n} \left[ \theta^{m+1} \cos n\theta \right]_0^\pi + \dfrac{m+1}{n} \int_0^\pi \theta^m \cos n\theta \, d\theta \\
     & = (-1)^{n+1} \dfrac{\pi^{m+1}}{n} + \dfrac{m+1}{n} a_{m,n}
  \end{align}
  より題意は示された．$\cdots$(答)
  ...
  \begin{figure}[H]
    \begin{tikzpicture}
      \begin{axis}[
          ...
          % xtick={-pi/2,-1,0,1},
          % ytick={-1,0,1},
          % yticklabels={$-1$, $0$, $1$, $-\dfrac{\pi}{2}$}, % Custom label for -pi/2
        ...
        \node at (axis cs:-pi/2,0) [anchor=north] {$-\dfrac{\pi}{2}$};
        \addplot[
          only marks,       % 点のみをプロット
          mark=*,           % 丸のマーク
        ...
        % %\node at (axis cs:\YminusXat0, \YminusYat0) [anchor=north west] {$-\dfrac{\pi}{2}$}; % This is actually Y(0) = -pi. The image seems to imply -pi/2.
  ...
  \begin{align}
    V_{s} & = \dfrac{4\pi}{3} \label{2000-2:eq:8}   \\
    V_{+} & = \dfrac{2\pi^4}{3} \label{2000-2:eq:9}
  \end{align}
  である．$V_{-}$は，$Y_{+}$と$Y_{-}$を分けて考えると
  \begin{align}
    V_{-}
     & = \pi \int_{-\dfrac{\pi}{2}}^{1} Y_{+}^2 dx - \pi \int_{-\dfrac{\pi}{2}}^{-1} Y_{-}^2 dx                                      \nonumber   \\
     & = \pi \int_{\dfrac{\pi}{2}}^{0} Y^2 \dfrac{dx}{d\theta} d\theta - \pi \int_{\dfrac{\pi}{2}}^{\pi} Y^2 \dfrac{dx}{d\theta} d\theta \nonumber \\
     & = -\pi \int_{0}^{\pi} Y^2 \dfrac{dx}{d\theta} d\theta \label{2000-2:eq:10}
  \end{align}
  ...
  \begin{align}
    V_{-}
    = & -\pi \int_{0}^{\pi} \left[
    \left(\theta^3-\theta\right)\dfrac{1}{4}\left(\cos 3\theta + 3\cos\theta \right)\right. \\
      & + \left. \dfrac{2}{4}\theta^2\left(-\sin 3\theta + 3\sin\theta \right)
    -2 \theta^2\sin\theta + \theta \cos\theta \right]  d\theta                             \\
    = & -\pi \int_{0}^{\pi} \left[
      \dfrac{1}{4}\left(\theta^3-\theta\right)\cos 3\theta
    -\dfrac{1}{2}\theta^2\sin 3\theta \right.                                               \\
      & \left. -\dfrac{1}{2}\theta^2\sin\theta
      +\dfrac{1}{4} \left(3\theta^3+\theta\right) \cos\theta \right]  d\theta
  \end{align}
  となる．各項の積分を，(1)の結果を利用して簡単にして解く．まず，積分を$a_{m,n}$および$b_{m,n}$で表すと
  \begin{align}
    V_{-}
     & = -\pi \left[ \dfrac{1}{4}a_{3,3}-\dfrac{1}{4}a_{1,3} - \dfrac{1}{2}b_{2,3} -\dfrac{1}{2}b_{2,1}+\dfrac{3}{4}a_{3,1}+\dfrac{1}{4}a_{1,1} \right] \label{2000-2:eq:5}
  \end{align}
  である．(1)の漸化式および$n\le 1$に対して$a_{0,n}=0$, $b_{0,n}=2/n$より，それぞれの項は$m$の小さい順に整理すると
  \begin{align}
    a_{1,1} & = - b_{0,1} = -2                                                         \\
    a_{1,3} & = -\dfrac{1}{3} b_{0,3} = -\dfrac{2}{9}                                    \\
    b_{2,1} & = \pi^2 + 2 a_{1,1} = \pi^2 - 4                                          \\
    b_{2,3} & = \dfrac{\pi^2}{3} + \dfrac{2}{3} a_{1,3} = \dfrac{\pi^2}{3} - \dfrac{4}{27} \\
    a_{3,1} & = - 3 b_{2,1}                                                            \\
    a_{3,3} & = -b_{2,3}
  \end{align}
  ...
  \begin{align}
    V_{-}
     & = -\pi \left[ -\dfrac{1}{4}b_{2,3}+\dfrac{1}{18} - \dfrac{1}{2}b_{2,3} -\dfrac{1}{2}b_{2,1}-\dfrac{9}{4}b_{2,1}-\dfrac{1}{2} \right] \nonumber           \\
     & = -\pi \left[ -\dfrac{3}{4}b_{2,3} -\dfrac{11}{4}b_{2,1}-\dfrac{4}{9} \right]                                                     \nonumber           \\
     & = -\pi \left[ -\dfrac{3}{4}\left( \dfrac{\pi^2}{3} - \dfrac{4}{27}\right) -\dfrac{11}{4}\left(\pi^2-4\right)-\dfrac{4}{9} \right]   \nonumber           \\
     & = -\pi \left[ -3\pi^2 + \dfrac{32}{3}  \right]                                                                                  \label{2000-2:eq:6}
  \end{align}
  と求まる．\cref{2000-2:eq:8,2000-2:eq:9,2000-2:eq:6}を\cref{2000-2:eq:7}に代入して求める体積は
  \begin{align}
    V
     & = \dfrac{2}{3}\pi^4 -\pi \left[ -3\pi^2 + \dfrac{32}{3}  \right] - \dfrac{4}{3}\pi \\
     & = \dfrac{2}{3}\pi^4 + 3\pi^2 -12 \pi
  \end{align}
  である．$\cdots$(答)
  ...
  \begin{align}
    ...
    = & \left[ \dfrac{1}{3}(\theta^3-\theta) \sin 3\theta + \dfrac{1}{9}(3\theta^2-1) \cos 3\theta \right. \\
      & \left. - \dfrac{1}{27} \cdot 6\theta \sin 3\theta- \dfrac{6}{81} \cos 3\theta \right]_0^\pi        \\
    = & \left[ -\dfrac{1}{9}(3\pi^2-1) + \dfrac{1}{9}+\dfrac{12}{81} \right]                                \\
    = & -\dfrac{1}{3}\pi^2 + \dfrac{10}{27}                                                                \\
    ...
    = & \left[ -\dfrac{1}{3}t^2 \cos 3t + \dfrac{2}{9}t \sin 3t + \dfrac{2}{27} \cos 3t \right]_0^\pi       \\
    = & \dfrac{1}{3}\pi^2 - \dfrac{4}{27}                                                                  \\
  ...
  \begin{align}
    \dfrac{V_{-}}{-\pi}
    = & \dfrac{1}{4}\left(-\dfrac{1}{3}\pi^2 + \dfrac{10}{27}\right) + \dfrac{1}{4}(-9\pi^2+34)          \\
      & - \dfrac{1}{2}\left(\dfrac{1}{3}\pi^2 - \dfrac{4}{27}\right) - \dfrac{1}{2}\left(\pi^2 -4\right) \\
    = & -3\pi^2 + \dfrac{32}{3}
  \end{align}
  ...
  \begin{align}
    \int y^2\dfrac{dx}{d\theta} d\theta
  \end{align}
  になっている．
