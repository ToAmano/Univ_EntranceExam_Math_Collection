...
\begin{multicols}{2}
  ...
  \begin{align}
    \int_{0}^{1} f(t) dt
     & = \int_{0}^{1} \left(1-\dfrac{x+1}{t+1}\right) dt \\
     & = \left[t - (x+1)\log(t+1)\right]_{0}^{1}        \\
     & = 1 - (x+1)\log 2
  \end{align}
  だから，$\displaystyle\int_{0}^{1} f(t) dt$は$x$の単調減少関数であり，\cref{1992-1:eq:1}より$F(x)$は$0\le x \le 1$で最小値を取る．
  以下この場合を調べる．
  ...
  \begin{align}
    F(x)
     ...
     & = G(0,x) + G(1,x) - 2G(x,x) \label{1992-1:eq:3}
  \end{align}
  と表せる．
  \begin{align}
    f_{x}(t)
  ...
  \begin{align}
    G(t,x) = t - (x+1)\log(t+1)
  \end{align}
  を得る．これを\cref{1992-1:eq:3}に代入して
  \begin{align}
    F(x)
     & = \left[1-(x+1)\log 2\right] -2 \left[x-(x+1)\log(x+1)\right] \nonumber \\
     & = \left[1-(x+1)\log 2\right] -2 \left[x-(x+1)\log(x+1)\right] \nonumber \\
     & = (x+1) \log\dfrac{\left(x+1\right)^2}{2} -2x + 1 \label{1992-1:eq:6}
  \end{align}
  となる．一階微分を計算すると
  \begin{align}
    F'(x)
     & = \log\dfrac{\left(x+1\right)^2}{2} + 2 -2 \\
     & = \log\dfrac{\left(x+1\right)^2}{2}
  \end{align}
  を得る．従って$F(x)$ の増減表は\cref{1992-1:table:1}となる．
  \begin{table}[H]
    \centering
    \caption{$F(x)$の増減表}
  ...
  したがって、$F(x)$ は $x=\sqrt{2}-1$ で最小値をとる．この時の最小値は\cref{1992-1:eq:6}に代入して
  \begin{align}
    \min F(x)
     & = F(\sqrt{2}-1)      \\
     & = -2(\sqrt{2}-1) + 1 \\
  ...
  \begin{align}
    ...
    J(x) & = 2F(\beta) - F(a) - F(b)  - \beta^2 x + \dfrac{1}{2}a^2 x + \dfrac{1}{2}b^2 x
  \end{align}
  ...
  \begin{align}
    I'(x)
     ...
     & = -2\alpha + a + b                                                                  \\
    J'(x)
     & = 2f(\beta)\beta'   - (2\beta\beta' x + 2\beta^2) + \dfrac{1}{2}a^2 + \dfrac{1}{2}b^2 \\
     & = -2\beta^2 + \dfrac{1}{2}a^2 + \dfrac{1}{2}b^2
  \end{align}
  を得る．従って，$I$および$J$が最小になるような$x$は$I'(x)=0$, $J'(x)=0$から
  \begin{align}
    \alpha & = \dfrac{a+b}{2}            \\
    \beta  & = \dfrac{\sqrt{a^2+b^2}}{2}
  \end{align}
  をみたさなければならない．
  ...
  \begin{align}
    f_{x}(t)
     & = \dfrac{t}{t+1} - \dfrac{x}{t+1} \\
     & = p(t) - q(t) x
  \end{align}
  ただし，
  \begin{align}
    q(t) & = \dfrac{1}{t+1}                             \\
    p(t) & = \dfrac{t}{t+1} = tq(t) \label{1992-1:eq:4}
  \end{align}
  とおいた．このように$f$を$t$に依存する部分と$x$に依存する部分に分離して書けば，
  \cref{1992-1:eq:3}の$F(x)$は$p$および$q$の原始関数$P$，$Q$を用いて
  \begin{align}
    F(x) =
     & \left[P(0)-Q(0)x \right] + \left[P(1)-Q(1)x \right] \\
     & -2 \left[P(x)-Q(x)x \right]\label{1992-1:eq:7}
  \end{align}
  ...
  \begin{align}
    F'(x)
     ...
     & = -Q(0) - Q(1) + 2Q(x)
  \end{align}
  を得る．ただし，三行目で$p$と$q$の関係$p(x)=xq(x)$ (\cref{1992-1:eq:4})を利用した．
  ここから，$F$が極小となるには$F'(x)=0$すなわち
  \begin{align}
    Q(x) = \dfrac{Q(0)+Q(1)}{2} \label{1992-1:eq:5}
  \end{align}
  となることが必要であることがわかる．
  ...
  また，\cref{1992-1:eq:5}までわかれば$Q(x) = \log (x+1)$から容易に$x=\sqrt{2}-1$を導くことができる．
  この時の最小値$F(x)$は，\cref{1992-1:eq:7}に\cref{1992-1:eq:5}を代入すれば$Q$に関する項が全て消えて
  \begin{align}
