...
\begin{multicols}{2}
  ...
  \end{numcases}
  の2つの場合分けで考える．\cref{1991-2:eq:1} の場合は，$\theta$ が最大となるとき $l, m$ は A または B を通る．したがって，$\min \angle \mathrm{OP}(\alpha)\mathrm{B}$, $\min \angle \mathrm{OP}(\alpha)\mathrm{A}$ のうち，小さい方が最大の $\theta$ を与える．一方で\cref{1991-2:eq:2} の場合は，$\theta$ が最大となるとき $l, m$ は B または D を通る．したがって，$\min \angle \mathrm{OP}(\alpha)\mathrm{B}$, $\min \angle \mathrm{OP}(\alpha)\mathrm{C}$ のうち，小さい方が最大の $\theta$ を与える．これらの場合をそれぞれ図に示す．

  これらの条件を数式に起こすと
  \begin{numcases}{\max\theta = \\}
    \min \left(\min \angle \mathrm{OPB}, \min \angle \mathrm{OPA}\right) & $\displaystyle 0 \le \alpha \le \dfrac{\pi}{6}$ \label{1991-2:eq:3}\\
    \min \left(\min \angle \mathrm{OPB}, \min \angle \mathrm{OPD}\right) & $\displaystyle \dfrac{\pi}{6} \le \alpha \le \dfrac{\pi}{4}$ \label{1991-2:eq:4}
  \end{numcases}
  ...
  \begin{align}
    \cos \angle \mathrm{OPA}
     & = \dfrac{\mathrm{OP}^2 + \mathrm{PA}^2 - \mathrm{OA}^2}{2 \mathrm{OP} \cdot \mathrm{PA}} \\
     & = \dfrac{2 + \mathrm{PA}^2 }{4\mathrm{PA}}                                               \\
     & = \dfrac{1}{4} \left(\mathrm{PA} + \dfrac{2}{\mathrm{PA}}\right)
  \end{align}
  を得る．ここで，PがK上の点であるから$\mathrm{OP} = 2$であること，および$\mathrm{OA}=\sqrt{2}$を利用した．ここで新しく関数$f(x)$を
  \begin{align}
    f(x) = \dfrac{1}{4}\left(x + \dfrac{2}{x}\right) \quad (x > 0)
  \end{align}
  ...
  \begin{align}
    \begin{dcases}
      \max \left(\cos \angle \mathrm{APO}\right) = \dfrac{\sqrt{3}}{2} & \mathrm{PA} = \sqrt{3}-1         \\
      \max \left(\cos \angle \mathrm{BPO}\right) =  \beta             & \mathrm{PB} = \sqrt{8-2\sqrt{3}}
    \end{dcases}
  \end{align}
  となる．ここで$\beta=f(\sqrt{8-2\sqrt{3}})$とおいた．$f(x)$のグラフの概形より$\sqrt{3}/2>\beta$だから，このときの$\max \theta$は\cref{1991-2:eq:3}より$\cos \angle \mathrm{APO} = \dfrac{\sqrt{3}}{2}$すなわち$\max \theta=\pi/6$である．
  ...
  \begin{align}
    \begin{dcases}
      \max \left(\cos \angle \mathrm{DPO}\right) = \dfrac{\sqrt{3}}{2} & \mathrm{PD} = \sqrt{3}+1 \\
      \max \left(\cos \angle \mathrm{BPO}\right) = \dfrac{\sqrt{6}}{3} & \mathrm{PB} = \sqrt{6}
    \end{dcases}
  \end{align}
  となる．$\sqrt{3}/2>\sqrt{6}{3}$より，したがってこのときの$\max \theta$は\cref{1991-2:eq:4}より$cos \angle \mathrm{DPO} = \dfrac{\sqrt{3}}{2}$すなわち$\max \theta=\pi/6$である．

  \vspace{10pt}
  以上(i),(ii)から，どちらの場合も$\max \theta = \pi/6$である．したがって，求める最大値は$\displaystyle \max\theta = \dfrac{\pi}{6}$である．$\cdots$(答)
  ...
  今回の回答では，\cref{1991-2:eq:3,1991-2:eq:4}から$\theta$の最大値を関数$f(x)$を導入してかなり丁寧に求めたが，$\angle \mathrm{APO}$,$\angle \mathrm{BPO}$,$\angle \mathrm{DPO}$が単調に変化することは図から自明としてしまっても良いかもしれない．この場合は後半の解答は細かい数値の比較は除いて以下のように簡略化できる．
