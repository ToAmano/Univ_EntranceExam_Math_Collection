\documentclass[../../main.tex]{subfiles}

\begin{document}
\rhead{東工大後期$2004$年$2$番}
\setcounter{equation}{0}
\setcounter{figure}{0}
\setcounter{table}{0}
\begin{oframed}
  \input{problem.tex}
\end{oframed}


\setlength{\columnseprule}{0.4pt}
\begin{multicols}{2}
  {\bf[解]}
  (1)
  $n$ は偶数とし, $f_n(x) = x^n$, $g_n(x) = n^x$ とおく.

  (1) $x < 0$ の時. $t=-x$とおくと，$t>0$であり，
  $f_n(x)$ と $g_n(x)$ が一致するとすると
  \begin{align}
    f_n(x) & = g_n(x)                     \\
    \iff
    (-t)^n & = \left(\dfrac{1}{n}\right)^t \\
    \iff
    t^n    & = \left(\dfrac{1}{n}\right)^t
  \end{align}
  ただし，ここで$n$が偶数であることを用いた．
  両辺正だから, 自然対数をとって
  \begin{align}
    n \log t         & = -t\log n  \nonumber                                             \\
    \iff
    \dfrac{\log t}{t} & = -\dfrac{\log n}{n} \quad (\text{ただし } x > 0) \label{2004-2:eq:1}
  \end{align}
  である．
  ここで $h(t) = \dfrac{\log t}{t}$ とおくとこの条件は
  \begin{align}
    h(t) = - h(n) \label{2004-2:eq:5}
  \end{align}
  と表せる．

  $h(t)$の一階微分は
  \begin{align}
    h'(t) = \dfrac{1 - \log t}{t^2}
  \end{align}
  であり，また$h(t)$の極限値は
  \begin{align}
    h(t) & \to -\infty \quad (t \to +0) \\
    h(t) & \to 0 \quad (t \to +\infty)
  \end{align}
  で与えられるから，$h(t)$の増減表は\cref{2004-2:table:1}となる．

  \begin{table}[H]
    \centering
    \caption{$h(x)$の増減表}
    \label{2004-2:table:1}
    \begin{tabular}{|c||c|c|c|c|c|}
      \hline
      $t$  & $(0)$       & $\cdots$   & $e$           & $\cdots$   & $(\infty)$ \\
      \hline
      $h'$ &             & $+$        & $0$           & $-$        &            \\
      \hline
      $h$  & $(-\infty)$ & $\nearrow$ & $\dfrac{1}{e}$ & $\searrow$ & $(0)$      \\
      \hline
    \end{tabular}
  \end{table}

  よって$h(t)$のグラフは\cref{2004-2:fig:1}となる.

  \begin{figure}[H]
    %\centering
    \begin{tikzpicture}
      \begin{axis}[
          axis lines=middle,
          xmin=0, xmax=6,
          ymin=-2, ymax=1,
          xlabel=$t$,
          ylabel=$y$,
          xtick={1,e},
          xticklabels={$1$,$e$},
          ytick={0.367879, -0.5}, % 1/e の近似値と -log n / n の例
          yticklabels={$\dfrac{1}{e}$, $-\dfrac{\log n}{n}$},
          grid=none,
          %no markers,
          clip=false, % ラベルが軸からはみ出しても表示されるように
        ]

        % h(x) = (ln x) / x のプロット
        \addplot[blue, thick, domain=0.5:5, samples=100,smooth] {ln(x)/x};

        % h(t)ラベル
        \node[above right] at (axis cs:e, {ln(e)/e}) {$h(t)$};

        % 極大点とy軸のラベルを結ぶ破線
        \draw[dashed] (axis cs:e, {ln(e)/e}) -- (axis cs:0, {ln(e)/e});
        \draw[dashed] (axis cs:e, {ln(e)/e}) -- (axis cs:e, 0);

        % -log n / n の水平線（例として）
        \draw[red, dashed] (axis cs:0.01, -0.5) -- (axis cs:6, -0.5);
      \end{axis}
    \end{tikzpicture}
    \caption{$h(t)$の概形.$t=e$で最大値をとる．}
    \label{2004-2:fig:1}
  \end{figure}

  ここで $n$ が$2$以上の偶数であるから図より$h(n)>0$であり，\cref{2004-2:eq:5}より従って
  \begin{align}
    h(t) < 0
  \end{align}
  である．

  グラフの概形から\cref{2004-2:eq:1}が成立する$t$が$0<t<1$にただひとつ存在する．
  したがって $C_1, C_2$ は$x= -t<0$にただ 1 つ交点を持つ.$\cdots$(答)

  \vspace{10pt}
  (2)
  $x>0$ の時. $f_n(x), g_n(x)$ 共に正だから, (1) と同様に自然対数をとって考えると
  \begin{align}
    f_n(x)           & = g_n(x)           \\
    \iff
    n \log x         & = x \log n         \\
    \iff
    \dfrac{\log x}{x} & = \dfrac{\log n}{n}
  \end{align}
  である．
  \cref{2004-2:fig:1}および$n$が$2$以上の偶数であることから
  \begin{align}
    0 < \dfrac{\log n}{n} < \dfrac{1}{e}
  \end{align}
  である．
  従って\cref{2004-2:fig:1}からこれみたす $x$ は 2 つある.

  最後に$x=0$の時は $f_n(0) = 0$, $g_n(0) = 1$ だから, $C_1, C_2$ は交わらない.

  以上で$x$について全ての場合が考えられた．$x<0$で一つ，$x=0$で0個，$x>0$で2つの解が存在する．
  $C_1, C_2$ の交点の数は $f_n(x) = g_n(x)$ の実解の数に等しいことから
  あわせて $3$ つの交点がある.$\cdots$(答)

  \vspace{10pt}
  (3) $P_n(-x_n, y_n)$ とおくと，(1)の結果から $0 < x_n < 1$ である．
  $P_n$ の条件から,
  \begin{align}
    y_n = (-x_n)^n = n^{-x_n} \label{2004-2:eq:2}
  \end{align}
  である.

  $n\to\infty$のとき，\cref{2004-2:fig:1}から $\dfrac{\log n}{n} \to 0$ だから,
  \cref{2004-2:eq:1}より
  \begin{align}
    \lim_{n\to\infty} \dfrac{\log x_n}{x_n} = 0
  \end{align}
  である. これを満たすには\cref{2004-2:fig:1}および$0<x_n<1$から
  \begin{align}
    \lim_{n\to\infty} x_n = 1 \label{2004-2:eq:3}
  \end{align}
  である．これを\cref{2004-2:eq:2}に代入して
  \begin{align}
    \lim_{n\to\infty} y_n = \lim_{n\to\infty} \left(\dfrac{1}{n}\right)^{x_n} = 0 \label{2004-2:eq:4}
  \end{align}
  である．\cref{2004-2:eq:3,2004-2:eq:4}より，求める$P_n$の極限値は
  \begin{align}
    \lim_{n\to\infty}P_n = (-1,0)
  \end{align}
  である．$\cdots$(答)

  \vspace{10pt}

  {\bf[解説]}
  典型的な関数や極限の問題である．
  $C_1$と$C_2$の交点の$x$座標は$y$を消去した式
  \begin{align}
    x^{n} = n^{x}
  \end{align}
  の解であることを利用してこの式の挙動を調べていくことになる．

  答案では両辺の自然対数をとって変数を$x$と$n$で分離する方針をとった．
  自然対数をとるときに$x<0$か$x>0$かが大事であり，それに応じて方程式は
  \begin{align}
    \dfrac{\log n}{n} =
    \begin{dcases}
      \dfrac{\log x}{x}      & (x>0) \\
      -\dfrac{\log |x|}{|x|} & (x<0)
    \end{dcases}
  \end{align}
  という形になる．(1)で$x<0$の場合を，(2)で$x>0$の場合を考えている．

  いずれにしても$h(x)=\log x/x$の挙動がわかればこの方程式の解の挙動がわかるため，$h(x)$のグラフを書いて考えている．
  すると$x\ge 1$の時には$x,y$が一対一対応するのに対して，$x>1, x\neq e$の時には$y$を与える$x$が二つ存在するため，結局解の数が$3$つになる．


  \begin{figure}[H]
    %\centering
    \begin{tikzpicture}
      \pgfmathsetmacro{\n}{7}
      \pgfmathsetmacro{\hn}{ln(\n)/\n}

      \begin{axis}[
          axis lines=middle,
          xmin=0, xmax=10,
          ymin=-0.2, ymax=0.8,
          xlabel=$t$,
          ylabel=$y$,
          xtick={1,e,\n},
          xticklabels={$1$,$e$,$n$},
          ytick={0.367879, 0.3}, % 1/e の近似値と -log n / n の例
          yticklabels={$1/e$, $\dfrac{\log n}{n}$},
          grid=none,
          %no markers,
          clip=false, % ラベルが軸からはみ出しても表示されるように
        ]

        % h(x) = (ln x) / x のプロット
        \addplot[blue, thick, domain=0.8:10, samples=100,smooth] {ln(x)/x};

        % h(t)ラベル
        \node[above right] at (axis cs:e, {ln(e)/e}) {$h(t)$};

        % 極大点とy軸のラベルを結ぶ破線
        \draw[dashed] (axis cs:e, {ln(e)/e}) -- (axis cs:0, {ln(e)/e});
        \draw[dashed] (axis cs:e, {ln(e)/e}) -- (axis cs:e, 0);

        % log n / n の水平線（例として）
        \draw[red, dashed] (axis cs:0.01, \hn) -- (axis cs:10, \hn);
        \draw[dashed] (axis cs:\n, 0) --  (axis cs:\n, \hn);
      \end{axis}
    \end{tikzpicture}
    \caption{$h(t)$の概形と，$h(t)=h(n)$の解}
    \label{2004-2:fig:2}
  \end{figure}

  \newpage
\end{multicols}
\end{document}