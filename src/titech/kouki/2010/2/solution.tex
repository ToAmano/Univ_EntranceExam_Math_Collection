...
\begin{multicols}{2}
  ...
  一階，二階微分は
  \begin{align}
    f'(x)  & = 2 \dfrac{\log x}{x}    \label{2010-2:eq:1} \\
    f''(x) & = 2 \dfrac{1-\log x}{x^2}
  \end{align}
  ...
  \begin{align}
    y
     & = l(x)                                               \\
     & = f'(x) (x - \alpha) + f(\alpha)                     \\
     & = 2\dfrac{\log \alpha}{\alpha} (x-\alpha) + f(\alpha)
  \end{align}
  であるから，$L(\alpha)$ と $C$ の共有点の個数は
  \begin{align}
     & l(x) = f(x)                                                                                   \\
    \therefore
     & (\log x)^2 - (\log \alpha)^2 - 2\dfrac{\log \alpha}{\alpha} (x-\alpha) = 0 \label{2010-2:eq:5}
  \end{align}
  の $x>0$ の解の個数にひとしい.
  \cref{2010-2:eq:5}の左辺を $g(x)$ とおき，$g(x)$の挙動から解の個数を求める．
  \begin{align}
    g(x) = (\log x)^2 - (\log \alpha)^2 - 2\dfrac{\log \alpha}{\alpha} (x-\alpha)
  \end{align}
  $g(x)$の一階微分は
  \begin{align}
    g'(x) = 2 \left( \dfrac{\log x}{x} - \dfrac{\log \alpha}{\alpha} \right)
  \end{align}
  であるから，新しく
  \begin{align}
    h(x) = \dfrac{\log x}{x}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      \begin{axis}[
          ...
          yticklabels={$\dfrac{1}{e}$, $\dfrac{\log \alpha}{\alpha}$},
          grid=none,
          %no markers,
          clip=false, % ラベルが軸からはみ出しても表示されるように
        ]
  ...
  % ここで $g'(x) = 2 \left( \dfrac{\log x}{x} - \dfrac{\log a}{a} \right)$ であること及び, $f''(x)$ の考え方から $y = \dfrac{\log x}{x}$ のグラフが下図であることから, つぎの $g(x)=0$ の解の数は
  % \begin{align}
  %   \begin{cases}
  ...
  すると$R$の$x$座標は
  \begin{align}
     & 2\dfrac{\log \alpha}{\alpha} (x-\alpha) + \left(\log \alpha\right)^2 = 0 \\
    \therefore
     & x = \alpha - \dfrac{\alpha}{2}\log\alpha
  \end{align}
  となる．
  ...
  \begin{align}
    |QR|
     & = \alpha - \dfrac{1}{2}\alpha\log\alpha - \alpha \\
     & = - \dfrac{1}{2}\alpha\log\alpha
  \end{align}
  および
  \begin{align}
  ...
  \begin{align}
    B(\alpha)
     & = \dfrac{1}{2}|QR||PQ|  \nonumber                                   \\
     & = \dfrac{-1}{4}\alpha\left(\log \alpha\right)^3 \label{2010-2:eq:3}
  \end{align}
  である．
  ...
  \begin{align}
    S(\alpha)
     & = 2 -\alpha (\log \alpha)^2 + 2\alpha \log \alpha - 2\alpha + \dfrac{1}{4}\alpha (\log \alpha)^3
  \end{align}
  が求める面積である．  $\cdots$(答)
  ...
  (2)では$y=\dfrac{\log x}{x}$の形の関数が出てくるが，これは2004年度の2番でも題材になっている．
