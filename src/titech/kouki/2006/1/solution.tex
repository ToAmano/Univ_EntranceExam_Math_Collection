...
\begin{multicols}{2}
  ...
  \begin{align}
     & 0 \le X \le \dfrac{1}{\sqrt{a}} \\
     & Y = -\sqrt{\dfrac{1 - aX^2}{b}}
  \end{align}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \node[above] at (axis cs:{sqrt(1/\aval)},0) {$\dfrac{1}{\sqrt{a}}$};
  ...
  \begin{align}
    g(X)
     & = tX + |Y|                       \\
     & = tX + \sqrt{\dfrac{1 - aX^2}{b}}
  \end{align}
  より，一階微分は
  \begin{align}
    g'(x)
     & = t + \dfrac{1}{\sqrt{b}} \dfrac{-2ax}{2\sqrt{1-ax^2}}                              \\
     & = \dfrac{t\sqrt{b(1-ax^2)}-ax}{\sqrt{b(1-ax^2)}}                                   \\
     & = \dfrac{t^2b(1-ax^2)-a^2x^2}{\sqrt{b(1-ax^2)}\left(t\sqrt{b(1-ax^2)}+ax\right)}   \\
     & = \dfrac{bt^2 - (abt^2+a^2)x^2}{\sqrt{b(1-ax^2)}\left(t\sqrt{b(1-ax^2)}+ax\right)}
    % & = \dfrac{(bt)^2 - (bt)^2 a^2 X^2}{b\sqrt{1-ax^2}\{\sqrt{b}t\sqrt{1-ax^2} - a x \sqrt{t}\}}
  \end{align}
  と書ける．
  分母は常に正であり，$g'=0$となるのは
  \begin{align}
    \alpha = \sqrt{\dfrac{bt^2}{abt^2 + a^2}}
  \end{align}
  の時である．
  ...
  であり，$X=\alpha$ に代入して
  \begin{align}
         & X  = \sqrt{\dfrac{bt^2}{abt^2 + a^2}}                         \\
         & X  = \sqrt{\dfrac{b(Y/X)^2}{ab(Y/X)^2 + a^2 }}                \\
    \iff & X^2 \{ ab Y^2  + a^2 X^2 \} = b Y^2 \quad (\because X \ge 0) \\
    \iff & Y^2 (b-abX^2) = a^2 X^4                                      \\
    \iff & Y = \dfrac{aX^2}{\sqrt{b(1-aX^2)}}
  \end{align}
  ただし，$X=1/\sqrt{a}$の時はこの式は成り立たないから，$0 < X < \dfrac{1}{\sqrt{a}}$である．
  この式は$t=0$でも成立するから，求めるべき曲線$C'$の表示は
  \begin{align}
    f(X) = \dfrac{aX^2}{\sqrt{b(1-aX^2)}} \quad \left(0 \le X < \dfrac{1}{\sqrt{a}}\right)
  \end{align}
  である．$\cdots$(答)
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        % Label for 1/sqrt(a) on the x-axis
        \node[above right] at (axis cs:{sqrt(1/\aval)},0) {$\dfrac{1}{\sqrt{a}}$};
        % Label for the origin
        \node[below left] at (axis cs:0,0) {O};
  ...
  \begin{align}
               & ax^2 + b\dfrac{(ax^2)^2}{b(1-ax^2)} = 1 \\
    \therefore & ax^2(1-ax^2) + a^2x^4 = 1-ax^2         \\
    \therefore & 2ax^2 = 1                              \\
    \therefore & x = \sqrt{\dfrac{1}{2a}}
  \end{align}
  であり，この時，
  \begin{align}
    y & = f(x)                         \\
      & =\dfrac{a/2a}{\sqrt{b(1-a/2a)}} \\
      & = \sqrt{\dfrac{1}{2b}}
  \end{align}
  である．以上から
  \begin{align}
     & \alpha = \sqrt{\dfrac{1}{2a}}
     & \beta  = \sqrt{\dfrac{1}{2b}}
  \end{align}
  である．$\cdots$(答)
  ...
  立体の体積は幅$dx$, 高さ$\dfrac{1}{\sqrt{2b}}-f(x)$, 長さ$2\pi x$の直方体で近似できるので,
  求める立体の体積$V$として，
  \begin{align}
    V = 2\pi \int_{0}^{\sqrt{\dfrac{1}{2a}}} \left(\dfrac{1}{\sqrt{2b}} - f(x)\right)x dx \label{2006-1:eq:1}
  \end{align}
  である．一項目は
  \begin{align}
    \int_{0}^{\sqrt{\dfrac{1}{2a}}} \dfrac{1}{\sqrt{2b}} x dx
     & = \dfrac{1}{2} \dfrac{1}{\sqrt{2b}} x^2 \Big|_{0}^{\sqrt{\dfrac{1}{2a}}} \nonumber \\
     & = \dfrac{1}{4\sqrt{2}} \dfrac{1}{a\sqrt{b}} \label{2006-1:eq:2}
  \end{align}
  であり，二項目は$s=1-ax^2$と置換すると
  \begin{align}
    \int_{0}^{\sqrt{\dfrac{1}{2a}}} f(x)x dx
     & = \int_{1}^{1/2} \dfrac{1-s}{\sqrt{bs}} \dfrac{1}{-2a} ds                        \nonumber       \\
     & = \dfrac{1}{2a\sqrt{b}} \int_{1/2}^{1} \left(\dfrac{1}{\sqrt{s}} - \sqrt{s}\right) ds  \nonumber \\
     & = \dfrac{1}{2a\sqrt{b}} \left[2\sqrt{s} - \dfrac{2}{3} s^{3/2}\right]_{1/2}^{1}     \nonumber    \\
     & = \dfrac{1}{2a\sqrt{b}} \left[2\left(1-\dfrac{1}{\sqrt{2}}\right) -
    \dfrac{2}{3}\left(1-\dfrac{1}{2\sqrt{2}}\right)\right]                               \nonumber      \\
     & = \dfrac{1}{2a\sqrt{b}} \left(\dfrac{4}{3} - \dfrac{5\sqrt{2}}{6}\right) \label{2006-1:eq:3}
  \end{align}
  だから，\cref{2006-1:eq:2,2006-1:eq:2}を\cref{2006-1:eq:1}に代入して
  \begin{align}
    V & = 2\pi \left[ \dfrac{1}{4\sqrt{2}a\sqrt{b}}  - \dfrac{1}{2a\sqrt{b}} \left(\dfrac{4}{3} - \dfrac{5\sqrt{2}}{6}\right) \right] \\
      & = \dfrac{2\pi}{a\sqrt{b}} \left[ \dfrac{13\sqrt{2}}{24} - \dfrac{2}{3} \right]
  \end{align}
  が求める体積である．  $\cdots$(答)
  ...
  この座標系での$PP'$の長さ
  \begin{align}
    PP' = \dfrac{t\sqrt{a}x'}{\sqrt{b}} + \sqrt{1-(x')^2}
  \end{align}
  ...
  \begin{align}
    |PP'| = g(\theta) = \dfrac{t\cos\theta}{\sqrt{a}} - \dfrac{\sin\theta}{\sqrt{b}}
  \end{align}
  と計算できるから，あとはこれを最大化する$\theta$を求める．
  これは三角関数の合成を使えば容易に求まる．簡単のため$\theta \to -\theta$の変換をかけると
  \begin{align}
    g(\theta)
     & = \dfrac{t}{\sqrt{a}}\cos\theta + \dfrac{1}{\sqrt{b}}\sin\theta     \\
     & = \sqrt{\dfrac{t^2}{a}+\dfrac{1}{b}}\sin\left(\theta + \beta\right)
  \end{align}
  なる$\beta$が存在する．ただし$0 \ge \beta \ge \pi/2$および
  \begin{align}
    \begin{dcases}
      \cos\beta = \dfrac{\dfrac{1}{\sqrt{b}}}{\sqrt{\dfrac{t^2}{a}+\dfrac{1}{b}}} = \dfrac{a}{bt^2+a} \\
      \sin\beta = \dfrac{\dfrac{t}{\sqrt{a}}}{\sqrt{\dfrac{t^2}{a}+\dfrac{1}{b}}} = \dfrac{bt^2}{bt^2+a}
    \end{dcases}
  \end{align}
  である．
  ...
  \begin{align}
    \max g(\theta) =  \sqrt{\dfrac{bt^2+a}{ab}}
  \end{align}
