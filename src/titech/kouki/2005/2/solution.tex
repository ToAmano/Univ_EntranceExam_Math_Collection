...
\begin{multicols}{2}
  ...
  \begin{align}
               & |O_A O_B|^2 = |OO_A|^2 + |OO_B|^2 -2 |OO_A||OO_B|\cos\theta \nonumber \\
  ...
  これは数式に起こすと$t = \tan\dfrac{\theta}{2}$,$c=\cos\dfrac{\theta}{2}$, $s=\sin\dfrac{\theta}{2}$ として， として
  \begin{align}
    \begin{dcases}
      0 < \theta < \pi \text{の時．} & \beta = t^2(1-\alpha) \quad \left(2\dfrac{(1-s)t}{c} \le \alpha < 1\right) \\
      \theta = \pi \text{の時．}     & \alpha=1, 0 < \beta \le \dfrac{1}{4}
  ...
  \begin{align}
    \alpha = \dfrac{2(1-s)t}{c}
  \end{align}
  で $T$ は最小で，この時の最小値は
  \begin{align}
    \dfrac{\min T}{\pi}
     & = \left[ 4\dfrac{(1-s)^2}{c^2} + 4\dfrac{s(1-s)}{c^2} - 2 \right] t^2 \nonumber \\
     & = \dfrac{2(s-1)^2 s^2}{c^4}                                          \nonumber \\
     & = \dfrac{2(1-s)^2 s^2}{(1-s^2)^2}                                    \nonumber \\
     & = \dfrac{2s^2}{(1+s)^2} \label{2005-2:eq:7}
  \end{align}
  を得る．
  ...
  $(\alpha,\beta)=(1,\dfrac{1}{4})$ の時の $\dfrac{1}{2}$ で，この時も\cref{2005-2:eq:7}で良い．
  ...
  \begin{align}
    S_\theta
     & = 2\pi \left(\dfrac{\sin\dfrac{\theta}{2}}{1+\sin\dfrac{\theta}{2}}\right)^2
  \end{align}
  である．$\cdots$(答)

  \vspace{10pt}
  (2) $x=1+\sin\dfrac{\theta}{2}$ とすると，$0\le x < 2\pi$で$0 \le x \le 2$ であり，
  \begin{align}
    S_\theta
     & = 2\pi \left(\dfrac{x-1}{x}\right)^2 \\
     & = 2\pi \left(1-\dfrac{1}{x}\right)^2
  \end{align}
  ...
  $\beta$との関係は
  \begin{align}
    \beta = \dfrac{\alpha^2 -\gamma^2}{4}
  \end{align}
  だから，条件\cref{2005-2:eq:3,2005-2:eq:4,2005-2:eq:5}は（簡単のため$\theta\neq\pi$の時のみ考えると）
  \begin{align}
     & \begin{dcases}
         S_{\theta} = \dfrac{\pi}{2}\left(\alpha^2+\gamma^2\right)              \\
         (1+\cos\theta)\dfrac{\alpha^2 -\gamma^2}{4} = (1-\cos\theta)(1-\alpha) \\
         0 < \dfrac{\alpha^2 -\gamma^2}{4}                                      \\
         \alpha-1 < \dfrac{\alpha^2 -\gamma^2}{4}                               \\
         \dfrac{\alpha^2 -\gamma^2}{4} \le \dfrac{1}{4}\alpha^2                  \\
         0 < \alpha < 2
       \end{dcases}                \\
    \therefore
     & \begin{dcases}
         \dfrac{2S_{\theta}}{\pi} = \left(\alpha^2+\gamma^2\right)           \\
         \gamma^2 = \alpha^2 + 4\dfrac{1-\cos\theta}{1+\cos\theta}(\alpha-1) \\
         \gamma^2 < \alpha^2                                                \\
         \gamma^2 < (2-\alpha)^2                                            \\
         0 < \alpha < 2
       \end{dcases}
  ...
  $\sqrt{\dfrac{2S_{\theta}}{\pi}}$は平面上の円の半径と解釈できる．
  よって，とりうる$S_{\theta}$の範囲は，赤線部の双曲線と円が交点を持つ範囲で規定される．
  図から$S_{\theta}$が最小になるのは
  円が双曲線と$\alpha$軸の交点$(\sqrt{\dfrac{4(1-\cos\theta)}{1+\cos\theta}},0)$を通る時である．
  ...
  \begin{align}
    S_{\theta}
     & = \dfrac{\pi}{2}\left[\left(\sqrt{\dfrac{4(1-\cos\theta)}{1+\cos\theta}}\right)^2+0^2\right] \\
     & = \dfrac{2\pi(1-\cos\theta)}{(1+\cos\theta)}                                                \\
     & = \dfrac{2\pi\sin\dfrac{\pi}{2}}{1+\sin\dfrac{\pi}{2}}
  \end{align}
  ...
  \begin{align}
    S_{\theta} = \pi\left(r_A^2+r_B^2\right) \le 2\pi \left(\dfrac{r_A+r_B}{2}\right)^2 = \dfrac{\pi}{2}
  \end{align}
