...
\begin{multicols}{2}
  ...
  そこで, $\triangle \text{OPQ}$ を含む平面の単位法線ベクトルを $\vec{m} =
  \begin{pmatrix} X \\ X \\ Z
  \end{pmatrix}$ とする.
  ...
  \begin{align}
    \cos \theta_1 & = \dfrac{\vec{a_1} \cdot \vec{m}}{|\vec{a_1}| |\vec{m}|} = Z, \\
    \cos \theta_2 & = \dfrac{\vec{a_2} \cdot \vec{m}}{|\vec{a_2}| |\vec{m}|} = X, \\
    \cos \theta_3 & = \dfrac{\vec{a_3} \cdot \vec{m}}{|\vec{a_3}| |\vec{m}|} = Y
  \end{align}
  となる.
  \begin{figure}[H]
    \begin{tikzpicture}
  ...
  \begin{align}
    ...
    S_1, S_2, S_3 \ge 0
  \end{align}
  をみたしながら動く.この曲面は\cref{2003-1:fig:2}のようになる．
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \node[right, align=left] at (axis cs:0.577, 0.577, 0.577) {
          $(\dfrac{S}{\sqrt{3}}S,\dfrac{S}{\sqrt{3}}S,\dfrac{S}{\sqrt{3}}S)$
        };
  ...
  \begin{align}
    ...
    & = \sqrt{2}S\cos\theta\sin\left(\psi+\dfrac{\pi}{4}\right) + S\sin\theta
  \end{align}
  の最大最小値である．区間内で$\cos\theta\ge 0$だから，まず$\theta$を固定して$\psi$を動かして
  最大最小値を考えると，
  \begin{align}
    \dfrac{1}{\sqrt{2}} \le \sin\left(\psi+\dfrac{\pi}{4}\right) \le 1
  \end{align}
  である．等号成立は，下側は$\psi=0,\pi/2$，上側は$\psi=\pi/4$である．従って
  \begin{align}
    S\cos\theta + S\sin\theta \le f(\theta,\psi) \le \sqrt{2}S\cos\theta + S\sin\theta \\
    \sqrt{2}S\sin\left(\theta+\dfrac{\pi}{4}\right) \le f(\theta,\psi) \le \sqrt{3}S\sin(\theta+\alpha)
  \end{align}
