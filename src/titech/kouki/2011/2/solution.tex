...
\begin{multicols}{2}
  ...
  \begin{align}
    x^2
     & = \dfrac{1}{\tan^2 \theta}             \\
     & = \dfrac{1-\cos\theta^2}{\cos\theta^2} \\
     & = \dfrac{1}{\cos\theta^2}-1            \\
     & = y^2-1
  \end{align}
  となり，$C$は双曲線である．
  $0 < \theta < \pi/2$ から, $0 < x, 1 \leq y$ で,
  求める$C$の概形は\cref{2011-2:fig:1}である．
  ...
  \begin{align}
    S_1
     ...
     & = \left[\dfrac{x}{2}\sqrt{x^2+1} + \dfrac{1}{2}\log(x+\sqrt{x^2+1})\right]_{0}^{t} \\
     & = \dfrac{1}{2}\left[t\sqrt{t^2+1} + \log(t+\sqrt{t^2+1})\right]                   \\
  \end{align}
  である． $\cdots$(答)

  次に$S_2$は底辺$|OP|=t$，高さ$|PQ|=\sqrt{t^2+1}$の三角形の面積であり，
  \begin{align}
    S_2 = \dfrac{1}{2}t\sqrt{t^2+1}
  \end{align}
  である． $\cdots$(答)
  ...
  \begin{align}
    \dfrac{S_1 - S_2}{\log t}
     & = \dfrac{1}{2}\dfrac{\log(t+\sqrt{t^2+1})}{\log t}                   \\
     & = \dfrac{1}{2}\dfrac{\log t + \log(1+\sqrt{1+1/t^2})}{\log t}        \\
     & = \dfrac{1}{2}\left(1+\dfrac{\log (1+\sqrt{1+1/t^2})}{\log t}\right) \\
     & =\to \dfrac{1}{2} \quad (t \to \infty)
  \end{align}
  ...
  どのように振る舞うかというのを示す問題である．この様子を\cref{2011-2:fig:2}に示す．
