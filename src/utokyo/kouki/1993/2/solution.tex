...
\begin{multicols}{2}
  ...
  \begin{align}
    \begin{dcases}
      x_g = \dfrac{x_1+x_2+x_3}{3} \\
      y_g = \dfrac{y_1+y_2+y_3}{3}
    \end{dcases}
  \end{align}
  とおく．
  ...
  \begin{align}
    d(l,k) = \dfrac{|g(x_k, y_k)|}{\sqrt{a^2+b^2}}
  \end{align}
  なので，
  \begin{align}
  ...
  \begin{align}
    f(l)
     & = \dfrac{1}{a^2+b^2} \sum_{k=1}^{3} \{g(x_k, y_k)\}^2                                                              \\
     & = \dfrac{1}{a^2+b^2} \sum_{k=1}^{3} [c^2 + 2T_kc + T_k^2]                                                          \\
     & = \dfrac{1}{a^2+b^2} \left[3c^2 + 2(\sum_{k=1}^{3} T_k)c + \sum_{k=1}^{3} T_k^2\right]                             \\
     & = \dfrac{3}{a^2+b^2} \left[\left(c+\dfrac{1}{3}\sum_{k=1}^{3} T_k\right)^2 + \dfrac{8}{9}\sum_{k=1}^{3} T_k^2\right]
  \end{align}
  である．
  $c$を動かすと
  \begin{align}
    c = -\dfrac{1}{3}\sum_{k=1}^{3}T_k
  \end{align}
  で $f(l)$ は最小値をとる．
  この時の$c$を具体的に計算すると
  \begin{align}
    c
     & = -\dfrac{1}{3} [a(x_1+x_2+x_3 - 3x_g) + b(y_1+y_2+y_3 - 3y_g)] \\
     & = 0
  \end{align}
  だから，$l$の方程式は
  \begin{align}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      \begin{axis}[
          ...
          xlabel=$x$,          % x軸ラベル
          ylabel=$y$,          % y軸ラベル
          xmin=-1.8, xmax=1.8, % x軸の範囲
          ymin=-1.8, ymax=1.8, % y軸の範囲
          xtick=\empty,        % x軸の目盛りを非表示
          ytick=\empty,        % y軸の目盛りを非表示
          axis equal,          % 軸のスケールを等しくする
          clip=false,          % 描画領域外の描画を許可
        ]

        % --- 座標の定義 ---
        \coordinate (O) at (0,0);
        \coordinate (A) at (1,0);
        % A(1,0)で重心がO(0,0)の正三角形の残り2頂点
  ...
  \begin{align}
    ...
    \end{dcases}
  \end{align}
  第二式から $y_2=0$ または $x_2=-\dfrac{1}{2}$ である．
  $y_2=0$の時\cref{1993-2:eq:1}から$y_3=0$となり$\triangle ABC$ が三角形とならず不適である．

  よって$x_2=-\dfrac{1}{2}$ である．この時第一式から$y_2=\pm\dfrac{\sqrt{3}}{2}$ である．
  \cref{1993-2:eq:1}から$x_3 =-\dfrac{1}{2}, y_3=\mp\dfrac{\sqrt{3}}{2}$である．

  まとめると
  \begin{align}
     & A=(1,0)
     & B=(-\dfrac{1}{2}, \pm\dfrac{\sqrt{3}}{2}) \\
     & C=(-\dfrac{1}{2}, \mp\dfrac{\sqrt{3}}{2})
  \end{align}
