...
\begin{multicols}{2}
  ...
  もし $A,B,C$ が\cref{1995-2:fig:1}で斜線部で, 境界を含まない部分にあるとすると $C$ から $AB$ に下ろした垂線と $E,F$ との交点のうち, 垂足 $H$ との距離が $|CH|$ よりも大きいもの $C'$ が存在し
  ...
  \begin{align}
    S(p)
    & = \dfrac{1}{2} |AB|\cdot |HC| \\
    & = \dfrac{1}{2} \cdot 2 \sqrt{1-p^2} \cdot (p+2) \\
    & = \sqrt{(1-p^2)(p+2)^2}                        \\
    & \equiv \sqrt{f(p)}
  \end{align}
  である．
  ...
  \begin{table}[H]
    ...
    \begin{tabular}{|c||c|c|c|c|c|}
      \hline
      $p$  & $0$ & $\cdots$   & $\dfrac{-1+\sqrt{3}}{2}$ & $\cdots$   & $1$ \\
      \hline
      $f'$ &     & $+$        & $0$                     & $-$        &     \\
      \hline
      $f$  &     & $\nearrow$ &                         & $\searrow$ &     \\
  ...
  したがって，$f$および$S$は$p=\dfrac{-1+\sqrt{3}}{2}$で最大値
  \begin{align}
    \max S
    & = \sqrt{\max f(p)}                               \\
    & = \sqrt{f\left(\dfrac{-1+\sqrt{3}}{2}\right)}     \\
    & = \sqrt{\dfrac{1+2p}{2}} \dfrac{3+\sqrt{3}}{2}     \\
    & = \sqrt{\dfrac{\sqrt{3}}{2}} \dfrac{3+\sqrt{3}}{2}
  \end{align}
  をとる．
