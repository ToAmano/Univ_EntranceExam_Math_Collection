...
\[
(与式) \iff 1 - x < (1+x)^{\dfrac{1}{x}}(1-x)^{\dfrac{1}{x}} \quad \cdots (2)
\]
与式の両辺対数をとって、
\[
(1 - \dfrac{1}{x}) \log(1-x) < \dfrac{1}{x} \log(1+x) \quad \cdots (3)
\]
...
\[
\begin{cases}
f'(x) = \dfrac{1}{x+1} - \log(1-x) - 1 \\
f''(x) = \dfrac{1}{1-x} - \dfrac{1}{(x+1)^2} = \dfrac{x(x+3)}{(x+1)^2(1-x)} \quad \cdots (5)
\end{cases}
\]
...
(2) まず、(1)に $x = \dfrac{1}{100}$ ((1)をみたす) を代入して、
\[
0.99 < 0.9999^{100} \quad \cdots (7)
\]
又、与式の両辺に $(1+x)^{1-\dfrac{1}{x}}$ をかけて、
\[
(1-x^2)^{1-\dfrac{1}{x}} < 1+x
\]
$x = -\dfrac{1}{100}$ ((1)をみたす) として、
\[
