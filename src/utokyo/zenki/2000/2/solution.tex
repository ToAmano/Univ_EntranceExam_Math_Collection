\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

[解] $OR \perp l$ から，
\[ \dfrac{w}{\alpha - \beta} \text{ は純虚数} \cdots \text{①} \]
さらに，Rは $l$ 上だから，$t \in \mathbb{R}$ として，
\[ (w - \beta) = t(\alpha - \beta) \]
\[ \therefore \dfrac{w-\beta}{\alpha-\beta} \in \mathbb{R} \cdots \text{②} \]
である．①，②から
\[ \begin{cases} \left( \dfrac{w}{\alpha-\beta} \right) + \overline{\left( \dfrac{w}{\alpha-\beta} \right)} = 0 \\ \left( \dfrac{w-\beta}{\alpha-\beta} \right) = \overline{\left( \dfrac{w-\beta}{\alpha-\beta} \right)} \end{cases} \]
\[ \begin{cases} w(\bar{\alpha}-\bar{\beta}) + \bar{w}(\alpha-\beta) = 0 \cdots \text{③} \\ (w-\beta)(\bar{\alpha}-\bar{\beta}) = (\bar{w}-\bar{\beta})(\alpha-\beta) \cdots \text{④} \end{cases} \]
まず，$w \neq 0$ だから，③から
\[ \bar{\alpha}-\bar{\beta} = -\dfrac{\bar{w}}{w}(\alpha-\beta) \]
だから，④に代入，セイリして，
\[ -(w-\beta)\dfrac{\bar{w}}{w}(\alpha-\beta) = (\bar{w}-\bar{\beta})(\alpha-\beta) \]
$\alpha \neq \beta, w \neq 0$ から，
\[ -(w-\beta)\bar{w} = w(\bar{w}-\bar{\beta}) \]
\[ 2|w|^2 - \bar{\beta}w - \beta\bar{w} = 0 \cdots \text{⑤} \]
同様にして，
\[ 2|w|^2 - \bar{\alpha}w - \alpha\bar{w} = 0 \cdots \text{⑥} \]
を得る．

$1^\circ$ 十分性
$w = \alpha\beta$ の時，⑤，⑥から，
\[ \begin{cases} 2|\alpha|^2|\beta|^2 - \alpha|\beta|^2 - \bar{\alpha}|\beta|^2 = 0 \\ 2|\alpha|^2|\beta|^2 - \beta|\alpha|^2 - \bar{\beta}|\alpha|^2 = 0 \end{cases} \]
$\alpha, \beta \neq 0$ だから，
\[ \left|\alpha - \dfrac{1}{2}\right|^2 = \dfrac{1}{4}, \quad \left|\beta - \dfrac{1}{2}\right|^2 = \dfrac{1}{4} \cdots \text{⑦} \]
となり，各辺 $0$ 以上だから
\[ \left|\alpha - \dfrac{1}{2}\right| = \dfrac{1}{2}, \quad \left|\beta - \dfrac{1}{2}\right| = \dfrac{1}{2} \cdots \text{⑧} \]
より，$P(\alpha), Q(\beta)$ は題意の円周上にある．

$2^\circ$ 必要性
⑧が成立するので，両辺2乗して⑦を得る．変形して，
\[ \begin{cases} \bar{\alpha} = \dfrac{\alpha}{2\alpha-1} \\ \bar{\beta} = \dfrac{\beta}{2\beta-1} \end{cases} \quad \left(\because \alpha, \beta \neq \dfrac{1}{2}\right) \cdots \text{⑨} \]
$A = \alpha - \beta$ とおく．$\alpha \neq \beta$ より $A \neq 0$ だから③より
\[ \bar{w} = -\dfrac{\bar{A}}{A}w \]
だから④に代入，セイリして，
\[ \left(-\dfrac{\bar{A}}{A}w - \bar{\beta}\right)A = (w - \beta)\bar{A} \]
\[ -\bar{A}w - \bar{\beta}A = \bar{A}w - \beta\bar{A} \]
\[ \beta\bar{A} - \bar{\beta}A = 2\bar{A}w \]
\[ w = \dfrac{\beta\bar{A} - \bar{\beta}A}{2\bar{A}} \]
\[ = \dfrac{\beta}{2} - \dfrac{1}{2} \left( \dfrac{\beta}{2\beta-1} \right) \dfrac{\alpha-\beta}{\dfrac{\alpha}{2\alpha-1} - \dfrac{\beta}{2\beta-1}} \quad (\because \text{⑨}) \]
\[ = \dfrac{\beta}{2} - \dfrac{1}{2} (2\alpha-1)\beta = \alpha\beta \]
を得る．
以上 $1^\circ$，$2^\circ$ から示された．
\begin{flushright}
$\blacksquare$
\end{flushright}

\begin{center}
\begin{tikzpicture}
\draw[->] (-1,0) -- (4,0) node[right] {$x$};
\draw[->] (0,-1) -- (0,3) node[above] {$y$};
\node at (0,0) [below left] {O};
\draw (0.5,2.5) -- (3,0.5);
\node at (0.8,2.2) [above right] {$P(\alpha)$};
\fill (0.8,2.26) circle (1.5pt);
\node at (2.5,0.9) [above right] {$Q(\beta)$};
\fill (2.5,0.9) circle (1.5pt);
\draw (0,0) -- (1.5,1.7);
\node at (1.5,1.7) [above right] {$R(w)$};
\fill (1.5,1.7) circle (1.5pt);
\draw (1.3,1.55) -- (1.45,1.4) -- (1.6,1.6);
\end{tikzpicture}
\end{center}

\end{document}
