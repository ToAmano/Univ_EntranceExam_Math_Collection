\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

\noindent
{\bf [解]} $n \in \mathbb{N}_{\ge 2} \quad \cdots \textcircled{1}$

\bigskip

\noindent
\paragraph{(1)}

連続 2 自然数の積が $n$ 乗数になるものがあると仮定し，それを $m(m+1)$ ($m \in \mathbb{N}$) とおくと，$k \in \mathbb{N}$ として
\begin{align}
m(m+1) = k^n
\end{align}
とかける．$m \perp m+1$ から，$m, m+1$ が共に $n$ 乗数であることが必要で，
$t, s \in \mathbb{N}$ として
\begin{align}
m = t^n, \quad m+1 = s^n
\end{align}
とかける．$m$ をけして，
\begin{align}
t^n = s^n - 1 \quad \therefore 1 = s^n - t^n = (s-t)(s^{n-1} + \dots + t^{n-1})
\end{align}
①から，$s-t \in \mathbb{Z}, s^{n-1} + \dots + t^{n-1} \ge 1+1 = 2$ となって矛盾．よって示された．

\bigskip

\noindent
\paragraph{(2)}

$n=2$ での成立は示されたので，$n \ge 3$ とする．
(1) と同様にして，$A_m = \dfrac{(m+n)!}{m!} = \prod_{k=1}^n (m+k)$ が $n$ 乗数だと仮定する．($m \in \mathbb{Z}_{\ge 0}, p \in \mathbb{N}$ として)
\begin{align}
A_m = p^n
\end{align}
とかける．$(m+1)^n < A_m < (m+n)^n$ から ($\because m \in \mathbb{Z}_{\ge 0}$)
\begin{align}
(m+1)^n &< p^n < (m+n)^n \\
m+1 &< p < m+n
\end{align}
したがって，$p = m+k \; (k=2, \dots, n-1)$ とかけるが，$A_m$ は $p$ と互いに素な $m+k+1$ を因数に持つ．不適．よって矛盾し題意は示された．

\end{document}
