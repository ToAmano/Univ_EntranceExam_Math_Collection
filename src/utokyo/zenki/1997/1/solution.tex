...
$\triangle OAB$ の中点を $M$ とする. $MC = \dfrac{\sqrt{3}}{2} AB$ 及び $MC \perp AB$ だから, $\vec{AB} = \begin{pmatrix} -a \\ b \end{pmatrix}$ より
\[
\vec{MC} = \pm \dfrac{\sqrt{3}}{2} \begin{pmatrix} b \\ a \end{pmatrix}
\]
となり,
\[
\vec{OC} = \vec{OM} + \vec{MC} = \dfrac{1}{2} \begin{pmatrix} a \\ b \end{pmatrix} \pm \dfrac{\sqrt{3}}{2} \begin{pmatrix} b \\ a \end{pmatrix} = \dfrac{1}{2} \begin{pmatrix} a \pm \sqrt{3}b \\ b \pm \sqrt{3}a \end{pmatrix} \quad (\text{複号同順})
\]
$C$ は第1象限の点だから, 複号正を採用して, $C\left(\dfrac{a + \sqrt{3}b}{2}, \dfrac{b + \sqrt{3}a}{2}\right)$ である. $\triangle ABC$ が $D$ に含まれるには, $A, B, C$ が $D$ に含まれていれば良い. $a, b > 0$ とあわせて,
\[
\begin{cases}
...
\node[right] at (1.1, 0.5) {$b = \dfrac{\sqrt{3}}{3}(2-a)$};
...
$\triangle ABC$ の一辺は $\sqrt{a^2+b^2}$ だから
\[
S = \dfrac{\sqrt{3}}{4}(a^2+b^2) \quad \cdots \text{①}
\]
