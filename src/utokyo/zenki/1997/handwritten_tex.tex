...
\noindent
\textbf{[解]}
(1) $\triangle OAB$ の中点を $M$ とする. $MC = \frac{\sqrt{3}}{2} AB$ 及び $MC \perp AB$ だから, $\vec{AB} =
\begin{pmatrix} -a \\ b
\end{pmatrix}$ より
\[
  \vec{MC} = \pm \frac{\sqrt{3}}{2}
  \begin{pmatrix} b \\ a
  \end{pmatrix}
\]
となり,
\[
  \vec{OC} = \vec{OM} + \vec{MC} = \frac{1}{2}
  \begin{pmatrix} a \\ b
  \end{pmatrix} \pm \frac{\sqrt{3}}{2}
  \begin{pmatrix} b \\ a
  \end{pmatrix} = \frac{1}{2}
  \begin{pmatrix} a \pm \sqrt{3}b \\ b \pm \sqrt{3}a
  \end{pmatrix} \quad (\text{複号同順})
\]
...
\[
  \vec{PA} \cdot \vec{PB} =
  \begin{pmatrix} 1-p \\ -p \\ -q
  \end{pmatrix} \cdot
  \begin{pmatrix} -p \\ 1-p \\ -q
  \end{pmatrix} = 2p(p-1) + q^2
\]
\[
