...
\textbf{[解1]} 
\[
(\text{与式}) \iff \left( \dfrac{n^2}{2n+1} - m \right) a + m^2 + m > 0 \quad \cdots \text{①}
\]
ここで, $f(m) = -\dfrac{m^2+m}{A-m} \quad \left(A = \dfrac{n^2}{2n+1}\right)$ とおく.
\[
f(m) = +\left( m+A+1 + \dfrac{A(A+1)}{m-A} \right)
\]
\[
f'(m) = +\left( \dfrac{m^2-2Am+A}{(m-A)^2} \right)
\]
より, 下表を与える.

\begin{center}
\begin{tabular}{c|c|c|c|c|c|c|c}
$m$ & $\dots$ & $-\dfrac{n}{2n+1}$ & $\dots$ & $A$ & $\dots$ & $n$ & $\dots$ \\ \hline
$f'$ & $+$ & $0$ & $-$ & / & $-$ & $0$ & $+$ \\ \hline
$f$ & $\nearrow$ & $\dfrac{1}{2n+1}$ & $\searrow$ & / & $\searrow$ & $2n+1$ & $\nearrow$
\end{tabular}
\end{center}
...
\node[below] at (-1,0) {$-\dfrac{n}{2n+1}$};
\node[above left] at (0, 0.3) {$\dfrac{1}{2n+1}$};
...
\noindent
\textbf{[解2]} $f(x) = x^2 - (a-1)x + Aa$ とおく $\left(A = \dfrac{n^2}{2n+1}\right)$. $f(x)=0$ の判別式 $D$ として
\[
D = (a-1)^2 - 4Aa = a^2 - 2(1+2A)a + 1
\]
から, $D < 0 \iff (1+2A) - \sqrt{4A^2+4A} < a < (1+2A) + \sqrt{4A^2+4A}$
\[
\iff \dfrac{1}{2n+1} < a < 2n+1
\]
の時, 任意の $x$ に対し $f(x) > 0$ となる. その他の時, $f(x)=0$ は
\[
x = \dfrac{1}{2}\left[ (a-1) \pm \sqrt{(a-1)^2 - 4Aa} \right] = \dfrac{1}{2}\left[ (a-1) \pm \sqrt{a^2 - 2(1+2A)a + 1} \right]
\]
...
$2^\circ \ 2n+1 \le a$
\[
f(n) = (A-n)a + n^2 + n = \dfrac{-(n^2+n)}{2n+1} a + n^2 + n \le 0
\]
から不適.
