...
\[
f\left(\dfrac{a}{2}\right) = b \iff b = \dfrac{1}{4}a^3 - a \quad \dots \text{②}
\]
この時 $(a,b) \ne (0,0)$ だから $a \ne 0$ で, ①に代入して,
\[
...
\[
\min\left(g(2X), g(X+1)\right) \le Y \le \max\left(g(X-1), g\left(\dfrac{2}{3}X\right)\right) \land g(0) \ne Y \quad \dots \text{⑤}
\]
ただし, $Y \ne g(2X), Y \ne g(X+1)$ である.
...
したがって, ④が $a \ne 0$ に3実解を持つ条件は
\[
\min\left(g\left(\dfrac{2}{3}X\right), g(X+1)\right) \le Y \le \max\left(g(2X), g(X-1)\right) \land g(0) \ne Y \quad \dots \text{⑥}
\]
ただし, $Y \ne g(2X), Y \ne g\left(\dfrac{2}{3}X\right)$ である.
...
&= X^3 - X \quad \dots \text{⑨} \\
g\left(\dfrac{2}{3}X\right) &= \dfrac{1}{4}\left(\dfrac{2}{3}X\right)^3 - \left(\dfrac{2}{3}X\right) + \left(\dfrac{1}{3}X\right)^3 - \left(\dfrac{1}{3}X\right) \\
...
$\vec{A} = \begin{pmatrix} a \\ b \end{pmatrix}$ だけ平行移動とする. $OA$ の中点 $M\left(\dfrac{1}{2}a, \dfrac{1}{2}b\right)$ とする. $C$ の任意の点 $(x,y)$ の, $M$ に関する対称点を $(x',y')$ とすると,
\[
x = a - x', \quad y = b - y'
\]
だから, $y = x^3 - x$ に代入して
...
となり,
\[
\dfrac{b}{2} = f\left(\dfrac{a}{2}\right) \quad \therefore b = \dfrac{1}{4}a^3 - a
\]
を得る.
