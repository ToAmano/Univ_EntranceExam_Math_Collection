...
\[
  \vec{n}_1 =
  \begin{pmatrix} 1 \\ -1 \\ -1
  \end{pmatrix}, \quad \vec{n}_2 =
  \begin{pmatrix} -1 \\ 1 \\ -1
  \end{pmatrix}, \quad \vec{n}_3 =
  \begin{pmatrix} -1 \\ -1 \\ 1
  \end{pmatrix}, \quad \vec{n}_4 =
  \begin{pmatrix} 1 \\ 1 \\ 1
  \end{pmatrix}
\]
...
$\vec{A} =
\begin{pmatrix} a \\ b
\end{pmatrix}$ だけ平行移動とする. $OA$ の中点 $M\left(\frac{1}{2}a, \frac{1}{2}b\right)$ とする. $C$ の任意の点 $(x,y)$ の, $M$ に関する対称点を $(x',y')$ とすると,
\[
  x = a - x', \quad y = b - y'
\]
だから, $y = x^3 - x$ に代入して
...
\[
  \vec{P_{n-1} P_n} =
  \begin{pmatrix} S^{n-1} a \\ 1
  \end{pmatrix}
\]
だから,
\begin{align*}
  \vec{OP_n} &= \sum_{k=1}^n \vec{P_{k-1} P_k} = \sum_{k=1}^n
  \begin{pmatrix} S^{k-1} a \\ 1
  \end{pmatrix} \\
  &=
  \begin{cases}
    \begin{pmatrix} \frac{1}{a} \frac{1-(1/S)^n}{1-1/S} \\ n
    \end{pmatrix} & (S \ne 1) \\[10pt]
    \begin{pmatrix} \frac{n}{a} \\ n
    \end{pmatrix} & (S=1)
  \end{cases}
\end{align*}
