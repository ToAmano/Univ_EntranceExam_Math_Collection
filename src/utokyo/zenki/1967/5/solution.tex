...
このとき，求める面積$S$は
     ...
     &=\left[\dfrac{a}{3}x^3\right]_0^t-\left[x(\log x-1)\right]_1^t  \\
     &=\dfrac{at^3}{3}-t(\log t-1)-1  \\
     &=\dfrac{1}{6}\sqrt{e}+\dfrac{1}{2}\sqrt{e}-1 \tag{$\because\eqref{1}$}     \\
     &=\dfrac{2}{3}\sqrt{e}-1\cdots\text{(答)}
     \end{align}
である．     
\newpage
\end{multicols}
