...
&= \dfrac{\alpha}{\sqrt{t^2+1}} \left[ \alpha (1 - 2t\sqrt{t^2+1}) + t + \sqrt{t^2+1} \right] \\
...
(2) $-t + \sqrt{t^2+1} \le t \iff \dfrac{1}{\sqrt{3}} \le t \quad (\because 0 < t < 1 \text{ だから (1) より})$
\[
\beta = \begin{cases} -t + \sqrt{t^2+1} & \left( \dfrac{1}{\sqrt{3}} \le t \le 1 \right) \quad \cdots \text{①} \\ t & \left( 0 < t \le \dfrac{1}{\sqrt{3}} \right) \quad \cdots \text{②} \end{cases}
\]
であり, ②の時
\[
f(\beta) = \dfrac{2\beta}{\beta (1-\beta^2)} = \dfrac{2}{1-\beta^2} \quad \left( 0 < \beta \le \dfrac{1}{\sqrt{3}} \right)
\]
...
\begin{tikzpicture}[scale=1.5]
  ...
  \draw[dashed] (0.577,0) node[below] {$\dfrac{1}{\sqrt{3}}$} -- (P1) -- (0,3) node[left] {$3$};
