...
[解] $C\left(c, \dfrac{1}{c}\right)$ ($c > 0$) での $y = \dfrac{1}{x}$ の接線は
\[
y = -\dfrac{1}{c^2} x + \dfrac{2}{c}
\]
であるから, これと $x, y$ 軸の交点は各々 $P(2c, 0)$, $Q\left(0, \dfrac{2}{c}\right)$ である。
...
\[
S = \dfrac{1}{2} \left| b(c - a) + \dfrac{a}{c} \right|
\]
である。今, $a, c$ を固定し, 上式を $f(b)$ とおくと,

$1^\circ$ $c - a = 0$ の時
\[
S = \dfrac{1}{2} \dfrac{a}{c} \quad (\because a, c > 0)
\]

$2^\circ$ $c - a \ne 0$ の時
$f(b)$ は $b$ の単調関数なので, $S$ は $b = 0$ または $b = \dfrac{2}{c}$ の時, 最大となるから,
\[
\max S = \max \left\{ \dfrac{1}{2} \dfrac{a}{c}, \,\, \dfrac{1}{2}\left|2 - \dfrac{a}{c}\right| \right\}
\]
である ($\because$ ①より $0 \le \dfrac{a}{c} \le 2$)。

次に $a$ を動かす。($0 \le a \le 2c, a \ne c$) の時, $\dfrac{1}{2c} a$, $\dfrac{1}{2}\left|2 - \dfrac{a}{c}\right|$ のグラフは右図のようになり,
\[
\max S = 1 \text{ であり, この時}
\]
...
\begin{tikzpicture}[scale=1.2, >=stealth]
  ...
  \begin{scope}[shift={(6,-2)}, scale=1.0]
    ...
    \node[below left] at (0,0) {$O$};
    \draw[thick] (0,0) -- (2,1) node[right] {$\dfrac{1}{2c}a$};
    \draw[thick] (0,1) -- (2,0) node[right] {$\dfrac{1}{2}\left(2-\dfrac{a}{c}\right)$};
    \draw[dashed] (1,0) node[below] {$c$} -- (1,0.5);
    \draw[dashed] (0,0.5) node[left] {$\dfrac{1}{2}$} -- (1,0.5);
