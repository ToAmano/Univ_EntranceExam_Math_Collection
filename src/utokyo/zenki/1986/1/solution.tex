...
{\bf [解]} $C\Bigl(c, \dfrac{1}{c}\Bigr)$ ($c > 0$) での $y = \dfrac{1}{x}$ の接線は
\begin{align}
...
\bigskip\noindent{\bf [解1]} $\triangle ABC$ の面積を $S$ とすると
\begin{align}
S = \dfrac{1}{2} \Bigl| b(c - a) + \dfrac{a}{c} \Bigr|
\end{align}
...
次に $a$ を動かす．($0 \le a \le 2c, a \ne c$) の時，$\dfrac{1}{2c} a$，$\dfrac{1}{2}\Bigl|2 - \dfrac{a}{c}\Bigr|$ のグラフは右図のようになり，
\begin{align}
\max S = 1 \text{ であり，この時}
\end{align}
