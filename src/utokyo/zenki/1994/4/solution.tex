...
であるから，漸化式に代入して
     ...
     &=cg_n(c)+g(c) \nonumber \\
     \therefore g_{n+1}(c)-\dfrac{g(c)}{1-c}&=c\left\{ g_{n}(c)-\dfrac{g(c)}{1-c}\right\} \label{1}
     \end{align}
...
であるから，\eqref{1}，\eqref{2}から
     \begin{align}
     g_n(c)&=c^{n-1}\left\{(1+c)g(c)-\dfrac{g(c)}{1-c}\right\}+\dfrac{g(c)}{1-c} \nonumber \\
     &\limit{n}{\infty} \dfrac{g(c)}{1-c}
     \end{align}     
を得る$(\because0<c<1)$．故に
     \begin{subnumcases}
     {}
     xf(x)=g(x)+x\dfrac{g(x)}{1-x}  \label{4}\\
     f(0)=1 \label{5}
     \end{subnumcases}
...
従って\eqref{6}を書き換えると
     \begin{align}
     \dfrac{dy}{y}&=\dfrac{dx}{x(1-x)}\nonumber{}　\\
     y&=C\dfrac{x}{1-x}  \nonumber\\
     \therefore f(x)&=C\dfrac{1}{(1-x)^2} \label{7}
     \end{align}       
