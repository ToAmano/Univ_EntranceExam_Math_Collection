\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

\noindent
\textbf{[研究]}

\noindent
$\triangleright$ 問題は(2). 換言すると
\[
p = \dfrac{1}{n}, \quad q = \dfrac{1}{m}, \quad p+4q < 1
\]
をみたす $(p,q)$ のうち
\[
E = \left(p - \dfrac{1}{2}\right)^2 + 2(q-1)^2
\]
を最大にする $(p,q)$ は？

\medskip

\noindent
$\to (p,q)$ をメインにするか $(n,m)$ をメインにするか.

\noindent
＊ ① $(p,q)$ メインなら, 図示してしまうのが良さそう.
$E$ は, $p,q$ についてそれぞれ単調減少だから, $(p,q)$ は大きい程良い.
$\to$ 定右面, どちらかで場合分けし、どれが良いかギンミする.
$\Rightarrow$ ただ $\dfrac{1}{\text{◯}}$ の形が使いにくいかも.

\noindent
＊ ② $(m,n)$ メインの時, $p+4q < 1 \iff 4n+m < nm$ から.
不定方程式を考えて, $m,n$ を極力小さくとれば良い.
$\longrightarrow (n-1)(m-4) > 4$ から, $m-4 \ge 1 \iff m \ge 5$ が必要.
さらに $m-4 \ge 5$ となれば, $n$ は任意.
$\Rightarrow$ こちらの方がよさそう.

\bigskip

\noindent
\textbf{[解]} 題意より, 下表をえる. ($r = 1 - p - 4q \quad \dots \textcircled{1}$)

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
1 & 2 & 3 & 4 & 5 & 6 \\ \hline
$q$ & $q$ & $p$ & $r$ & $q$ & $q$ \\ \hline
\end{tabular}
\end{center}

\noindent
(1) $1^\circ \ a+b \le 4$

\noindent
1枚目の $a$ の値で場合分けして ($a=1, a=2, a=3$)
\[
P(A) = q(q+q+p) + q(q+q) + pq = 4q^2 + 2pq \quad \dots \textcircled{2}
\]

\noindent
$2^\circ \ a < b$

\noindent
1と同様に ($a=1, a=2, a=3, a=4, a=5$)
\[
P(B) = q(1-q) + q(1-2q) + p(1-2q-p) + r(q+q) + q \cdot q
\]
\[
= -p^2 + p - 4pq - 10q^2 + 4q \quad (\because \text{\textcircled{1}}) \quad \dots \textcircled{3}
\]

\noindent
\textcircled{2}, \textcircled{3} から
\[
E = 2(4q^2 + 2pq) + (-p^2 + p - 4pq - 10q^2 + 4q)
\]
\[
= -p^2 + p - 2q^2 + 4q \quad \hfill \qed
\]

\bigskip

\noindent
(2) $\dfrac{1}{p} = n, \dfrac{1}{q} = m \; (n,m \in \mathbb{N})$ とおく. $0 < p, q, r < 1$ から,
\[
0 < \dfrac{1}{n}, \dfrac{1}{m} < 1, \quad 0 < 1 - \dfrac{1}{n} - \dfrac{4}{m} < 1
\]
前者から, $n, m \ge 2$ であり, 後者から
\[
0 < 4n + m < mn
\]
左側の不等式の成立は母から明らかなので, 右についてかんがえると,
\[
(n-1)(m-4) > 4 \quad \dots \textcircled{5}
\]

\noindent
又,
\[
E = -\left(p - \dfrac{1}{2}\right)^2 + \dfrac{1}{4} - 2(q-1)^2 + 2
\]
\[
= \dfrac{9}{4} - \left(\dfrac{2-n}{2n}\right)^2 - 2\left(\dfrac{1-m}{m}\right)^2 \quad \dots \textcircled{6}
\]

\noindent
ここで, $y = f(x) = \dfrac{2-x}{2x}$, $y = g(x) = \dfrac{1-x}{x}$ のグラフは下図.

\begin{center}
\begin{tikzpicture}[scale=0.9, >=stealth]
  \begin{scope}[shift={(0,0)}]
    \draw[->] (-0.5,0) -- (3,0) node[right] {$x$};
    \draw[->] (0,-1.5) -- (0,1.5) node[above] {$y$};
    \draw[dashed] (0,-0.5) -- (3,-0.5) node[right] {$y=-\dfrac{1}{2}$};
    \draw[domain=0.5:2.8, smooth, variable=\x, blue, thick] plot ({\x}, {(2-\x)/(2*\x)});
    \node[below left] at (2,0) {$2$};
    \node at (1.5,1) {$y = f(x)$};
  \end{scope}

  \begin{scope}[shift={(5,0)}]
    \draw[->] (-0.5,0) -- (3,0) node[right] {$x$};
    \draw[->] (0,-1.5) -- (0,1.5) node[above] {$y$};
    \draw[dashed] (0,-1) -- (3,-1) node[right] {$y=-1$};
    \draw[domain=0.4:2.8, smooth, variable=\x, red, thick] plot ({\x}, {(1-\x)/\x});
    \node[below left] at (1,0) {$1$};
    \node at (1.5,1) {$y = g(x)$};
  \end{scope}
\end{tikzpicture}
\end{center}

\noindent
又, \textcircled{5} をみたす $(m,n)$ は以下のようになる.

\begin{center}
\begin{tabular}{c|c}
$n$ & $m$ \\ \hline
2 & $m \ge 9$ \\ \hline
3 & $m \ge 7$ \\ \hline
4 & $m \ge 6$ \\ \hline
5 & $m \ge 6$ \\ \hline
$n \ge 6$ & $m \ge 5$
\end{tabular}
\end{center}

\noindent
左をみたす定義域内では, $\{f(n)\}^2, \{g(m)\}^2$ は共に単調増加だから, $E$ を最大にする候補は左表の等号成立時の $(n,m)$ である.

\noindent
又, $n = 4, 5$ の時については $m \ge 6$ だから, $(n,m) = (5,6)$ の場合より $(n,m) = (4,6)$ の時の方が $E$ は大きい.

\begin{center}
\begin{tabular}{c|l}
$(n,m)$ & $\{f(n)\}^2 + 2\{g(m)\}^2$ \\ \hline
$(2,9)$ & $0 + 2\left(\dfrac{8}{9}\right)^2 = \dfrac{128}{81}$ \\ \hline
$(3,7)$ & $\left(\dfrac{1}{6}\right)^2 + 2\left(\dfrac{6}{7}\right)^2 = \dfrac{7^2 + 2 \cdot 6^4}{6^2 \cdot 7^2}$ \\ \hline
$(4,6)$ & $\left(\dfrac{1}{4}\right)^2 + 2\left(\dfrac{5}{6}\right)^2 = \dfrac{209}{9 \cdot 16}$ \\ \hline
$(6,5)$ & $\left(\dfrac{1}{3}\right)^2 + 2\left(\dfrac{4}{5}\right)^2 = \dfrac{313}{9 \cdot 25}$
\end{tabular}
\end{center}

\begin{itemize}
  \item $\displaystyle \dfrac{313}{9 \cdot 25} < \dfrac{209}{9 \cdot 16}, \quad \dfrac{209}{9 \cdot 16} < \dfrac{128}{81}$
  \item $\displaystyle \dfrac{7^2 + 2 \cdot 6^4}{6^2 \cdot 7^2} > \dfrac{313}{9 \cdot 25}$
\end{itemize}

\noindent
このうち $\{f(n)\}^2 + 2\{g(m)\}^2$ がもっとも小さいのは $(n,m) = (6,5)$ の時で, この時 $E$ が最大. よって求める $(p,q)$ は $(p,q) = \left(\dfrac{1}{6}, \dfrac{1}{5}\right)$ である. \hfill $\qed$

\end{document}
