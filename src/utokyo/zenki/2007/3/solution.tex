\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

\begin{proof}[解]
\paragraph{(1)}

$P(\alpha, \alpha^2), Q(\beta, \beta^2) \quad (-1 \le \alpha \le \beta \le 1)$ とおいて良い. $R(a, b)$ とすると
\[
a = \dfrac{1}{3}(2\alpha + \beta), \quad b = \dfrac{1}{3}(2\alpha^2 + \beta^2)
\]
前者から $\beta$ を消去して $\beta = 3a - 2\alpha$ だから, 後者, 定義域に代入
\[
-1 \le 3a - 2\alpha \le 1 \quad \cdots ①
\]
\[
b = \dfrac{1}{3} \left[ 2\alpha^2 + (3a - 2\alpha)^2 \right] = 2\alpha^2 - 4a\alpha + 3a^2 \equiv f(\alpha) \quad \cdots ②
\]
以下, ① の $-1 \le \alpha \le 1$ のもとで $b$ のとりうる値を調べる.

\begin{center}
\begin{tikzpicture}[scale=2]
  \draw[->] (-1.3,0) -- (1.3,0) node[right] {$a$};
  \draw[->] (0,-1.3) -- (0,1.3) node[above] {$\alpha$};
  
  \fill[gray!20] (-1,-1) -- (-1/3,-1) -- (1,1) -- (1/3,1) -- cycle;
  \draw[thick] (-1,-1) -- (-1/3,-1) -- (1,1) -- (1/3,1) -- cycle;
  
  \draw[dashed] (-1.2,-1.2) -- (1.2,1.2) node[above right] {軸: $\alpha = a$};
  \draw[dashed] (-1, -1) -- (1/3, 1) node[above] {$\alpha = \dfrac{1}{2}(3a+1)$};
  \draw[dashed] (-1/3, -1) -- (1, 1) node[right] {$\alpha = \dfrac{1}{2}(3a-1)$};
  
  \node[below] at (-1,0) {$-1$};
  \node[below] at (-1/3,0) {$-\dfrac{1}{3}$};
  \node[below] at (1/3,0) {$\dfrac{1}{3}$};
  \node[below] at (1,0) {$1$};
  
  \node[left] at (0,-1) {$-1$};
  \node[left] at (0,1) {$1$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{center}

\[
\begin{cases}
-1 \le a \le -\dfrac{1}{3} \text{ の時 } f(a) \le b \le f(-1) \\
-\dfrac{1}{3} \le a \le 0 \text{ の時 } f(a) \le b \le f\left(\dfrac{3a-1}{2}\right) \\
0 \le a \le \dfrac{1}{3} \text{ の時 } f(a) \le b \le f\left(\dfrac{3a+1}{2}\right) \\
\dfrac{1}{3} \le a \le 1 \text{ の時 } f(a) \le b \le f(1)
\end{cases}
\]
$\iff$
\[
\begin{cases}
-1 \le a \le -\dfrac{1}{3} \text{ の時 } a^2 \le b \le 3a^2 + 4a + 2 \\
-\dfrac{1}{3} \le a \le 0 \text{ の時 } a^2 \le b \le \dfrac{3}{2}a^2 - a + \dfrac{1}{2} \\
0 \le a \le \dfrac{1}{3} \text{ の時 } a^2 \le b \le \dfrac{3}{2}a^2 + a + \dfrac{1}{2} \\
\dfrac{1}{3} \le a \le 1 \text{ の時 } a^2 \le b \le 3a^2 - 4a + 2
\end{cases}
\quad \cdots \text{\textcircled{井}}
\]

\begin{center}
\begin{tikzpicture}[scale=2.5]
  \draw[->] (-1.3,0) -- (1.3,0) node[right] {$a$};
  \draw[->] (0,-0.2) -- (0,1.3) node[above] {$b$};
  
  \fill[pattern=north west lines, pattern color=gray] 
    plot[domain=-1:-1/3, smooth] (\x, {3*\x*\x + 4*\x + 2}) --
    plot[domain=-1/3:0, smooth] (\x, {1.5*\x*\x - \x + 0.5}) --
    plot[domain=0:1/3, smooth] (\x, {1.5*\x*\x + \x + 0.5}) --
    plot[domain=1/3:1, smooth] (\x, {3*\x*\x - 4*\x + 2}) --
    plot[domain=1:-1, smooth] (\x, {\x*\x}) -- cycle;

  \draw[thick, domain=-1:1, smooth] plot (\x, {\x*\x}) node[right] {$b = a^2$};
  \draw[thick, domain=-1:-1/3, smooth] plot (\x, {3*\x*\x + 4*\x + 2});
  \draw[thick, domain=-1/3:0, smooth] plot (\x, {1.5*\x*\x - \x + 0.5});
  \draw[thick, domain=0:1/3, smooth] plot (\x, {1.5*\x*\x + \x + 0.5});
  \draw[thick, domain=1/3:1, smooth] plot (\x, {3*\x*\x - 4*\x + 2});
  
  \draw[dashed] (-1,0) -- (-1,1);
  \draw[dashed] (-1/3,0) -- (-1/3,1);
  \draw[dashed] (1/3,0) -- (1/3,1);
  \draw[dashed] (1,0) -- (1,1);
  \draw[dashed] (0,0.5) -- (0.33,0.5);
  
  \node[below] at (-1,0) {$-1$};
  \node[below] at (-2/3,0) {$-\dfrac{2}{3}$};
  \node[below] at (-1/3,0) {$-\dfrac{1}{3}$};
  \node[below] at (0,0) {$O$};
  \node[below] at (1/3,0) {$\dfrac{1}{3}$};
  \node[below] at (2/3,0) {$\dfrac{2}{3}$};
  \node[below] at (1,0) {$1$};
  
  \node[left] at (0,1) {$1$};
  \node[left] at (0,2/3) {$\dfrac{2}{3}$};
  \node[left] at (0,1/2) {$\dfrac{1}{2}$};
\end{tikzpicture}
\end{center}
\end{proof}

\end{document}
