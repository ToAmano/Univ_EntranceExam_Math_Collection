...
{\bf [解]} $0 \le \alpha < \pi/2 \quad \dots \text{①}$

$Z > 0 \quad \dots \text{②}$ から，$P$ は $A, B, C$ であることに適する．
\begin{align}
  \vec{AP} =
  \begin{pmatrix} x-1 \\ y-1 \\ Z
  \end{pmatrix}, \quad
  \vec{BP} =
  \begin{pmatrix} x-1 \\ y+1 \\ Z
  \end{pmatrix}, \quad
...
\begin{align}
  ...
  \iff
  \begin{cases}
    Z = \sqrt{(x-1)^2 + (y-1)^2 + Z^2} \cdot \dfrac{1}{\sqrt{2}} \\
    Z = \sqrt{(x-1)^2 + (y+1)^2 + Z^2} \cdot \dfrac{1}{\sqrt{2}} \\
...
⑥から $Z^2 = \Bigl( \dfrac{x}{\tan\alpha} \Bigr)^2$ だから，⑤に代入して，$t = \tan\alpha$ とおくと，
\begin{align}
  \begin{cases}
    x \neq 0 \\
  ...
  \end{cases}
\end{align}
⑦をみたす $x$ に対し，⑥から $Z$ が $Z = \Bigl|\dfrac{x}{\tan\alpha}\Bigr|$ と一意に定まり，$y=0$ だから，⑦をみたす $x$ の数が $P$ の数に等しい．$t^2 - 1 \neq 0$ の時，⑦の判別式 $D$ として
\begin{align}
...
以上まとめて，⑦の解が ($D \ge 0$ の時) $x = \dfrac{t^2 \pm \sqrt{t^2(2-t^2)}}{t^2 - 1}$ であるから
\begin{align}
  Z = \Bigl| \dfrac{x}{t} \Bigr| = \Bigl| \dfrac{t \pm \sqrt{2-t^2}}{t^2 - 1} \Bigr|
\end{align}
であることをあわせて，
\begin{align}
  \begin{cases}
    \alpha = 0 \text{ の時}，Z = \sqrt{2} \text{ の } 1 \text{ つ} \\
    0 < \tan\alpha < \sqrt{2} \land \alpha \neq \pi/4 \text{ の時}，Z = \Bigl| \dfrac{t \pm \sqrt{2-t^2}}{t^2 - 1} \Bigr| \text{ の } 2 \text{ つ} \\
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[scale=1.5, >=stealth]
    ...
    \fill (C) circle (1.5pt);
  \end{tikzpicture}
  \caption{点 $A,B,C$ と点 $P$ のなす角 $\alpha$}
\end{figure}
