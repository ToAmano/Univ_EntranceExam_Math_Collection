...
⑥から $Z^2 = \left( \dfrac{x}{\tan\alpha} \right)^2$ だから, ⑤に代入して, $t = \tan\alpha$ とおくと,
\[
\begin{cases}
x \neq 0 \\
...
\end{cases}
\]
⑦をみたす $x$ に対し, ⑥から $Z$ が $Z = \left|\dfrac{x}{\tan\alpha}\right|$ と一意に定まり, $y=0$ だから, ⑦をみたす $x$ の数が $P$ の数に等しい．$t^2 - 1 \neq 0$ の時, ⑦の判別式 $D$ として
\[
...
\[
Z = \left| \dfrac{x}{t} \right| = \left| \dfrac{t \pm \sqrt{2-t^2}}{t^2 - 1} \right|
\]
であることをあわせて,
\[
\begin{cases}
\alpha = 0 \text{ の時}, Z = \sqrt{2} \text{ の } 1 \text{ つ} \\
0 < \tan\alpha < \sqrt{2} \land \alpha \neq \pi/4 \text{ の時}, Z = \left| \dfrac{t \pm \sqrt{2-t^2}}{t^2 - 1} \right| \text{ の } 2 \text{ つ} \\
