...
\noindent
{\bf [解]} 題意を M.I. で示す．$n=1$ の時
\begin{align}
  f'(x) = \dfrac{1 - \log x}{x^2}
\end{align}
だから $a_1 = 1, b_1 = -1$ とすれば良い．$n=k$ での成立を仮定する．
\begin{align}
  f^{(k+1)}(x) &= \dfrac{b_k \cdot x^k - (k+1) \cdot x^k (a_k + b_k \log x)}{(x^{k+1})^2} \\
...
(1)から，$b_n = (-1)^n n!$ であるから，
\begin{align}
  a_{n+1} = -(n+1) a_n + (-1)^n \cdot n! \quad \cdots \textcircled{1}
\end{align}
$c_n = \dfrac{a_n}{(-1)^n n!}$ とおいて ($c_1 = -1$)，\textcircled{1}の両辺を $(-1)^{n+1} (n+1)!$ で割ると
\begin{align}
  c_{n+1} = c_n + \dfrac{1}{-(n+1)}
\end{align}
だから，階差数列より $n \ge 2$ の時
\begin{align}
  c_n &= c_1 + \sum_{k=2}^n \dfrac{1}{-k} \\
  &= -h_n
\end{align}
$n=1$ でもこれは成立するから
\begin{align}
