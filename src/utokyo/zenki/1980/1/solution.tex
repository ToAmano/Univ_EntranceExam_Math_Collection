\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

\noindent
\textbf{[解]} $0 < x < \dfrac{1}{2} \quad \cdots \text{①}$

\begin{center}
\begin{tikzpicture}[scale=2.0]
  % Triangle ABC
  \coordinate (A) at (60:3);
  \coordinate (B) at (0,0);
  \coordinate (C) at (3,0);

  % Points P, Q, R on sides
  \coordinate (P) at (0.8,0);
  \coordinate (Q) at ($(C)!0.267!(A)$);
  \coordinate (R) at ($(A)!0.267!(B)$);

  % Intersections
  \coordinate (A') at (intersection of A--P and C--R);
  \coordinate (B') at (intersection of A--P and B--Q);
  \coordinate (C') at (intersection of B--Q and C--R);

  % Draw main triangle
  \draw[thick] (A) -- (B) -- (C) -- cycle;

  % Draw lines AP, BQ, CR
  \draw (A) -- (P);
  \draw (B) -- (Q);
  \draw (C) -- (R);

  % Node labels
  \node[above] at (A) {$A$};
  \node[below left] at (B) {$B$};
  \node[below right] at (C) {$C$};

  \node[below] at (P) {$P$};
  \node[right] at (Q) {$Q$};
  \node[left] at (R) {$R$};

  \node[above right] at (A') {$A'$};
  \node[left] at (B') {$B'$};
  \node[below] at (C') {$C'$};

  % Length labels
  \node[below] at ($(B)!0.5!(P)$) {$x$};
  \node[right] at ($(C)!0.5!(Q)$) {$x$};
  \node[left] at ($(A)!0.5!(R)$) {$x$};

  \node[right] at ($(A)!0.5!(A')$) {$p$};
  \node[right] at ($(A')!0.5!(B')$) {$r$};
  \node[right] at ($(B')!0.5!(P)$) {$q$};

  \node[above] at ($(B)!0.5!(B')$) {$p$};
  \node[above] at ($(B')!0.5!(C')$) {$r$};
  \node[above] at ($(C')!0.5!(Q)$) {$q$};

  \node[left] at ($(C)!0.5!(C')$) {$p$};
  \node[left] at ($(C')!0.5!(A')$) {$r$};
  \node[left] at ($(A')!0.5!(R)$) {$q$};
\end{tikzpicture}
\end{center}

\begin{enumerate}
  \item[(1)] 対称性から $|BB'| = |CC'| = |AA'| = p$,
  $|PB'| = |QC'| = |RA'| = q$, $|AB'| = |BC'| = |CA'| = r$
  とおける。メネラウスの定理から
  \[
  \dfrac{B'C'}{BB'} \cdot \dfrac{A'R}{C'A'} \cdot \dfrac{AB}{RA} = 1 \quad \cdots \text{②}
  \]
  \[
  \dfrac{QA}{CQ} \cdot \dfrac{B'P}{AB'} \cdot \dfrac{BC}{PB} = 1 \quad \cdots \text{③}
  \]
  まず ③ から
  \[
  \dfrac{1-x}{x} \cdot \dfrac{q}{p+r} \cdot \dfrac{1}{x} = 1
  \]
  ② から
  \[
  \dfrac{r}{p} \cdot \dfrac{q}{r} \cdot \dfrac{1}{x} = 1
  \]
  したがって
  \[
  \begin{cases}
  q = xp \\
  r = \dfrac{1-2x}{x} p
  \end{cases}
  \quad \cdots \text{④}
  \]
  一方, $\triangle BCR$ に余弦定理を用いて,
  \[
  p + q + r = \sqrt{x^2 - x + 1} \quad \cdots \text{⑤}
  \]
  ④ を ⑤ に代入して,
  \[
  \left( 1 + x + \dfrac{1-2x}{x} \right) p = \sqrt{x^2 - x + 1} \quad \therefore p = \dfrac{x}{\sqrt{x^2 - x + 1}}
  \]
  ④ から
  \[
  r = \dfrac{1-2x}{\sqrt{x^2 - x + 1}}, \quad q = \dfrac{x^2}{\sqrt{x^2 - x + 1}}
  \]
  以上から, $|BB'| = p = \dfrac{x}{\sqrt{x^2 - x + 1}}$, $|PB'| = q = \dfrac{x^2}{\sqrt{x^2 - x + 1}}$

  \item[(2)] $\triangle A'B'C'$ は一辺の長さ $r$ の正三角形でその面積 $T$ は
  \[
  T = \dfrac{\sqrt{3}}{4} r^2 = \dfrac{\sqrt{3}}{4} \cdot \dfrac{4x^2 - 4x + 1}{x^2 - x + 1}
  \]
  と表せる。これが $\dfrac{1}{2} \cdot \dfrac{\sqrt{3}}{4}$ に等しいので
  \[
  \dfrac{1}{2} = \dfrac{4x^2 - 4x + 1}{x^2 - x + 1}
  \]
  \[
  8x^2 - 8x + 2 = x^2 - x + 1
  \]
  \[
  7x^2 - 7x + 1 = 0
  \]
  \[
  \therefore x = \dfrac{7 \pm \sqrt{21}}{14}
  \]
  $0 < x < \dfrac{1}{2}$ から複号負を採用して,
  \[
  x = \dfrac{7 - \sqrt{21}}{14}
  \]
\end{enumerate}

\end{document}
