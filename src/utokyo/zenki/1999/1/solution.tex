...
$xy$ 座標平面上で，$x$ 軸の正の部分 (原点 $O$ を端点とする半直線) を始線とし，一般角 $\theta$ に対する動径上で $O$ からの距離 $r\, (r>0)$ の点 $P$ の座標を $(x,y)$ とする．そこで $\sin\theta = \dfrac{y}{r}, \cos\theta = \dfrac{x}{r}$ と定義する．
...
2次元ベクトル $\vec{t}_\theta = \begin{pmatrix} \cos\theta \\ \sin\theta \end{pmatrix}$ とする．
...
とすると，(1)から，
\begin{align}
\vec{p} &= \begin{pmatrix} \cos\alpha \\ \sin\alpha \end{pmatrix}, \\
\vec{q} &= \begin{pmatrix} -\sin\alpha \\ \cos\alpha \end{pmatrix} \quad \cdots \textcircled{1}
\end{align}
...
\end{tikzpicture}
\caption{正規直交基底を$\alpha$回転した直交基底$\vec{p},\vec{q}$}
\end{figure}
...
したがって，
\begin{align}
\sin(\alpha+\beta) &= \cos\left( \dfrac{\pi}{2} - (\alpha+\beta) \right) \hfill (*) \\
...
\begin{itemize}
  ...
  \item[(II)] $\sin\left(\dfrac{\pi}{2} - \alpha\right) = \cos\alpha, \quad \cos\left(\dfrac{\pi}{2} - \alpha\right) = \sin\alpha$ \\
  点 $\left(\cos\left(\dfrac{\pi}{2}-\alpha\right), \sin\left(\dfrac{\pi}{2}-\alpha\right)\right)$ は点 $(\cos\alpha, \sin\alpha)$ と $x=y$ 対称．
\end{itemize}

\bigskip

\bigskip\noindent{\bf [解2]} (2) $r=1$ として考える．右図で $\vec{OP} = \vec{t}_{\alpha+\beta}$ である．
...
\begin{tikzpicture}[scale=1.5, >=stealth]
  ...
  \node at (-15:0.45) {$-\beta$};
\end{tikzpicture}
\caption{左:点$A,P$の配置と$\alpha+\beta$．右:点$Q,R$の配置と$\alpha,-\beta$}
\end{figure}
...
&= 2 - 2\cos(\alpha+\beta) \quad \cdots \textcircled{1}
\end{align}
一方，右図で $\vec{OQ} = \vec{t}_\alpha, \vec{OR} = \vec{t}_{-\beta}$ である．
\begin{align}
|QR|^2 &= \{\cos\alpha - \cos(-\beta)\}^2 + \{\sin\alpha - \sin(-\beta)\}^2 \\
&= 2 - 2(\cos\alpha \cos\beta - \sin\alpha \sin\beta) \quad \cdots \textcircled{2} \hfill (*)
\end{align}
