...
となる．ゆえに$a>0,b>0,c>1$から
     \begin{align}
     A\left(\dfrac{c}{a},0,0\right) , B\left(0,\dfrac{c}{b},0\right) , C\left(0,0,\dfrac{c}{c-1}\right)
     \end{align}     
である．そこで
     \begin{align}
     p=\dfrac{c}{a} , q=\dfrac{c}{b} , r=\dfrac{c}{c-1} \label{2}
     \end{align}
とおけば     
     \begin{align}
...
である．よって三角形$ABC$の面積$S$は
     \begin{align}
     S&=\dfrac{1}{2}\sqrt{|\vector{CA}|^2|\vector{CB}|^2-(\vector{CA}\cdot\vector{CB})^2}    \\
     &=\dfrac{1}{2}\sqrt{(p^2+r^2)(q^2+r^2)-r^4} \\
     &=\dfrac{1}{2}\sqrt{(pqr)^2(\dfrac{1}{p^2}+\dfrac{1}{q^2}+\dfrac{1}{r^2})}     \\
     &=\dfrac{1}{2}\dfrac{pqr}{c} \tag{$\because\eqref{1},\eqref{2}$}
     \end{align}
...
である．等号成立は$a=b$の時．したがって$c-1>0$に注意して
     \begin{align}
     S&=\dfrac{c^2}{ab(c-1)} \nonumber\\
     &\ge \dfrac{1}{2c-c^2}\dfrac{c^2}{c-1} \nonumber\\
     &=\dfrac{1}{3-(c+2/c)} \nonumber\\
     &\ge \dfrac{1}{3-2\sqrt{2}}\tag{$\because$AM-GM} \\
     &=3+2\sqrt{2} \label{3}
     \end{align}
