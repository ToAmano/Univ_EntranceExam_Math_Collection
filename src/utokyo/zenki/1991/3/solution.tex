...
\noindent
{\bf [解]} $g(x) = x^3 - 3x$ とおく．$g'(x) = 3(x+1)(x-1)$ から，下表を得る．
...
\end{tikzpicture}
\caption{$y=g(x)=x^3-3x$ のグラフ}
\end{figure}

\noindent
\paragraph{(1)}

$|p| > 2$ の時．図から解はただ 1 つあり，これを $\alpha$ として $f(p) = \alpha^2$ となる．$\alpha^2$ は $|p|$ の単調増加関数であるから，$|p| \to 2$ をかんがえて，
\begin{align}
f(p) > 4 \quad \dots \text{①}
\end{align}
...
$y = g(x)$ が奇関数だから，$y = f(p)$ は偶関数である．よって $0 \le p$ でかんがえる．
...
\begin{align}
f'(p) = 2t \dfrac{dt}{dp} = 2 h(p) \cdot h'(p) > 0 \quad \dots \text{④}
\end{align}
又，$*$ から $s$ を消して
\begin{align}
p = -t(t^2 - 3)
...
だから両辺 $p$ で微分して
\begin{align}
1 = -3t^2 \cdot t' + 3t' \quad \therefore \dfrac{1}{3} = (1 - t^2) \cdot t'
\end{align}
もう1回 $p$ で微分して
\begin{align}
...
&= 2 \Bigl\{ (\alpha')^2 + \alpha \dfrac{-2\alpha(\alpha')^2}{\alpha^2 - 1} \Bigr\} \quad (\because \text{⑥}) \\
&= 2(\alpha')^2 \dfrac{-(\alpha^2 + 1)}{\alpha^2 - 1} < 0
\end{align}
