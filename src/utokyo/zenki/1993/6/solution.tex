...
$f'(t) = -2S - 2\sin 2t$, $g'(t) = 2\cos 2t$ だから，$\vec{v} = \begin{pmatrix} f'(t) \\ g'(t) \end{pmatrix}$ とすると，
\begin{align}
...
$C$ & $-1$ & $\dots$ & $-\dfrac{2}{3}$ & $\dots$ & $\dfrac{1}{2}$ & $\dots$ & $1$ \\ \hline
$h'$ & & $-$ & $0$ & $+$ & $0$ & $-$ & \\ \hline
$h$ & $4$ & $\searrow$ & $\dfrac{8}{27}$ & $\nearrow$ & $13$ & $\searrow$ & $4$
\end{tabular}
\caption{$h(C)$ の増減表}
\end{table}

したがって $|\vec{v}| \ge 0$ より，$\max |\vec{v}| = \sqrt{13}$, $\min |\vec{v}| = \sqrt{\dfrac{8}{27}}$ \quad \text{\#\#}

\bigskip
\noindent
\paragraph{(2)}

$f'(t) = -2S(1+2C)$, $g'(t) = 2\cos 2t$ から下表を得る．($f(\pi+t) = f(\pi-t), g(\pi+t) = -g(\pi-t)$)

\begin{table}[htb]
\centering
\begin{tabular}{c|ccccccccc}
$t$ & $0$ & $\dots$ & $\pi/4$ & $\dots$ & $\dfrac{2}{3}\pi$ & $\dots$ & $\dfrac{3}{4}\pi$ & $\dots$ & $\pi$ \\ \hline
$f'$ & $0$ & $-$ & $-$ & $-$ & $0$ & $+$ & $+$ & $+$ & $0$ \\ \hline
$g'$ & $+$ & $+$ & $0$ & $-$ & $-$ & $-$ & $0$ & $+$ & $+$ \\ \hline
$(x,y)$ & $(3,0)$ & $\searrow$ & $(\sqrt{2},1)$ & $\swarrow$ & & $\nwarrow$ & & $\nearrow$ & $(-1,0)$
\end{tabular}
\caption{$(x,y)=(f(t),g(t))$ の増減表}
\end{table}
...
\end{cases}
\end{align}
$\dfrac{\pi}{2} < t_1 < t_2 < \pi$ 及び ② から，$t_2 = \dfrac{3}{2}\pi - t_1$ である．この時 ① から
\begin{align}
& 2\cos t_2 + \cos 2t_2 = -2\cos \Bigl(\dfrac{\pi}{2} - t_1\Bigr) - \cos 2t_1 = 2\cos t_1 + \cos 2t_1 \\
...
\end{align}
一方，$\dfrac{\pi}{2} < t_1 < t_2 < \pi$ だったから $t_2 = \dfrac{3}{2}\pi - t_1$ から
\begin{align}
\dfrac{\pi}{2} < t_1 < \dfrac{3}{4}\pi
\end{align}
だから $-\dfrac{\sqrt{2}}{2} < P < 0$ となる．③ を同時にみたす $P$ はなく矛盾．

\bigskip
\noindent
以上から，交点はただ1つ存在し，同時刻 $t = \dfrac{\pi}{2}, \pi, \dfrac{3}{2}\pi$ での速度ベクトルは各々
\begin{align}
...
\end{tikzpicture}
\caption{交点 $(-1,0)$ における各時刻の速度ベクトル}
\end{figure}

\bigskip
\noindent{\bf [解2] (交点の唯一性)}\\
②から $t_1, t_2 = \dfrac{3}{4}\pi \pm \alpha$ ($0 < \alpha < \dfrac{3}{4}\pi$) とおける．$f(t) = 2C^2 + 2C - 1$ だから
\begin{align}
f\Bigl(\dfrac{3}{4}\pi \pm \alpha\Bigr) = -\sqrt{2}\cos\alpha \pm (1-\sqrt{2})\sin\alpha - 1
\end{align}
