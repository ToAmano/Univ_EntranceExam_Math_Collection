...
$\longrightarrow$ 漸近線 $\dfrac{x}{a} \pm \dfrac{y}{\sqrt{c^2-a^2}} = 0$
...
\noindent
{\bf [解]} $R(x,y)$ とおく．下図のように，$xy$ 平面を分割する．ただし $\square PTQS$ は正方形．
...
\begin{tikzpicture}[scale=0.35, >=stealth]
  ...
  \node at (9,-5) {$B$};
\end{tikzpicture}
\caption{$xy$ 平面の分割（正方形 $PTQS$ と点 $A,B,C$ の領域）}
\end{figure}
...
\begin{align}
\begin{cases}
d(P,R) = \min \{ |PL_3| + |L_3R|, |PL_4| + |L_4R| \} \\
d(Q,R) = \min \{ |QL_1| + |L_1R|, |QL_2| + |L_2R| \}
\end{cases}
\quad \dots \text{②}
\end{align}
である．$|PL_3| = |QL_1| = 2\sqrt{5}$, $|PL_4| = |QL_2| = 4\sqrt{5}$ \dots ③ だから，
\begin{align}
|PL_3| + |L_3R| = |PL_4| + |L_4R| &\iff |L_3R| - |L_4R| = 2\sqrt{5} \quad \dots \text{④} \\
|QL_1| + |L_1R| = |QL_2| + |L_2R| &\iff |L_1R| - |L_2R| = 2\sqrt{5} \quad \dots \text{⑤}
\end{align}
...
\begin{tikzpicture}[scale=0.35, >=stealth]
  ...
  \draw[thick, domain=-5:5, samples=50] plot (\x, {-sqrt(5*(1 + (\x-5)^2/20))});
\end{tikzpicture}
\caption{$C$ 内の領域 $C_1, C_2, C_3$ への分割}
\end{figure}

\begin{align}
\begin{cases}
C_1 : d(P,R) = |PL_4| + |L_4R|, \quad d(Q,R) = |QL_1| + |L_1R| \\
C_2 : d(P,R) = |PL_3| + |L_3R|, \quad d(Q,R) = |QL_2| + |L_2R| \\
C_3 : d(P,R) = |PL_3| + |L_3R|, \quad d(Q,R) = |QL_1| + |L_1R|
\end{cases}
\end{align}
境界は
\begin{align}
\dfrac{(x+5)^2}{20} - \dfrac{y^2}{5} = -1, \quad \dfrac{(x-5)^2}{20} - \dfrac{y^2}{5} = -1
\end{align}

まず $C_1, C_2$ について，対称性から $C_1$ のみ考える．$C_1$ から
\begin{align}
d(P,R) = d(Q,R) \iff |PL_4| + |L_4R| = |QL_1| + |L_1R| \iff |L_1R| - |L_4R| = 2\sqrt{5}
\end{align}
を満たす $R$ は，双曲線
\begin{align}
\dfrac{x^2}{5} - \dfrac{(y-5)^2}{20} = 1
\end{align}
上にある．

$C_3$ について，④から，
\begin{align}
d(P,R) = d(Q,R) \iff |PL_3| + |L_3R| = |QL_1| + |L_1R| \iff |L_3R| = |L_1R|
\end{align}
だから，$R$ は $|L_1L_3|$ の垂直二等分線 $y = -x$ 上にある．
...
\begin{cases}
y = -3x & (|x| \ge 3) \\
y = -x & \Bigl( |x| \le \dfrac{-5+4\sqrt{10}}{3} \Bigr) \\
\dfrac{x^2}{5} - \dfrac{(y-5)^2}{20} = 1 & (C_1 \text{内}) \\
\dfrac{x^2}{5} - \dfrac{(y+5)^2}{20} = 1 & (C_2 \text{内})
\end{cases}
\end{align}
だから，図示して下図太線部．
...
\begin{framed}
\begin{align}
&y = -x \text{ と } \dfrac{x^2}{5} - \dfrac{(y-5)^2}{20} = 1 \text{ の交点は } 3x^2 - 10x - 45 = 0 \text{ の解} \\
&\text{共に } \Bigl( \dfrac{5-4\sqrt{10}}{3}, \dfrac{-5+4\sqrt{10}}{3} \Bigr) \text{ である．}
\end{align}
\end{framed}
...
\end{tikzpicture}
\caption{$d(P,R)=d(Q,R)$ を満たす点 $R$ の軌跡（太線部）}
\end{figure}
