\documentclass[../../main.tex]{subfiles}
\begin{document}

\begin{flushright}
\footnotesize\textit{【自動文字起こし・要確認】}
\end{flushright}

[解] $t = e^x$ とおく. $x > 0$ から $t > 1$ である.

\paragraph{(1)}
\[ f'(x) = \dfrac{dt}{dx} \dfrac{df}{dt} = t \cdot \left\{ \dfrac{12(t^2 - 3t)}{(t^2 - 1)} \right\}' = 12t \cdot \dfrac{t^4 + 3}{(t^2 - 1)^2} > 0 \quad (\because t > 1) \]
だから $f(x)$は単調増加. これと $f(x) = 12 \dfrac{t - 3/t}{1 - 1/t^2} \xrightarrow{x \to \infty} \infty \quad (\because t \to \infty)$,
$f(x) = 12 \dfrac{t(t^2 - 3)}{(t+1)(t-1)} \xrightarrow{x \to +0} -\infty \quad (\because t \to 1+0)$ から, 任意の定数 $a$ に対し $f(x) = a$ なる $x > 0$ がただ1つある. (終)

\paragraph{(2)}

グラフの概形は右図で求める面積 $S$ は斜線部.
\begin{align}
f(x) = 8 &\iff 3t^3 - 2t^2 - 9t + 2 = 0 \\
&\iff (t - 2)(3t^2 + 4t - 1) = 0 \\
&\therefore t = 2 \quad (\because t > 1) \\
f(x) = 27 &\iff 4t^3 - 9t^2 - 12t + 9 = 0 \\
&\iff (t - 3)(4t^2 + 3t - 3) = 0 \\
&\therefore t = 3 \quad (\because t > 1)
\end{align}
だから,
\[ S = 27 \log 3 - 8 \log 2 - \int_{\log 2}^{\log 3} f(x) dx \quad \cdots \text{①} \]
となる. ここで,
\begin{align}
\int_{\log 2}^{\log 3} f(x) dx &= \int_2^3 \dfrac{12(t^3 - 3t)}{t^2 - 1} \dfrac{1}{t} dt \\
&= 12 \int_2^3 \dfrac{t^2 - 3}{t^2 - 1} dt = 12 \int_2^3 \left( 1 - \dfrac{1}{t - 1} + \dfrac{1}{t + 1} \right) dt \\
&= 12 \Big[ t - \log(t-1) + \log(t+1) \Big]_2^3 \\
&= 12(1 + \log 2 - \log 3) \quad \cdots \text{②}
\end{align}
だから, ②を①に代入して
\[ S = 39 \log 3 - 20 \log 2 - 12 \quad (\text{終}) \]

[解2]
\paragraph{(2)}

$S = \int_8^{27} g(x) dx$ とおく. $x = f(y)$ なる置換をして,
\begin{align}
S &= \int_{\log 2}^{\log 3} y \cdot \dfrac{df(y)}{dy} dy \\
&= \int_{\log 2}^{\log 3} y \cdot f'(y) dy \\
&= \Big[ y f(y) \Big]_{\log 2}^{\log 3} - \int_{\log 2}^{\log 3} f(y) dy \\
&= 27 \log 3 - 8 \log 2 - \int_{\log 2}^{\log 3} f(y) dy
\end{align}
(以下同文)

\begin{center}
\begin{tikzpicture}
\draw[->] (0,0) -- (4,0) node[right]{$x$};
\draw[->] (0,-1) -- (0,4) node[above]{$y$};
\draw[domain=1.5:3.5, smooth] plot (\x, {\x*1.2 - 1}) node[right]{$y=f(x)$};
\draw[dashed] (0,1) node[left]{$8$} -- (1.66,1) -- (1.66,0) node[below]{$\log 2$};
\draw[dashed] (0,3) node[left]{$27$} -- (3.33,3) -- (3.33,0) node[below]{$\log 3$};
\node at (1,0) [below] {$(\dfrac{1}{2}\log 3)$};
\fill[pattern=north east lines] (0,1) -- (1.66,1) -- (3.33,3) -- (0,3) -- cycle;
\end{tikzpicture}
\end{center}

\end{document}
