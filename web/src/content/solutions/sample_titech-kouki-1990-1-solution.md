...
実数 $`x`$ に対して $`f$x$ = $x+1$$x-2$`$, $`g$x$ = 1+5x`$
とおく．題意から，
``` math
...
```
をみたす $`x \in \mathbb{R}`$ をもとめればよい． $`g$x$ \in \mathbb{Z}`$
から $`5x \in \mathbb{Z}`$．つまり，$`x = \frac{t}{5}`$
$$`t \in \mathbb{Z}`$$
とかける．<a href="#$式1, 式1, 式1$\left$\frac{t}{5}-2\right$ < t+\frac{3}{2}  \\
...
のグラフの概形は<a href="#1990-1:fig:1" data-reference-type="ref+label"
data-reference="1990-1:fig:1">1</a>のようになっており，
``` math
...
本問題の解答である$`x = \frac{32}{5}, -\frac{2}{5}`$
