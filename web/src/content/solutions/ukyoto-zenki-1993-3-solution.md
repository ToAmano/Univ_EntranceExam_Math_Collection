...
question: "3"
type: "solution"
title: "UKYOTO 1993 zenki Q3 (solution)"
---
...
$$
\begin{align*}
(\text{左辺}) = F(x+1)-F(x) &= a_n(x+1)^n + a_{n-1}(x+1)^{n-1} - a_n x^n - a_{n-1} x^{n-1} + \cdots\\&= n \cdot a_n x^{n-1} + \{(n-1)a_{n-1} + {}_n\text{C}_2 \cdot a_n \} x^{n-2} + \cdots\\(\text{右辺}) = c F'(x) &= c n a_n x^{n-1} + (n-1) \cdot c a_{n-1} x^{n-2} + \cdots
\end{align*}
$$
