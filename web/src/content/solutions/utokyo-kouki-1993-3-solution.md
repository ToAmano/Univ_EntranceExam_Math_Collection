...
question: "3"
type: "solution"
title: "UTOKYO 1993 kouki Q3 (solution)"
---
...
<figure id="1993-3:fig:1">
  <img src="/Univ_EntranceExam_Math_Collection/images/tikz/utokyo/kouki/1993/3/fig_1.svg" alt="図 1" />
...
<figure id="1993-3:fig:2">
  <img src="/Univ_EntranceExam_Math_Collection/images/tikz/utokyo/kouki/1993/3/fig_2.svg" alt="図 2" />
...
<figure id="1993-3:fig:3">
  <img src="/Univ_EntranceExam_Math_Collection/images/tikz/utokyo/kouki/1993/3/fig_3.svg" alt="図 3" />
...
$$
\begin{align}
& \begin{pmatrix} 1 \\ 2u \end{pmatrix}\cdot\vec{QP} = 0                                   \\\Leftrightarrow& \begin{pmatrix} 1 \\ 2u \end{pmatrix}\cdot\begin{pmatrix} 0-u \\ t-u^2 \end{pmatrix} = 0 \\& \Leftrightarrow
...
$$
\begin{align}
& u^2+(u^2-t)^2=r^2                            \\\therefore& u^4+(1-2t)u^2+t^2-r^2=0  \label{1993-3:eq:3}
\end{align}
$$

  である．対称性から円は放物線と$(u,u^2), (-u,u^2)$のみで共通点を持たないといけないから，
  [(式3)](#1993-3:eq:3)が実数解として重解のみを持てば良い．

  
  \textbf{$1^{\circ}$ $0<t \le \frac{1}{2}$ の時}

  [(式2)](#1993-3:eq:2)を満たす$u$は$u=0$のみである．
  [(式3)](#1993-3:eq:3)に代入して
...
$$
  ...
  $u=0$の時は[(式3)](#1993-3:eq:3)が成立するためには$r=t$が必要だが，
  この時[(式3)](#1993-3:eq:3)は
...
$$
\begin{align}
& \sqrt{t-\frac{1}{2}}\le 2          \\\therefore& t - \frac{1}{2}\le 4               \\\therefore& t \le\frac{9}{2}\label{1993-eq:5}
\end{align}
$$
...
$$
\begin{align*}
& \left(t-\frac{1}{2}\right)^2 + (1-2t)\left(t-\frac{1}{2}\right) + t^2 - r^2 = 0 \\\therefore& -\left(t^2-t+\frac{1}{4}\right) + t^2 - r^2 = 0                                 \\\therefore& t = r^2 + \frac{1}{4}
\end{align*}
$$

  であることが必要である．この時[(式5)](#1993-eq:5)より
  

$$
\begin{align*}
& \frac{1}{2}\le t \le\frac{9}{2}\\& \frac{1}{2}\le  r^2 + \frac{1}{4}\le\frac{9}{2}\\& \frac{1}{4}\le  r^2  \le\frac{17}{4}\\& \frac{1}{2}\le  r  \le\frac{\sqrt{17}}{2}
\end{align*}
$$

  である．

  逆にこの時，[(式3)](#1993-3:eq:3)は
  

$$
\begin{align*}
& u^4 + (1-2t)u^2 + \left(t^2-t + \frac{1}{4}\right) = 0 \\\therefore& \left[u^2-\left(t-\frac{1}{2}\right)\right]^2 = 0
\end{align*}
$$
...
\begin{align}
s
     & = 4 - t                                                                                                           \\& = 4 - \left(r^2 + \frac{1}{4}\right)\\& = \frac{15}{4} - r^2                 & \left(\frac{1}{2}\le  r  \le\frac{\sqrt{17}}{2}\right)\label{1993-eq:6}
\end{align}
$$
  ...
  この時[(式5)](#1993-eq:5)より，円の中心の$y$座標が
...
$$
\begin{align*}
2^2 + (4-t)^2 = r^2 \\\therefore(4-t)^2 = r^2 -4
\end{align*}
$$

  である．$t$の条件[(式7)](#1993-eq:7)から$r$の条件は
...
$$
  ...
  この時円と放物線を連立した[(式3)](#1993-3:eq:3)は
  

$$
\begin{align*}
& u^4 + (1-2t)u^2 + t^2 -4 - (4-t)^2 = 0 \\\therefore& u^4 + (1-2t)u^2 + 8t -20 = 0           \\\therefore& (u^2-4)\left(u^2 - (2t-5)\right) = 0
\end{align*}
$$
...
\begin{align}
s
     & = 4 - t                                                                      \\& = - \sqrt{r^2-4}& \left(r \ge\frac{\sqrt{17}}{2}\right)\label{1993-eq:8}
\end{align}
$$

  以上三つの場合わけ[(式8)](#1993-3:eq:4,1993-3:eq:6,1993-3:eq:8)をまとめて，$s$を$r$の関数として表すと
...
<figure id="1993-3:fig:4">
  <img src="/Univ_EntranceExam_Math_Collection/images/tikz/utokyo/kouki/1993/3/fig_4.svg" alt="図 4" />
...
\begin{align*}
V(r)
     & = \pi\int_{t-r}^{4}\left[r^2-(y-t)^2\right] dy        \\& = \pi\int_{-r}^{s}\left[r^2-y^2\right] dy             \\& = \pi\left[\frac{2}{3}r^3+r^2s-\frac{1}{3}s^3 \right]
\end{align*}
$$
...
\begin{align*}
V'(r)
     & = \pi\left[ 2r(r+s)+s'(r^2-s^2) \right]\\& = \pi\left[ 2r(r+s)+s'(r+s)(r-s) \right]\\& = \pi\left(r+s\right)\left[ 2r+s'(r-s) \right]\\& = \begin{dcases}
