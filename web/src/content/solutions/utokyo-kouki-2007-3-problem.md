...
question: "3"
type: "problem"
title: "UTOKYO 2007 kouki Q3 (problem)"
---

$N$を$2$以上の自然数とする．$x_1\le \cdots \le x_N$をみたす実数$x_1,\cdots,x_N$に対し実数$k_n$，$p_n$，$q_n\, (n=0,1,2,\cdots)$を次の手続きで定める．

\begin{itemize}
\item $k_0=1$，$p_0=x_1$，$q_0=x_N$
  \item $n$が奇数のとき$k_n$は$x_i\le \frac{p_{n-1}+q_{n-1}}{2}$をみたす$x_i$の個数，$p_n=p_{n-1}$，$q_n=q_{n-1}$
  \item $n$が偶数$(n\ge 2)$のとき$k_n=k_{n-1}$，$p_n=\frac{1}{k_n}\sum_{i=1}^{k_n}x_i$，$q_n=\frac{1}{N-k_n}\sum_{i=k_n+1}^{N}x_i$．
\end{itemize}
