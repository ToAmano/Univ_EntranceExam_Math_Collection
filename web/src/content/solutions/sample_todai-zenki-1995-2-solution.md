...
<img
src="/Math-Solutions/images/tikz/sample_todai-zenki-1995-2_tikz_1.svg" />
<figcaption>点ABCの様子</figcaption>
</figure>

$\mathrm{PQ}=1$ だから， $\mathrm{P}(0,0)$, $\mathrm{Q}(1,0)$ とし,
$\triangle \mathrm{ABC}$ が $xy$ 平面にあるとする.
(条件)をみたす時,$A,B,C$ は[1](#1995-2:fig:1){reference-type="ref+label"
reference="1995-2:fig:1"}の斜線部分内にある.

もし $A,B,C$ が[1](#1995-2:fig:1){reference-type="ref+label"
reference="1995-2:fig:1"}で斜線部で, 境界を含まない部分にあるとすると
...
上を動く. この時三角形PAHを考えると $$\begin{align*}
    \overline{AB}
     & = 2\overline{AH} \\
     & = 2\sqrt{1-p^2}
\end{align*}$$ で一定である．
...
\end{align}$$ である． このような点はE上に2点,
F上に2点あるので，E上のものを$X_1, X_2$，F上のものを$X_3, X_4$とすると
$X_1,X_2$について $$\begin{align}
    \begin{dcases}
      \overline{HX_1} = 1 + p \\
      \overline{HX_2} = 1 - p
    \end{dcases} \label{1995-2:eq:2}
\end{align}$$
であり，$X_3, X_4$については[\[1995-2:eq:1\]](#1995-2:eq:1){reference-type="ref+label"
reference="1995-2:eq:1"}より $$\begin{align*}
     & X_3(1+\cos\theta, \sin\theta) \\
     & X_4(1-\cos\theta, \sin\theta)
\end{align*}$$ として良い． $X_3,X_4$と線分ABの距離を$L_3,L_4$とすると
$$\begin{align}
    \begin{dcases}
...
\end{align}$$
である．$0 \le p \le 1$および$-1 \le \cos\theta \le 1$から，[\[1995-2:eq:2,1995-2:eq:3\]](#1995-2:eq:2,1995-2:eq:3){reference-type="ref+label"
reference="1995-2:eq:2,1995-2:eq:3"}のうちで最大のものは
$L_4$において$\cos\theta = -1$として時で $$\begin{align*}
    \max L_4 = (p+2)
\end{align*}$$ である．
...
[3](#1995-2:fig:3){reference-type="ref+label"
reference="1995-2:fig:3"}のようにHが$x$軸上に存在する時である．

<figure id="1995-2:fig:3" data-latex-placement="H">
<img
src="/Math-Solutions/images/tikz/sample_todai-zenki-1995-2_tikz_3.svg" />
<figcaption><span
class="math inline">\(p\)</span>を固定した時，三角形ABCの面積が最大になるような場合．</figcaption>
</figure>

この時の面積$S(p)$として， $$\begin{align*}
    S(p)
     & = \frac{1}{2} \overline{AB}\cdot \overline{HC} \\
     & = \frac{1}{2} \cdot 2 \sqrt{1-p^2} \cdot (p+2) \\
     & = \sqrt{(1-p^2)(p+2)^2}                        \\
     & \equiv \sqrt{f(p)}
\end{align*}$$ である． 以下この最大値を求める．
$f(p)$が最大の時$S(p)$も最大だから，$f(p)$の増減表を書く． 一階微分は
$$\begin{align*}
    f'(p)
     & = 2(p+2)(1-p^2) - 2p(p+2)^2 \\
     & = 2(p+2)(1-2p-2p^2)
\end{align*}$$ より，[1](#1995-2:table:1){reference-type="ref+label"
reference="1995-2:table:1"}を得る．
