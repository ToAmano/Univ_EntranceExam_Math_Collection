...
3.  $1\le k\le n$のとき，$k$回目に出た目の数が$1,2,3,4$のときには，$P_{k-1}$をそれぞれ
...
2.  $P_n$が$x$軸上にあれば$P_0$，$P_1$，$\dots$，$P_{n-1}$もすべて$x$軸上にあること
     を示せ．

3.  $P_n$が第一象限$\{(x,y)|x>0,y>0\}$にある確率を$n$で示せ．
...
2.  $l$回目$(a\le l\le n-1,l\in\mathbb{N})$にはじめて南北方向に動いたとすると，移動量は
...
3.  $P_n$が$x$軸上にあるという事象を$X$，$y$軸上にあるという事象を$Y$とする．
