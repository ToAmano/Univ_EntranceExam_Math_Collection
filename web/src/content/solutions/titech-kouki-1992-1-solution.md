...
question: "1"
type: "solution"
title: "TITECH 1992 kouki Q1 (solution)"
---
...
\begin{align*}
\int_{0}^{1} f(t) dt
     & = \int_{0}^{1}\left(1-\frac{x+1}{t+1}\right) dt \\& = \left[t - (x+1)\log(t+1)\right]_{0}^{1}\\& = 1 - (x+1)\log 2
\end{align*}
$$

  だから，$\displaystyle\int_{0}^{1} f(t) dt$は$x$の単調減少関数であり，$\eqref{1992-1:eq:1}$より$F(x)$は$0\le x \le 1$で最小値を取る．
  以下この場合を調べる．
...
\begin{align}
F(x)
     & = [-G(t,x)]_{0}^{x} + [G(t,x)]_{x}^{1}\nonumber\\& = G(0,x) + G(1,x) - 2G(x,x) \label{1992-1:eq:3}
\end{align}
$$
...
$$
  ...
  を得る．これを$\eqref{1992-1:eq:3}$に代入して
  
$$
\begin{align}
F(x)
     & = \left[1-(x+1)\log 2\right] -2 \left[x-(x+1)\log(x+1)\right]\nonumber\\& = \left[1-(x+1)\log 2\right] -2 \left[x-(x+1)\log(x+1)\right]\nonumber\\& = (x+1) \log\frac{\left(x+1\right)^2}{2} -2x + 1 \label{1992-1:eq:6}
\end{align}
$$
...
\begin{align*}
F'(x)
     & = \log\frac{\left(x+1\right)^2}{2} + 2 -2 \\& = \log\frac{\left(x+1\right)^2}{2}
\end{align*}
$$

  を得る．従って$F(x)$ の増減表は[表1](#1992-1:table:1)となる．
  

<figure id="1992-1:table:1" class="table-wrapper">
...
| $F(x)$  |       | $\searrow$ |      最小      | $\nearrow$ |       |
  ...
  したがって、$F(x)$ は $x=\sqrt{2}-1$ で最小値をとる．この時の最小値は$\eqref{1992-1:eq:6}$に代入して
  
$$
\begin{align*}
\min F(x)
     & = F(\sqrt{2}-1)      \\& = -2(\sqrt{2}-1) + 1 \\& = 3 - 2\sqrt{2}
\end{align*}
$$
...
$$
  ...
  ただし積分区間内で絶対値の中身は一回だけ符号が入れ替わるとする．
...
\begin{align*}
I'(x)
     & = 2f(\alpha)\alpha' - (2\alpha + 2\alpha' x) + a +b                                 \\& = -2\alpha + a + b                                                                  \\
    J'(x)
     & = 2f(\beta)\beta'   - (2\beta\beta' x + 2\beta^2) + \frac{1}{2}a^2 + \frac{1}{2}b^2 \\& = -2\beta^2 + \frac{1}{2}a^2 + \frac{1}{2}b^2
\end{align*}
$$
...
$$
\begin{align}
\alpha& = \frac{a+b}{2}\\\beta& = \frac{\sqrt{a^2+b^2}}{2}
\end{align}
$$
...
\begin{align}
f_{x}(t)
     & = \frac{t}{t+1} - \frac{x}{t+1}\\& = p(t) - q(t) x
\end{align}
$$
...
\begin{align}
q(t) & = \frac{1}{t+1}\\
    p(t) & = \frac{t}{t+1} = tq(t) \label{1992-1:eq:4}
\end{align}
$$

  とおいた．このように$f$を$t$に依存する部分と$x$に依存する部分に分離して書けば，
  $\eqref{1992-1:eq:3}$の$F(x)$は$p$および$q$の原始関数$P$，$Q$を用いて
  
$$
\begin{align}
F(x) =
     & \left[P(0)-Q(0)x \right] + \left[P(1)-Q(1)x \right]\\& -2 \left[P(x)-Q(x)x \right]\label{1992-1:eq:7}
\end{align}
$$
...
\begin{align*}
F'(x)
     & = -Q(0) - Q(1) -2 \left[P'(x)-Q'(x)x -Q(x)\right]\\& = -Q(0) - Q(1) -2 \left[p(x)-q(x)x -Q(x)\right]\\& = -Q(0) - Q(1) -2 \left[q(x)x-q(x)x -Q(x)\right]\\& = -Q(0) - Q(1) + 2Q(x)
\end{align*}
$$

  を得る．ただし，三行目で$p$と$q$の関係$p(x)=xq(x)$ ($\eqref{1992-1:eq:4}$)を利用した．
  ここから，$F$が極小となるには$F'(x)=0$すなわち
  
$$
\begin{align}
Q(x) = \frac{Q(0)+Q(1)}{2}\label{1992-1:eq:5}
\end{align}
$$
  ...
  また，$\eqref{1992-1:eq:5}$までわかれば$Q(x) = \log (x+1)$から容易に$x=\sqrt{2}-1$を導くことができる．
  この時の最小値$F(x)$は，$\eqref{1992-1:eq:7}$に$\eqref{1992-1:eq:5}$を代入すれば$Q$に関する項が全て消えて
