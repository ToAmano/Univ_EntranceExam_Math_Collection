...
$$
  ...
  だから，$\displaystyle\int_{0}^{1} f(t) dt$は$x$の単調減少関数であり，[(式1)](#1992-1:eq:1)より$F(x)$は$0\le x \le 1$で最小値を取る．
  以下この場合を調べる．
...
| $F(x)$  |       | $\searrow$ |      最小      | $\nearrow$ |       |
  ...
  したがって、$F(x)$ は $x=\sqrt{2}-1$ で最小値をとる．この時の最小値は[(式6)](#1992-1:eq:6)に代入して
...
$$
  ...
  [(式3)](#1992-1:eq:3)の$F(x)$は$p$および$q$の原始関数$P$，$Q$を用いて
...
$$
  ...
  を得る．ただし，三行目で$p$と$q$の関係$p(x)=xq(x)$ ([(式4)](#1992-1:eq:4))を利用した．
...
$$
  ...
  また，[(式5)](#1992-1:eq:5)までわかれば$Q(x) = \log (x+1)$から容易に$x=\sqrt{2}-1$を導くことができる．
  この時の最小値$F(x)$は，[(式7)](#1992-1:eq:7)に[(式5)](#1992-1:eq:5)を代入すれば$Q$に関する項が全て消えて
