...
2 **\[解\]** $xy$平面で切断してかんがえる．
対称性から球の中心は$y$軸上にある．
そこで、円の中心をP$(0, t)$として，この円Cを $$\begin{align}
    x^2 + (y-t)^2 = r^2 \label{1993-3:eq:1}
\end{align}$$ とかける．元の水面の高さは$4$だから求める値$s$は
$$\begin{align}
    s = 4 - t
\end{align}$$ である．
...
<img
src="/Math-Solutions/images/tikz/sample_todai-zenki-1993-3_tikz_1.svg" />
<figcaption><span
...
<img
src="/Math-Solutions/images/tikz/sample_todai-zenki-1993-3_tikz_2.svg" />
<figcaption><span
...
<img
src="/Math-Solutions/images/tikz/sample_todai-zenki-1993-3_tikz_3.svg" />
<figcaption><span
...
また，CがQを通る条件から $$\begin{align}
     & u^2+(u^2-t)^2=r^2                            \\
    \therefore
     & u^4+(1-2t)u^2+t^2-r^2=0  \label{1993-3:eq:3}
\end{align}$$
である．対称性から円は放物線と$(u,u^2), (-u,u^2)$のみで共通点を持たないといけないから，
[\[1993-3:eq:3\]](#1993-3:eq:3){reference-type="ref+label"
reference="1993-3:eq:3"}が実数解として重解のみを持てば良い．

**$1^{\circ}$ $0<t \le \frac{1}{2}$ の時**

[\[1993-3:eq:2\]](#1993-3:eq:2){reference-type="ref+label"
reference="1993-3:eq:2"}を満たす$u$は$u=0$のみである．
[\[1993-3:eq:3\]](#1993-3:eq:3){reference-type="ref+label"
reference="1993-3:eq:3"}に代入して $$\begin{align*}
    t^2-r^2 = 0
\end{align*}$$ で，$r,t>0$から$r=t$である．
この時$3^{\circ}$のようになる． 求める$s=4-t$は $$\begin{align}
...
[\[1993-3:eq:2\]](#1993-3:eq:2){reference-type="ref+label"
reference="1993-3:eq:2"}から $$\begin{align*}
    u =
    \begin{dcases}
      0 \\
      \sqrt{t-\frac{1}{2}}
...
$u=0$の時は[\[1993-3:eq:3\]](#1993-3:eq:3){reference-type="ref+label"
reference="1993-3:eq:3"}が成立するためには$r=t$が必要だが，
この時[\[1993-3:eq:3\]](#1993-3:eq:3){reference-type="ref+label"
reference="1993-3:eq:3"}は $$\begin{align*}
    u^2(u^2+(1-2t))=0
\end{align*}$$
...
reference="1993-3:eq:3"}に代入して $$\begin{align*}
    ...
    \therefore
     & t = r^2 + \frac{1}{4}
\end{align*}$$
であることが必要である．この時[\[1993-eq:5\]](#1993-eq:5){reference-type="ref+label"
reference="1993-eq:5"}より $$\begin{align*}
...
逆にこの時，[\[1993-3:eq:3\]](#1993-3:eq:3){reference-type="ref+label"
reference="1993-3:eq:3"}は $$\begin{align*}
     & u^4 + (1-2t)u^2 + \left(t^2-t + \frac{1}{4}\right) = 0 \\
    \therefore
     & \left[u^2-\left(t-\frac{1}{2}\right)\right]^2 = 0
\end{align*}$$ となって$u$について重解のみを持ち十分である．
従って求める値$s$は $$\begin{align}
    s
     & = 4 - t                                                                                                           \\
     & = 4 - \left(r^2 + \frac{1}{4}\right)                                                                              \\
     & = \frac{15}{4} - r^2                 & \left(\frac{1}{2} \le  r  \le \frac{\sqrt{17}}{2}\right) \label{1993-eq:6}
\end{align}$$ である．

最後に，$1^{\circ}$の場合について考える．
この時[\[1993-eq:5\]](#1993-eq:5){reference-type="ref+label"
reference="1993-eq:5"}より，円の中心の$y$座標が $$\begin{align}
    t \ge \frac{9}{2} \label{1993-eq:7}
\end{align}$$ であって，また円$C$が$(\pm2, 4)$を通るから
$$\begin{align*}
    2^2 + (4-t)^2 = r^2 \\
    \therefore
    (4-t)^2 = r^2 -4
\end{align*}$$
である．$t$の条件[\[1993-eq:7\]](#1993-eq:7){reference-type="ref+label"
reference="1993-eq:7"}から$r$の条件は $$\begin{align*}
    r^2 -4 \ge \frac{1}{4} \\
    r \ge \frac{\sqrt{17}}{2}
\end{align*}$$ である．

この時円と放物線を連立した[\[1993-3:eq:3\]](#1993-3:eq:3){reference-type="ref+label"
reference="1993-3:eq:3"}は $$\begin{align*}
...
\end{align*}$$ だから，交点は$u=\pm 2, \pm\sqrt{2t-5}$である．
$t \ge \frac{9}{2}$の時$\sqrt{2t-5}\ge 2$であるから，$0\le x <2$に他の交点は存在せず十分である．
以上から$s$は $$\begin{align}
    s
     & = 4 - t                                                                      \\
     & = - \sqrt{r^2-4} & \left(r \ge \frac{\sqrt{17}}{2} \right) \label{1993-eq:8}
\end{align}$$

以上三つの場合わけ[\[1993-3:eq:4,1993-3:eq:6,1993-3:eq:8\]](#1993-3:eq:4,1993-3:eq:6,1993-3:eq:8){reference-type="ref+label"
reference="1993-3:eq:4,1993-3:eq:6,1993-3:eq:8"}をまとめて，$s$を$r$の関数として表すと
$$\begin{align*}
    s & =
    \begin{dcases}
...
$$\begin{align*}
    ...
    \end{dcases}
\end{align*}$$ で，これが4より大きいかで場合分けする． $$\begin{align*}
    \begin{dcases}
      0 < r \le \frac{3}{2} \text{の時} & y \le 4 \\
      \frac{3}{2} < r \text{の時}       & y \ge 4
    \end{dcases}
\end{align*}$$ だから，各場合わけは以下のようになる．

**㋐ $0 < r \le \frac{3}{2}$ の時**

$V(r)$は球の体積に等しく $$\begin{align*}
    V(r) = \frac{4}{3}\pi r^3
\end{align*}$$ であり，これは$r$について単調増加である．

**㋑ $\frac{3}{2} \le r$ の時**

<figure id="1993-3:fig:4" data-latex-placement="H">
<img
src="/Math-Solutions/images/tikz/sample_todai-zenki-1993-3_tikz_4.svg" />
<figcaption>Tは斜線部．</figcaption>
</figure>

Tの概形は[4](#1993-3:fig:4){reference-type="ref+label"
reference="1993-3:fig:4"}の斜線部である． 従って $$\begin{align*}
    V(r)
...
\end{align*}$$ であるから，$V(r)$の一階微分は $$\begin{align*}
    V'(r)
     ...
     & = \begin{dcases}
         ...
         \end{dcases}
\end{align*}$$ である．
従って$V(r)$の増減表は[1](#1993-3:table:1){reference-type="ref+label"
reference="1993-3:table:1"}となる．
...
したがって$V(r)$が最大になるのは $$\begin{align*}
    r=\frac{-1+2\sqrt{5}}{2}
\end{align*}$$ の時である．$\cdots$(答)
