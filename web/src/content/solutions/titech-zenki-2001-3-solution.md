...
question: "3"
type: "solution"
title: "TITECH 2001 zenki Q3 (solution)"
---
...
**(1)** $P_N(1)$：1回目で$1$が出れば良く，$P_N(1)=\dfrac1N$．
...
**(2)** $N=3$として，同様に和がちょうど4，5となる最初の到達までの数列を数え上げる．

**$P_3(4)$：** 長さ2（和4，$\{1,3\},\{2,2\}$）：$(1,3),(3,1),(2,2)$の3通り．長さ3（和4，最小値1が3つ以上必要，分割$1+1+2$）：$3$通り．長さ4（分割$1+1+1+1$）：$1$通り．
...
**$P_3(5)$：** 長さ2（分割$2+3$）：2通り．長さ3（分割$1+1+3$：3通り，$1+2+2$：3通り，計6通り）．長さ4（分割$1+1+1+2$）：4通り．長さ5（分割$1+1+1+1+1$）：1通り．
...
**(3)** $k\le N$とする．$S_k=\displaystyle\sum_{\ell=1}^kP_N(\ell)$とおく．1回目に引いた札の値で場合分けすると，$1\le k+1\le N$の時
