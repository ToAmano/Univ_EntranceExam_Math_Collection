...
question: "3"
type: "solution"
title: "KYODAI 1996 kouki Q3 (solution)"
---
...
<figure id="1996-3:fig:1">
  <img src="/Univ_EntranceExam_Math_Collection/images/tikz/kyodai/kouki/1996/3/fig_1.svg" alt="図 1" />
...
$$
  ...
  $\eqref{1996-3:eq:1,1996-3:eq:2}$から
...
$$
  ...
  である．
  $\eqref{1996-3:eq:2}$の両辺$\arg$をとって，
...
\begin{align*}
|P_{k+1}Q_{k}|
     & = 2\left|\sin\left(\alpha_{k+1}-\frac{\pi}{3}\right)\right|\\& = 2\left|\sin\left(\alpha_{k}+\frac{\pi}{3}\right)\right|\quad(\because\text{$\eqref{1996-3:eq:3}$})
\end{align*}
$$
...
\begin{align*}
L
     & = \sum_{k=1}^3 \{ |P_k Q_k| + |Q_k P_{k+1}| \}\\& = 2 \sum_{k=1}^3 |\sin\alpha_k| + 2 \sum_{k=1}^3 \left|\sin(\alpha_k + \frac{\pi}{3})\right|\\& = 2 \sum_{k=0}^{5}\left|\sin(\alpha_1 + \frac{k\pi}{3})\right|\quad(\because\text{$\eqref{1996-3:eq:3}$})
\end{align*}
$$
...
L
    ...
    = & 4\left[\sin(\alpha_1) + \sin\left(\alpha_1+\frac{\pi}{3}\right) + \sin\left(\alpha_1+\frac{2\pi}{3}\right)\right]\quad(\because\text{$\eqref{1996-3:eq:4}$})
\end{align*}
$$
...
### (2)
の最大値のとき，P，Qの作る図形は正六角形となり，この一周の長さは確かに$8$である．