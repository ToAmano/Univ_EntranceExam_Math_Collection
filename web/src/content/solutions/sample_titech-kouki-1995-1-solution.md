...
\(1\) [1](#1995-1:fig:1){reference-type="ref+label"
reference="1995-1:fig:1"}のように，立方体の頂点A, B, C, D, E, F, G,
Hに対し題意の3面を ABCD, AEFB, AEHDとする. 各球の中心は
立方体及び球の対称性から対角線AG上にある．
$S_n$の中心を$O_n$，半径を$r_n$とおく．
断面AEGCを[2](#1995-1:fig:2){reference-type="ref+label"
reference="1995-1:fig:2"}に示す．$AC=2\sqrt{2}$,$AE=2$より，$\angle\mathrm{GAC}=\theta$と置くと
$$\begin{align}
...
\end{align}$$ が成り立つことに注意する．

<figure id="1995-1:fig:1" data-latex-placement="H">
<img
src="/Math-Solutions/images/tikz/sample_titech/kouki/1995/1/fig_1.svg" />
<figcaption>立方体と頂点の定義</figcaption>
</figure>

<figure id="1995-1:fig:2" data-latex-placement="H">
<img
src="/Math-Solutions/images/tikz/sample_titech/kouki/1995/1/fig_2.svg" />
<figcaption>断面AEGC</figcaption>
</figure>

半径$r_n$に関する漸化式を導出することで$r_n$の一般項を求める．
円$S_n$と$S_{n+1}$に着目して[3](#1995-1:fig:3){reference-type="ref+label"
reference="1995-1:fig:3"}を考える．
...
\end{align*}$$ と表されるので，$r_n$と$r_{n+1}$の関係は $$\begin{align}
    r_{n}
            & =r_{n+1}+(r_{n}+r_{n+1}\sin\theta)        \\
    \therefore
    r_{n+1} & = \frac{1-\sin\theta}{1+\sin\theta} r_{n}
\end{align}$$ となる．$r_0=1$と合わせると，この等比級数の解は
$$\begin{align}
    r_{n}
     & = \left(\frac{1-\sin\theta}{1+\sin\theta}\right)^n \\
     & = (2-\sqrt{3})^n
\end{align}$$
となる．ただし，[\[1995-1:eq:1\]](#1995-1:eq:1){reference-type="ref+label"
reference="1995-1:eq:1"}を用いた．$\cdots$(答)
...
$0<(2-\sqrt{3})^3 < 1$ だから求める体積$V$は $$\begin{align*}
