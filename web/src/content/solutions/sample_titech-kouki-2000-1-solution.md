...
\(2\) 新しく \begin{align*}
    g(x) = x^2+(1-a)x+b
\end{align*}
とおく．$f(x)$の三次の係数が正だから，$g(x)$の解の数によって関数$f(x)$の概形が1の4パターンに変化する．
...
1.  $f(x)$が異なる三つの実数解を持つとき．\
    ...
    \end{align*}
    (2000-1:eq:2\)より，求める条件は \begin{align*}
                 & \begin{dcases}
                     -a \le 0 \\
...
2.  $f(x)$が異なる二つの実数解を持つとき．\
    ...
    後者の条件は，(2000-1:eq:3\)より \begin{align*}
                b & = - 2a^2+a        \\
                D & = (a-1)^2 -4b > 0
    \end{align*}
...
\end{align*} これを整理して \begin{align*}
    ...
    \end{dcases}
\end{align*} である．
$ab$平面にこの条件を図示すれば，2の斜線部が求める答えである．
ただし，実線部および境界を含む．
...
結果的に1のように5パターンの場合わけを処理すれば解答に辿り着ける．

簡単な検算としては，$f(0)=ab$で，これが条件より$0$以上なので$ab\ge 0$であり，
最後の2はこの条件を満たしているのでぱっと見大丈夫そうなのが確認できる．
