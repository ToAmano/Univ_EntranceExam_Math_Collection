...
question: "2"
type: "solution"
title: "TITECH 2001 zenki Q2 (solution)"
---
...
**$z\le0$の部分：** 原点から$z\le0$の点へは，そこへ向かう直線が常に$z\le0$内にあるから，速さ$a$で直進するのが最適で，1秒で到達できるのは半径$a$の球内．よってこの部分は半球$x^2+y^2+z^2\le a^2,\ z\le0$．

**$z>0$の部分：** 点$Q(X,Z)$（$Z>0$）へ到達する一つの方法として，時刻$t$（$0\le t\le1$）まで$x$軸の正方向（$z=0$の境界，速さ$a$が使える）へ進み点$(at,0)$に達し，そこから残り時間$1-t$でまっすぐ$Q$へ向かう（速さ$1$以下）という進み方を考える．これで$Q$に到達できる条件は，ある$t\in[0,1]$に対して
...
**$1^\circ$ $t_0\le0\iff X\le1/a$の時：** $f$は$[0,1]$で単調増加だから，条件は$f(0)\le0$：
...
**$2^\circ$ $0\le t_0\le1\iff1/a\le X\le a$の時：** 条件は$D\ge0$：
...
**A（半球）：** $V_A=\dfrac43\pi a^3\cdot\dfrac12=\dfrac23\pi a^3$

**B（円錐台，⑩の直線を回転）：** 底円半径$a$，高さ$a/\sqrt{a^2-1}$の円錐から，底円半径$1/a$，高さ$\dfrac1{a\sqrt{a^2-1}}$の円錐を引いたもの：
