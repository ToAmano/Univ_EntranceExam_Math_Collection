...
1.  $\a$が$0<\a<1$を満たす有理数ならば，区間$0\le x\le1$の上で不等式$1+\dfrac{\a}{2}x\le(1+x)^\a$が成り立つことを示せ．

2.  $2^{200}$の桁数はいくつか．またその最上位の数は何か．その理由を述べよ．
               \begin{itemize}
               [注1]．例えば$2^{10}=1024$の桁数は$4$，最上位の数は$1$である．なおこの数が$10^3$に近いことに注意せよ．
               [注2]．$\log_{10}2=0.3010$であるが，この数値を証明に用いてはならない．
               \end{itemize}

3.  $0.300<\log_{10}2<0.302$であることを示せ．

4.  [注1]．例えば$2^{10}=1024$の桁数は$4$，最上位の数は$1$である．なおこの数が$10^3$に近いことに注意せよ．

5.  [注2]．$\log_{10}2=0.3010$であるが，この数値を証明に用いてはならない．
...
1.  $f(x)=(1+x)^\a-\left(1+\dfrac{\a}{2}x\right)$とおく．
...
2.  $2^{200}=(2^{10})^{20}$だから，$2^{10}>10^3$ゆえ，
          \begin{align}
          2^{200}>10^{60}\label{1}
          \end{align}
      である．
