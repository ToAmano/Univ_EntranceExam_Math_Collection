...
$$
    ...
    である．
    題意を示すには，$\eqref{2000-1:eq:1}$をみたす$k$次多項式$P_k(x)$が唯一存在することを示せば良い．
    証明は数学的帰納法による．

### (1)
から、$k=1,2$の時は成立するため，以下$k \leqq l \ (l \in \mathbb{N} \geqq 2)$の成立を仮定する．
    二項定理より
...
$$
    ...
    次に，二つ目の条件は$\eqref{2000-1:eq:2}$に$x=0$を代入して，仮定より
...
$$
    ...
    以上より$\eqref{2000-1:eq:2}$の$P_{l+1}(x)$は条件(C)をみたす．
...
$$
    ...
    の二つの条件を示せば良い．
    まず$\eqref{2000-1:eq:7}$について，
    条件(C)より$P_{k}(0)=0$であり，また$\eqref{2000-1:eq:4}$より$Q_{k}(0)=0$も従う．
    これらを$\eqref{2000-1:eq:5}$に代入して$R_{0}=0$であるから示された．

    よって$\eqref{2000-1:eq:5}$は
...
$$
    ...
    と書き直せる．このもとで$\eqref{2000-1:eq:6}$を示す．
...
$$
    ...
    であり，左辺は$\eqref{2000-1:eq:1}$を代入して
